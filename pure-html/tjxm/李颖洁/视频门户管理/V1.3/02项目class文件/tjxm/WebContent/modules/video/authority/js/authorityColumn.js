$(function(){
	...
	$("#columnId").val(theRequest.columnId);
...
function addTabFn(obj){
    ...
    var trCustom = "";
    trCustom += '<tr>';
    trCustom += '<td class="text-center" style="width: 5%"><input type="checkbox" name="checkboxName" class="checkLine" value="option1"></td>';
...
function saveData(){
    ...
    if(checkResult){
        ...
        $.ajax({
        	type: "POST",
        	url:"/video/authority/contentFbZwqx/saveColumn",
...
function checkForm(){
	...
	if($subtableRows.size() > 0){
		$subtableRows.each(function (i,tr) {
			...
			//if(deptId=="" ||  deptId==null || detpName=="" || detpName==null || checkedSize==0){
		...
		if(sum==$subtableRows.size()){
			tableFlag = null;
		}else if(tableFlag==true && sum>0&&sum<3){
			msg = "若不再添加【指定部门下的职务权限】，请删除空行！";
			tableFlag = false;
		}
	}else{
...
function getFormData(){
    ...
    var data = {
        ...
        zwqxList: zwqxList,
        groupList: groupList,
        deptZwqxList: deptZwqxListList,
        deptZwqxListList: deptZwqxList
    };
    console.log(data);
    return data;
}
