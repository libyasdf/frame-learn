$(function() {
	...
	if ($("#id").val() != "") {
		...
		httpRequest("get", "/zhbg/kqgl/absenteeism/edit", datas, function(data) {
				...
				var theRequest = GetRequest();
				if(theRequest.oper=='VIEW'){
					if(data.data.absenteeismType=='0'){
						$("#absenteeismType").text("旷工");
					}else if(data.data.absenteeismType=='1'){
						$("#absenteeismType").text("无故迟到");
					}else if(data.data.absenteeismType=='2'){
						$("#absenteeismType").text("无故早退");
					}else if(data.data.absenteeismType==''){
						$("#absenteeismType").text("");
					}
				}
			});
	} else {
