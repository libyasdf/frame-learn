...
public class MeetingNoticeBackImpl implements MeetingNoticeBackService{
		...
		public PageImpl getPageListDraft1(Pageable pageable, PageImpl pageImpl, String startDate, String endDate,
			...
			sb1.append("  ) a1 where ROWNUM <= ? ) where RN >=  ?");
 ...
 +" sum(case when b.remark is not null then 1 else 0 end) as remark_count");
		 ...
		 * @author 李颖洁  
		 * @date 2020年1月1日上午1:44:22
		 * @param applyId 会议申请id
		 * @return
		 */
		@Override
		public String getAllAttendPersonIds(String applyId) {
			String userIds = "";
			String sql = "select to_char(wm_concat(nvl(t.attend_person_id,'') ||nvl(t.attend_person_chu_id,'') || nvl(t.attend_person_leave_id,''))) attendPersonId from hygl_notice_back t where t.meeting_apply_id = ?";
