...
public class Task {
    ...
    /**
     * 
     * TODO 发送不走流程待办摘出来（定时和发布（当前日期跟考试开始时间一致）用）
     * @author 王磊
     * @Date 2019年4月11日 下午2:01:18
     * @param TestList
     */
    public void sendNoflowWaitDo(List<XxkhTestInfo> TestList){
    	if(TestList.size()>0){
            //如果当天的考试数量大于0那么就获取人员id发待办
            for(int i = 0;i< TestList.size();i++){
                ...
                for(int j = 0;j<list.size();j++){
                    ...
                    String testType = "";
                    if("fzks".equals(TestList.get(i).getType())){
                        testType = "法制法规";
                    }else if("zzllks".equals(TestList.get(i).getType())){
                        testType = "政治理论";
                    }else if("bmks".equals(TestList.get(i).getType())){
                        testType = "保密";
                    }else if("deptks".equals(TestList.get(i).getType())){
                        testType = "部门考试";
                    }

                    //拿到人员id发送不走流程待办
                    String content = "考试名称："+TestList.get(i).getName()+"\n" +
                            "所属大类：" +testType+"\n" +
                            "答题时长：" +TestList.get(i).getAnswerTime()+"\n" +
                            "难易程度：" +TestList.get(i).getDifficultyLevel()+"\n" +
                            "考试满分：" +TestList.get(i).getFullScore()+"\n"+
                            "及格分数："+TestList.get(i).getPassScore()+"\n" +
                    ...
                    //李颖洁20190329改动，原来传递的deptid 是接收人的处室id，导致接收不到待办，改为接收人所在的单位id
                    waitNoflw.setReceiveDeptName(list.get(j).getCandidateDeptId());//接收人所在单位
                    waitNoflw.setReceiveDeptId(list.get(j).getCandidateDeptId());//接收人所在单位id 必填
                    ...
                    sysWaitNoflowService.saveWaitNoflowSystem(waitNoflw,true,subject,content,messageURL+"?id="+TestList.get(i).getId());
