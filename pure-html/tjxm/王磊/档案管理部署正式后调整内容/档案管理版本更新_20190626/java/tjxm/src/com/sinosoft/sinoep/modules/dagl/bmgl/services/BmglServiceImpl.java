		...
		if("add".equals(type)){
			...
			if(all.equals(ranking)){
				...
				if(Integer.parseInt(ranking)==3){
					...
					if(DAGLCommonConstants.ARCHIVE_FLAG[0].equals(archiveFlag)){
						//查询父表记录（单位库）
						String sql = "select t.* from "+ppTableName+" t where 1=1 and t."+ppmColName+" = '"+sColData+"' and t.archive_flag='"+DAGLCommonConstants.ARCHIVE_FLAG[0]+"'";
			...
			if(DAGLCommonConstants.ARCHIVE_FLAG[0].equals(archiveFlag)){
				//查询父表记录（单位库）
				String sql = "select t.* from "+pTableName+" t where 1=1 and t."+pmColName+" = '"+sColData+"' and t.archive_flag='"+DAGLCommonConstants.ARCHIVE_FLAG[0]+"'";
				...
				if(pTableDataList.size()>0){
					...
					if(Integer.parseInt(ranking)==3){
						...
						if(DAGLCommonConstants.ARCHIVE_FLAG[0].equals(ppArchiveFlag)){
							//查询父表记录（单位库）
							String ppsql = "select t.* from "+ppTableName+" t where 1=1 and t."+ppmColName+" = '"+ppsColData+"' and t.archive_flag='"+DAGLCommonConstants.ARCHIVE_FLAG[0]+"'";
							if(ppTableName.indexOf("_dak")==-1){
								//查父表记录条件:关联字段、立卷单位（上边的处室id）、创建人id一致
