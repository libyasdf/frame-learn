...
public class DaglSendFileServiceImpl implements DaglSendFileService {
	...
	public DaglSendFile editSendFile(DaglSendFile sendFile) {
		if(sendFile.getId()==null ||sendFile.getId()==""){
	...
	public int batchDelete(String ids) {
		String jpql = "delete from DaglSendFile t  where t.id in ("+ids+") ";
	...
	public Page<DaglSendFile> list(DaglSendFile sendFile, PageImpl pageImpl,Pageable pageable) {
		...
		if(!StringUtils.isBlank(sendFile.getTitle())){
			querySql.append("   and t.title like '%"+sendFile.getTitle()+"%'");
		}
		if(!StringUtils.isBlank(sendFile.getFileDept())){
			querySql.append("   and t.fileDept like '%"+sendFile.getFileDept()+"%'");
		}
	...
	public List<DaglSendFile> getTidyRecodedList(String ids) {
		String querySql = "";
		//全部列表查询
		querySql = "select t.* from dagl_send_file t	where t.visible = '"+CommonConstants.VISIBLE[1]+"' and t.id in ("+ids+")";
		List<DaglSendFile> page = jdbcTemplate.query(querySql,new BeanPropertyRowMapper<>(DaglSendFile.class) );
		return page;
	}
	...
	public int tidyRecode(ContrastingRelations relations, String ids) throws Exception{
		...
		for(int k=0;k<notNullData.size();k++){
			...
			if(notNullData.get(k).getColumnName().indexOf("archive_flag")!=-1){//archiveFlag组卷标识字段
				...
				arrval[k] = "'"+UserUtil.getCruChushiName()+"'";//赋值为当前登录人的处室名称
				flag = true;
			}else{
	...
	public int updateState(String ids,String status) {
		...
		String jpql = "update  DaglSendFile t set t.state = '"+status+"' where t.id in ("+ids+")";
		log.info(jpql);
		return dao.update(jpql);
	}
	...
	public List<DaglSendFile> getDaglSendFileList(DaglSendFile daglSendFile,String ids) {
		...
		if(!ids.equals("")){
			...
			if(!StringUtils.isBlank(daglSendFile.getCreChushiId())){
				querySql.append("   and t.creChushiId = '" + daglSendFile.getCreChushiId() + "'");
			}
			if(!StringUtils.isBlank(daglSendFile.getType())){
				querySql.append("   and t.type = '" + daglSendFile.getType() + "'");
			}
			if(!StringUtils.isBlank(daglSendFile.getState())){
				querySql.append("   and t.state = '" + daglSendFile.getState() + "'");
			}
			if(!StringUtils.isBlank(daglSendFile.getSecurityClass())){
				querySql.append("   and t.securityClass = '" + daglSendFile.getSecurityClass() + "'");
			}
			if(!StringUtils.isBlank(daglSendFile.getTitle())){
				querySql.append("   and t.title like '%"+daglSendFile.getTitle()+"%'");
			}
			if(!StringUtils.isBlank(daglSendFile.getFileDept())){
				querySql.append("   and t.fileDept like '%"+daglSendFile.getFileDept()+"%'");
			}
	...
	public int batchUpdate(String ids,String type) {
		String jpql = "update DaglSendFile t set t.type ='"+type+"' ";
	...
	public List<DaglSendFile> getList(DaglSendFile daglSendFile) {
		...
		if(!StringUtils.isBlank(daglSendFile.getSecurityClass())){
			querySql.append("   and t.security_class = '"+daglSendFile.getSecurityClass()+"'");
		}
		if(!StringUtils.isBlank(daglSendFile.getTitle())){
			querySql.append("   and t.title like '%"+daglSendFile.getTitle()+"%'");
		}
		if(!StringUtils.isBlank(daglSendFile.getFileDept())){
			querySql.append("   and t.file_dept like '%"+daglSendFile.getFileDept()+"%'");
		}
		String timeRange = daglSendFile.getTimeRange();
		if (StringUtils.isNotBlank(timeRange)) {
			String startTime = timeRange.substring(0, (timeRange.length() + 1) / 2 - 1).trim();
		 ...
		 list = jdbcTemplate.query(querySql.toString(), new BeanPropertyRowMapper<>(DaglSendFile.class));
		return list;
	}
}
