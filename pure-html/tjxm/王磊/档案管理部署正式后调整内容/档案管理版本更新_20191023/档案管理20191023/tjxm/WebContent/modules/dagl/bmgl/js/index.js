...
function initPanel() {
    ...
    $('.tab-panel-container').append($('<div class="tab-panel">' +
        ...
        '                                   </button>' +
        '                                   <button onclick="easyFind()"  type="button" class="list_table_btn2" style="margin-right:46.8%;font-size:15px">' +
    ...
    $.ajax({
        ...
        success: function (data) {
           ...
           // var marks=[];
            ...
            // 添加复选框
            $('#table thead tr:first-child').prepend($('<th style="text-align: center;"><input type="checkbox" id="all" >全选 <input type="checkbox" id="alll" >反选</th>'));
...
function initTableBody(pageNumber) {
    ...
    $.ajax({
        ...
        data: {
            ...
            all:all,
            SortName:orderName,
            ljdanweiAndRenyuan:JSON.stringify(object)//立卷单位-20190710-王富康
        },
        type: 'post',
        dataType: 'json',
        success: function (data) {
            ...
            for (var i = 0; i < data.length; ++i) {
            	...
            	if(data[i].ARCHIVE_ENTITY_STATUS=="已归档"){
            		continue;
            	}
                // 渲染表格内容
                var $tr = $('<tr></tr>')
...
function newlyAdded() {
    ...
    layer.open({
        ...
        content: '/modules/dagl/bmgl/form.html?categoryCode=' + $.fn.zTree.getZTreeObj("treeDemo").getSelectedNodes()[0].categoryCode+"&fids="+getFids()+"&notAdmin="+parameter.notAdmin,
        area: ['1100px', '660px'],
        offset: '30px',
        resize: false,
        maxmin: true,
...
function dels(event) {
    ...
    $.ajax({
        ...
        success: function (result) {
            var alreadysealUpCount = 0;
            for( var i=0;i<result.data.rows.length;i++){
                if (null != result.data.rows[i].FHR_ID || "已移交待汇总" == result.data.rows[i].ARCHIVE_ENTITY_STATUS) {
                    alreadysealUpCount++;
                }
            }

            if(0 == alreadysealUpCount){
    ...
    $.ajax({
        ...
        success: function (data) {
            if (null == data[0].FHR_ID) {
                ...
                layer.msg("无法删除：该项已被【"+data[0].FHR_NAME+"】封号！", {
                    icon: 5
                });
            }
        },
...
function modify(event) {
    ...
    var rid = $(this).parent().parent().find("td").eq(0).find("input").val();//getSelectedData
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function (data) {
            if (null == data[0].FHR_ID && ("待移交" == data[0].ARCHIVE_ENTITY_STATUS || "已退回待处置" == data[0].ARCHIVE_ENTITY_STATUS)) {
                // 打开修改弹框
                layer.open({
                    type: 2,
                    title: ['修改','font-size:16px;font-weight:bold;'],
                    content: '/modules/dagl/bmgl/modify.html?categoryCode=' + $.fn.zTree.getZTreeObj("treeDemo").getSelectedNodes()[0].categoryCode+"&fids="+getFids()+"&notAdmin="+parameter.notAdmin,
                    area: ['1100px', '660px'],
                    offset: '30px',
                    resize: false,
                    maxmin: true,
                ...
                if("待移交" != data[0].ARCHIVE_ENTITY_STATUS && "已退回待处置" != data[0].ARCHIVE_ENTITY_STATUS){
                    layer.msg("无法编辑：只能修改档案实体状态为【待移交】和【已退回待处置】的档案！", {
                        icon: 5
                    });
                }else if(null != data[0].FHR_ID && ("待移交" == data[0].ARCHIVE_ENTITY_STATUS || "已退回待处置" == data[0].ARCHIVE_ENTITY_STATUS)){
                    layer.msg("无法编辑：该项已被【"+data[0].FHR_NAME+"】封号！", {
                        icon: 5
                    });
                }
            }
