				...
				if(query.size()==2){
						...
						if(pTableList.size()>0) {
							...
							if(pTableDataList.size()>0){
								...
								String insertPData = "";
								if(count>1){
									insertPData = "insert into "+pTableName +"(recid,"+commonalityColumns+dhCol+"maintitle,archive_entity_status,basefolder_unit,page_num,piece_num,archive_flag";
									if(pigeonholeDateNum==0){
										insertPData += ",pigeonhole_date";
									}
									insertPData+=") values ('"+uuid+"',"+pVals+dhVal+"'中转库归档文件，请及时修改','"+DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[0]+"','"+ljdwName+"','0','0','"+DAGLCommonConstants.ARCHIVE_FLAG[0]+"'";
									if(pigeonholeDateNum==0){
										insertPData += " ,'"+DateUtil.getDateText(new Date(), "yyyyMMdd")+"'";
									}
									insertPData+=")";
								}else{
									insertPData = "insert into "+pTableName +"(recid,"+commonalityColumns+dhCol+"maintitle,archive_entity_status,basefolder_unit,page_num,piece_num";
									if(pigeonholeDateNum==0){
										insertPData += ",pigeonhole_date";
									}
									insertPData += ") values ('"+uuid+"',"+pVals+dhVal+"'中转库归档文件，请及时修改','"+DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[0]+"','"+ljdwName+"','0','0'";
									if(pigeonholeDateNum==0){
										insertPData += " ,'"+DateUtil.getDateText(new Date(), "yyyyMMdd")+"'";
									}
									insertPData +=")";
