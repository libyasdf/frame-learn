    ...
    nodetpl.render(html, {data: fileList}, function (html) {
        ...
        for (var i = 0; i < fileList.length; i++) {
            if ($("#opertation").val() != "VIEW") {
                ...
                //$("#securityClass" + (i + 1)).val(fileList[i].securityClass);
...
function addTabFn(obj) {
    ...
    trCustom += '</td>';
    trCustom += '<td class="text-center" style="width: 6%">';
    trCustom += '	<select CK_info="立卷单位"  placeholder="请选择立卷单位" CK_type="ck_required" class="form-control" id="ljdwMark' + subTableLineNum + '" name="ljdwMark">';
    trCustom += getSelect("dagl_ljdw", "1");
    trCustom += '</select>';
    trCustom += '</td>';
    trCustom += '<td class="text-center" style="width: 6%">';
    trCustom += '	<select CK_info="密级"  placeholder="请输入密级" CK_type="ck_required" class="form-control" id="securityClass' + subTableLineNum + '" name="securityClass">';
    trCustom += getSelect("04", "1");
    trCustom += '</select>';
    trCustom += '</td>';
    trCustom += '<td class="text-center" style="width: 8%">';
    trCustom += '	<input type="text" CK_type="ck_required"  CK_info="页数"  placeholder="请输入页数" class="form-control" id="pageNum' + subTableLineNum + '" name="pageNum" />';
    trCustom += '</td>';
    trCustom += '</tr>';
    return trCustom;
}
...
function getFormData() {
    ...
    $subTableRows.each(function (i, index) {
        var file = {
            ...
            securityClass: $(index).find('[name="securityClass"]').find("option:selected").text(),
