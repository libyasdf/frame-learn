...
public class ConfigServiceImpl implements ConfigService{
    ...
    public List<Config> queryOne(String dutyMonth) {
        ...
        String sql="select t.* from zbgl_duty_config t where t.visible = '"+CommonConstants.VISIBLE[1]+"' and t.duty_month = ?";

        List<Config> list = jdbcTemplate.query(sql,new BeanPropertyRowMapper<>(Config.class),dutyMonth);
        return list;
    }
     ...
     **/
    @Override
    public Config save(Config con,String type) {
            //如果id为空，则为新增
        if("".equals(type)){
            //如果type为空则证明不是从定时器过来的，人员登录的情况下进行的新增
    ...
    public List<Config> getConfigByMonthAndUnitId(String month){
        String sql="select t.* from zbgl_duty_config t where t.visible = '"+CommonConstants.VISIBLE[1]+"' and t.duty_month = ? and t.unit_id like '%"+UserUtil.getCruChushiId()+"%'";

        List<Config> list = jdbcTemplate.query(sql,new BeanPropertyRowMapper<>(Config.class),month);
        return list;
    }
}
