...
public class GradesearchController {
		...
		try {
			...
			if (getbyTestContent.size()>0) {
				for(XxkhUserPaper paper:getbyTestContent) {
					if(paper.getPaperObjectiveScore()==null||paper.getPaperObjectiveScore().equals("")) {
						if(paper.getArtificialMarkingState().equals("1")) {
							if (Integer.parseInt(paper.getPaperSubjectiveScore())>=Integer.parseInt(paper.getPassScore())) {
								paper.setIsJiGe("是");
							}else {
								paper.setIsJiGe("否");
							}
							paper.setSumScore(Integer.parseInt(paper.getPaperSubjectiveScore())+"");
						}else {
							paper.setIsJiGe("否");
	 ...
	 * @author 李颖洁  
	 * @date 2018年10月9日上午9:35:49
	 * @param info
	 * @return JSONObject
	 */
	@LogAnnotation(value = "query",opName = "查询个人考试试卷信息")
	@ResponseBody
	@RequestMapping(value="getPersonalTestPaper")
	public JSONObject getPersonalTestPaper(XxkhTestInfo info){
		JSONObject json = new JSONObject();
		JSONObject data = new JSONObject();
		json.put("flag", "0");
		try {
			List<XxkhUserPaper> list = gradesearchService.getPersonalTestPaper(info);
