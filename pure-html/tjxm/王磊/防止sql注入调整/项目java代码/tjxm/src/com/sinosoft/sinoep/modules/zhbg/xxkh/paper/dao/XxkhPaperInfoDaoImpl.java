...
public class XxkhPaperInfoDaoImpl implements XxkhPaperInfoRepository {
	...
	public List<XxkhTestInfo> isCanDelete(String paperID) {
		...
		String sql = "select * " +
				...
				"         where t.paper_id = ? ) " +
				"   and i.visible = '1' ";
		return jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(XxkhTestInfo.class),paperID);
	}
	...
	public XxkhPaperInfo getTestResult(XxkhPaperInfo paperInfo, String testId, String userId){
		...
		String sql =	"	SELECT t.*, \n" +
						...
						"   and s.test_id = ? \n" +
						"   and s.user_id = ? ";
		//防止sql注入调整 王磊 20190426
		List<Object> list = new ArrayList<>();
		list.add(paperInfo.getId());
		list.add(testId);
		list.add(userId);
		List<XxkhPaperInfo> xxkhPaperInfoLists= jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(XxkhPaperInfo.class),list.toArray());
