...
public class KqLogServiceImpl implements KqLogService {
	...
	public PageImpl getPageList(Pageable pageable, PageImpl pageImpl, String startDate, String endDate)
		...
		if (StringUtils.isNotBlank(startDate)) {
			querySql.append("   and t.cre_time >= ?");
		...
		//拼接最终sql
		String sumSql = "select sum(To_number(success_num)) from ("+querySql.toString()+") where visible = '"+ CommonConstants.VISIBLE[1]+"'";
		...
		sql = "select s.* from ("+sql1+"  where rownum<= ?) s where rn>= ?";
		para.add(after);
		para.add(pre);
		List<KqLog> list = jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(KqLog.class),para.toArray());
