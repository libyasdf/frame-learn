...
public class VideoColumnController {
    ...
    @LogAnnotation(value = "query",opName = "获取视频发布内容树")
    @RequestMapping("/getContentColumnTree")
    @ResponseBody
    public JSONObject getContentColumnTree(String columnName,String userId){
        return service.getContentColumnTree(columnName,userId);
    }
    
    @LogAnnotation(value = "query",opName = "根据父id获取栏目列表")
    @RequestMapping("getColumnList")
    @ResponseBody
    public PageImpl getColumnList(PageImpl pageImpl, String superId, String columnName, String columnCode,String isShow){
        Pageable pageable = new PageRequest(pageImpl.getPageNumber()-1,pageImpl.getPageSize());
        return service.getColumnPageList(pageable,pageImpl,superId,columnName,columnCode,isShow);
    }
    ...
    @ResponseBody
    public JSONObject getZdList1(PageImpl pageImpl,String superId){
    	JSONObject json = new JSONObject();
		json.put("flag", "1");
		List<VideoColumn>list=new ArrayList<VideoColumn>();
		try {
			  list = service.getZdList(superId);
    ...
    @ResponseBody
    public JSONObject getShowPage(String columnId,String isFirst){
        JSONObject json = new JSONObject();
        json.put("flag","0");
        ...
        try{
            if(StringUtils.isNotBlank(columnId)){
                ...
                if(  rolesNo.contains(VideoConstants.VIDEOFBGLYKEROLENO) || rolesNo.contains(VideoConstants.VIDEOFBGLYCHUROLENO) || "1".equals(isFirst)){
    ...
    @ResponseBody
    public VideoColumn getColumInfo(String columnId){
