...
public class SurveyAnswerManagerImpl extends
	...
	@Override
	public List<TQuestion> findAnswerDetail(SurveyAnswer answer) {
	 ...
	 * @return
	 */
	private int getquestionAnswer(String surveyAnswerId, TQuestion question) {
		int score = 0;
	...
	public Long getCountByIp(String surveyId, String ip) {
		...
		Long count;
		try {
			count = (Long) surveyAnswerDao.findUniObjs(hql, surveyId, ip);
		} catch (Exception e) {
			return (long) 0;
		}
		return count;
...
//		SurveyDirectory surveyDirectory = directoryManager.getSurvey(surveyId);
	 ...
	 * @param surveyAnswer
	 */
	private void exportXLSRow(XLSExportUtil exportUtil,String surveyAnswerId, List<TQuestion> questions,SurveyAnswer surveyAnswer) {
		int cellIndex = 0;
		for (TQuestion question : questions) {
			getquestionAnswer(surveyAnswerId, question);
			QuType quType = question.getQuType();
...
//		exportUtil.setCell(cellIndex++,  surveyAnswer.getIpAddr());//回答者IP
//		exportUtil.setCell(cellIndex++,  surveyAnswer.getCity());//IP所在地
		exportUtil.setCell(cellIndex++,  surveyAnswer.getDeptName());//所在部门
		exportUtil.setCell(cellIndex++,  surveyAnswer.getChuShiName());//所在处室
		exportUtil.setCell(cellIndex++,  new SimpleDateFormat("yyyy年MM月dd日 HH时mm分ss秒").format(surveyAnswer.getEndAnDate()));//回答时间
	}
	 ...
	 * TODO 
	 * @author 李利广
	 * @Date 2018年11月7日 上午10:37:12
	 * @param exportUtil
	 * @param questions
	 */
	private void exportXLSTitle(XLSExportUtil exportUtil, List<TQuestion> questions) {
...
//				List<QuChenColumn> quChenColumns = question.getColumns();
		...
		exportUtil.setCell(cellIndex++,  "所在部门");
		exportUtil.setCell(cellIndex++,  "所在处室");
	...
	public void delete(SurveyAnswer t) {
		if(t!=null){
			...
			//更新当前答卷的回答记录值
			// 得到题列表
			List<TQuestion> questions = questionManager.findDetails(t.getSurveyId(),"2");
			for (TQuestion question : questions) {
				String quId=question.getId();
				QuType quType = question.getQuType();
