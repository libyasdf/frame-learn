...
/*    console.log(groupFlag);*/
	...
	if(!deptFlag  && !userGroupFlag && !positionFlag && (!tableFlag || tableFlag==null)){
		msg = msg ? msg : "请至少选择一种权限范围";
	}
	//部门、职务、群组只要有一个不为false  并且  表格为true或者为空
	if((deptFlag || positionFlag || userGroupFlag) && (tableFlag==null ||tableFlag)){
		res = true;
	}
	//部门、职务、群组均为空，并且  表格有填写完整的一行
	if(!deptFlag  && !userGroupFlag && !positionFlag && tableFlag != null && tableFlag){
		res = true;
	}
	if(!res){
...
function openNoticeZtree(){
    openNoticeSelectZtree({'id':'sysUserId','name':'sysUserName','parentId':'sysDeptId','groupid':'sysGrupId','groupname':'sysGrupName','check':'s','cancle':'s','url':'/system/noticeGrup/getSurveyGroupTree','subId':Config.sysId})
}