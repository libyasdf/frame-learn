...
$(function(){
    //加载数据字典
    bootstrapSelect.dictionaries({selectID:'typeCode',mark:"xtsz_ryqz_fblx",global:whatformData,type:"1",bolRecord:true});
	...
	if($("#id").val() != ""){
		...
		httpRequest("get","/system/noticeGrup/view",datas,function (data){
			...
			debugger;
            setTimeout(function(){
                $('#typeCode').selectpicker('val', data.data.typeCode);
            },301)
...
function saveForm(){
    ...
    var key = whatformData[typeCode];
    $("#typeName").val(key.name);
	var res = "";
...
function getFormData(){
    var infoColumn ={
        ...
        officeScopeId:$('[name="officeScopeId"]').val()||"",
        officeScopeName:$('[name="officeScopeName"]').val()||"",
...
function openTree(){
    ...
    openSelectZtree({'id':'officeScopeId','name':'officeScopeName','type':'2','deptId':juId,'asyn':false,'level':'2','url':'/system/component/tree/deptTree'});
}
