$(function () {
   ...
   var meetingNoticeId= GetRequest().meetingNoticeId;
    ...
    function init() {
        $.ajax({
            url: "/zhbg/hygl/arrangeSeat/getList",
            async: false,
            data: {
    ...
    */
    function loadData(seat) {
        ...
        seatData[target].seats[row][col] = {
            ...
            attendDeptId: seat.attendDeptId,
    ...
    function isSeatHasOwner(positionText) {
        ...
        if (state === 3) {
            return {
                ...
                attendDeptName: seatData[target].seats[col][row - 1].attendDeptName,
                attendDeptId: seatData[target].seats[col][row - 1].attendDeptId
            }
        }
    ...
    */
    function updateSeatData(updateObject) {
        seatData[updateObject.target].seats[updateObject.row][updateObject.col] = {
            ...
            attendDeptName: updateObject.attendDeptName,
            attendDeptId: updateObject.attendDeptId,
        }
    }
    ...
    */
    function initManualSortPanel() {
        ...
        $('.manual-sort-right').append($(''
            ...
            + '<input type="checkbox" value="正处长" checked><span>处室主要领导</span>'
            + '</div>'
            + '<ul class="attendees-list"></ul>'
        ));

        for (var i = 0; i < selected.length; ++i) {
            $('.manual-sort-left').append($('<div class="ms-selected" data-seatid="" data-active="0" data-owner="" data-ownerid="" data-owner-current="" data-ownerid-current="" data-owner-deptid="" data-owner-deptname=""><span>' + $(selected[i]).html() + '</span><br><span></span></div>'));
            ...
            $('.manual-sort-left .ms-selected:eq(' + i + ')')
                ...
                .attr("data-owner-deptid", seatInfo.attendDeptId)
                .attr("data-owner-deptname", seatInfo.attendDeptName);
    ...
    */
    function loadgetAllAttendees() {
        layer.load(2);
        console.log('用户选择的座位信息************************************************************')
        console.log(userSelecte)
        if (userSelecte && userSelecte.length>0) {
            $.each(userSelecte,function (index,dom) {
                // console.log(userSelecte[index].getAttribute('data-owner'),userSelecte[index].getAttribute('data-ownerid'))
                var selectedSeat = $(userSelecte[index]).find('span').eq(0).text();
                var selectedOwnerid = $('.ms-worker-list[data-seat='+ selectedSeat +']').attr('data-ownerid');
                var selectedOwner = $('.ms-worker-list[data-seat='+ selectedSeat +']').find('.mswl-center h3').text();
                var ownerDeptid = $('.ms-worker-list[data-seat='+ selectedSeat +']').attr('data-owner-deptid');
                var ownerDeptname = $('.ms-worker-list[data-seat='+ selectedSeat +']').attr('data-owner-deptname');
                $(userSelecte[index]).attr('data-ownerid',selectedOwnerid);
                $(userSelecte[index]).attr('data-owner',selectedOwner);
                $(userSelecte[index]).attr('data-owner-deptid',ownerDeptid);
                $(userSelecte[index]).attr('data-owner-deptname',ownerDeptname);
            })
        }
        ...
        $.ajax({
            ...
            success: function (data, message, res) {
                ...
                for (var i = 0; i < data.length; ++i) {
                    ...
                    if (data[i].id !== undefined && data[i].id) {
                        ...
                        if (canAppend) {
                            $('.manual-sort-left').append($(''
                                ...
                                + 'data-owner-current="' + data[i].userNameFull + '" data-ownerid-current="' + data[i].userId + '" data-owner-deptid="'+data[i].attendDeptId+'" data-owner-deptname="'+data[i].attendDeptName+'">'
                    ...
                    if ($arrangedSeat) {
                        $('.manual-sort-right .attendees-list').append($(''
                            + '<div class="ms-worker-list" data-active="0" data-seat="" data-seatid="" data-ownerid="' + data[i].userId + '" data-seat-current=""  data-seatid-current="" data-owner-deptid="'+data[i].attendDeptId+'" data-owner-deptname="'+data[i].attendDeptName+'">'
                ...
                initMCustomScrollbar();
                hiddenPersonnel(1);
                drag.init('manual-sort-panel');
                $('#manualSort').on('hide.bs.modal', function () {
                    $('.' + drag.class_name).off('mousedown', '.ms-worker-list')
                })
            },
            error: function () {
                layer.msg('获取人员列表失败了，请稍后再试！',{icon:2});
                layer.closeAll('loading');
            }
        });
    }
    ...
    function manualSortSubmit() {
        ...
        for (var i = 0; i < selected.length; ++i) {
            ...
            seatDataJson.push({
                ...
                attendDeptId:$(selected[i]).attr("data-owner-deptid"),
                attendDeptName:$(selected[i]).attr("data-owner-deptname")
            });
        ...
        data.seatDataJson = seatDataJson;
        ...
        $.ajax({
            ...
            success: function (data, message, res) {
                ...
                if (Number(data.flag)) {
                    init();
                    opener.fresh();
                    layer.msg('排座成功！',{icon:1});
                } else {
                    layer.msg('排座失败了，请稍后再试！',{icon:2});
                    init();
                }
            },
            error: function (err) {
                layer.msg('提交失败了，请稍后再试！',{icon:2});
                layer.closeAll('loading');
                init();
                throw err;
            }
    ...
    function reserve() {
        ...
        $.ajax({
            ...
            success: function (flag, message, res) {
                ...
                if (flag.flag == 1) {
                    init();
                    layer.msg('预留座位成功！',{icon:1});
                }
            },
            error: function (err) {
                layer.msg('预留失败了，请稍后再试！',{icon:2});
                layer.closeAll('loading');
                throw err;
            }
        });
        ...
        $.ajax({
            ...
            success: function (flag, message, res) {
                ...
                if (flag.flag == "1") {
                    ...
                    init();
                    layer.msg('取消预留座位成功！',{icon:1});
                }
            },
            error: function (err) {
                layer.msg('取消失败了，请稍后再试！',{icon:2});
                layer.closeAll('loading');
                throw err;
            }
        });
    ...
    */
    function publish() {
    	_index=layer.open({
        	type: 2,
            content: '/modules/zhbg/hygl/meetingNotice/seatPrint/meetingAttentionItem.html?noticeId=' + meetingNoticeId+"&fankuiType=" + fankuiType + "&flg=1",
            area: ['800px', '330px'],
            title: ['填写注意事项', 'font-size:16px;font-weight:bold;'],
            success: function(layero, index){
                var body = layer.getChildFrame('body',index);//建立父子联系
                var iframeWin = window[layero.find('iframe')[0]['name']];
                iframeWin.parentFn = putBackData1;
    ...
    **/
    ...
    function hiddenPersonnel(sign){
        ...
        if (sign==1) { // 存储所有用户选择的座位
            ...
            console.log(userSelecte)
            userSelecte.each(function (index,dom) {
                var usOwnerid = $(dom).attr('data-ownerid');
                var usOwner = $(dom).attr('data-owner');
                var usOwnerDeptid =  $(dom).attr('data-owner-deptid');
                var usOwnerDeptname =  $(dom).attr('data-owner-deptname');

                $('.ms-selected').each(function (i,d) {
                    var sdOwner = $(d).find('span').eq(0).text()
                    if (usOwner==sdOwner){
                        $(d).attr('data-ownerid',usOwnerid);
                        $(d).find('span').eq(1).text(usOwner);
                        $(d).attr('data-owner-deptid',usOwnerDeptid);
                        $(d).attr('data-owner-deptname',usOwnerDeptname);
                        $(d).addClass('left-arranged');
                    }
                })
            })
            ...
            for(var i=0; i<AllSeats.length; i++){  // 遍历所有的座位
                AllSeats[i].style.display="none";   // 将所有座位全部隐藏
                 for(var j=0; j<userSelecte.length; j++){  // 遍历用户选择的座位
                    var seatId = AllSeats[i].getAttribute("data-ownerid");
                    var us = userSelecte[j].getAttribute('data-ownerid') ;
                     if(seatId == us){
