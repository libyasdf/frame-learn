...
public class BmglServiceImpl implements BmglService {
     ...
     */
	@Override
	public JSONObject dynamicAdd(String jsonStr, String tName, String fids) {
		...
		if(key.indexOf("archive_flag")==-1 && count > 0){
			if(fid.length == 1 && !"".equals(fids)){
			    //含有父id，并且只有一个父id时，为关联新增，组卷标识设置为已组卷
	...
	//				name = String.valueOf(tNames.get("S_COL_NAME"));
		...
		if (StringUtils.isNotEmpty(fids)) {
			...
			for(Map<String,Object> map:findSublevelDedicated) {
				userFiltration= "and cre_user_id='"+map.get("CRE_USER_ID").toString()+"' ";
				i++;
	...
	public List<Map<String, Object>> getChildDataByFartherId(String tName, String fid,String basefolderUnit,String creUserId) {
		...
		if(list.size()>0){
			...
			String sql = "select * from "+name+" where "+conCol+"= ? and cre_chushi_id = ? and cre_user_id= ? and archive_flag='"+DAGLCommonConstants.ARCHIVE_FLAG[0]+"' order by "+orderCol+" asc" ;
			list = jdbcTemplate.queryForList(sql,fid,basefolderUnit,creUserId);
		}
		return list;
	}
	...
	public int openVolume(String tName, String ids, String archiveFlag, String conCol,String conVal) {
		...
		String sql = "UPDATE "+tableName+" SET "+col+" = ? ,ARCHIVE_FLAG = ? WHERE RECID IN ("+inString+")";
		log.info(sql);
		int num = jdbcTemplate.update(sql,para.toArray());
		return num;
	}
	 ...
	 */
	@Override
	public int isRepetition(String column,String relevancyId, String tName, String chushiId, String recId) {
	...
	public List<Map<String,Object>> findSublevelDedicated(String fids,String tName,String column) {
		...
		String sql = "select "+column+",cre_user_id  from "+tName+" where recid in "+buffer.toString()+"";
	 ...
	 **/
	@Override
	public int dynamicPlAdd(String jsonStr,String tName,String addCount, String categoryCode, String fids){
		...
		for(int i=0;i<addcount;i++){
			...
			String newFolderNo = "";//新的案卷级档号
			String newYearFolderNo = "";//新的案卷号
			String newJsonStr = "";//新的字符串
			...
			for(int j=0;j<list.size();j++){
				...
				if("year_folder_no".equals(list.get(j).getRuleField())){
					...
					newFolderNo += newYearFolderNo+Connector;
			...
			countofadd += (int)dynamicAdd(newJsonStr, tName,fids).get("flag");
		}
		return countofadd;
	}
	 ...
	 */
	@Override
	public PageImpl getFirstLevel(PageImpl pageImpl,Pageable pageable,String tableName,String liJuanDanWei,String luRuRen,String mainTitle,String dangHao){
		...
		if(tableName.contains("_dak")){
			...
			selData.append(" and t.CRE_USER_ID='"+UserUtil.getCruUserId()+"' and t.ARCHIVE_ENTITY_STATUS <> '"+DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[2]+"' ");
		}else{
		...
		if(dhgz.size() >= levelNum){
			...
			if(Integer.parseInt(nowData.get(0).get(dhgz.get(dhgz.size()-levelNum).get("RULE_FIELD").toString().toUpperCase()).toString()) < Integer.parseInt(newXuHao)){
				...
				if(tName.contains("_dak")){
					...
					String selNewDataCount = "select count(recid) from "+tName+" t where (t."+dhgz.get(dhgz.size()-levelNum).get("RULE_FIELD")+"= ? ";
					int newDataCount = jdbcTemplate.queryForObject(selNewDataCount, Integer.class,newXuHao);
					if(newDataCount == 0){
						//不存在，提示用户应该输入一个存在的序号
			...
			if(levelNum == 1){
				...
				if("asc".equals(order)){
					...
					for(int xh=start;xh<=end;xh++){
						...
						if(tableName.contains("_dak")){
							//管理员归档库操作
							upNextData = "update "+tableName+" t set t."+dangHaoCol+"='"+newDangHaoPartStr.toString()+xuHao+"',t."+dhgz.get(dhgz.size()-1).get("RULE_FIELD").toString()+"='"+xuHao+"' where to_number(t."+dhgz.get(dhgz.size()-1).get("RULE_FIELD").toString()+") ="+xh;
						}else{
							//录入人单位预立库操作
							upNextData = "update "+tableName+" t set t."+dangHaoCol+"='"+newDangHaoPartStr.toString()+xuHao+"',t."+dhgz.get(dhgz.size()-1).get("RULE_FIELD").toString()+"='"+xuHao+"' where to_number(t."+dhgz.get(dhgz.size()-1).get("RULE_FIELD").toString()+") ="+xh
					...
					for(int xh=start;xh>=end;xh--){
						...
						if(tableName.contains("_dak")){
							//管理员归档库操作
							upNextData = "update "+tableName+" t set t.archive_no='"+newDangHaoPartStr.toString()+xuHao+"',t."+dhgz.get(dhgz.size()-1).get("RULE_FIELD").toString()+"='"+xuHao+"' where to_number(t."+dhgz.get(dhgz.size()-1).get("RULE_FIELD").toString()+") ="+xh;
						}else{
							//录入人单位预立库操作
							upNextData = "update "+tableName+" t set t.archive_no='"+newDangHaoPartStr.toString()+xuHao+"',t."+dhgz.get(dhgz.size()-1).get("RULE_FIELD").toString()+"='"+xuHao+"' where to_number(t."+dhgz.get(dhgz.size()-1).get("RULE_FIELD").toString()+") ="+xh
									+" and t.CRE_USER_ID='"+nowData.get(0).get("CRE_USER_ID").toString()+"' and t.CRE_CHUSHI_ID='"+nowData.get(0).get("CRE_CHUSHI_ID").toString()+"' ";
				...
				updateSales.setString(1,newDangHaoStr.toString());
				...
				//得到新的档号
				newDangHaoStr.append(newXuHao);
				String updateNowData = "update "+tableName+" t set t."+pCol+"='"+newDangHaoStr.toString()+"',t."+dhgz.get(dhgz.size()-2).get("RULE_FIELD").toString()+" = ? where t.recid='"+nowData.get(0).get("RECID").toString()+"'";
				...
				if("asc".equals(order)){
					...
					for(int xh=start;xh<=end;xh++){
						...
						if(tableName.contains("_dak")){
							//管理员归档库操作
							upNextData = "update "+tableName+" t set t."+pCol+"='"+newDangHaoPartStr.toString()+xuHao+"',t."+dhgz.get(dhgz.size()-2).get("RULE_FIELD").toString()+"='"+xuHao+"' where to_number(t."+dhgz.get(dhgz.size()-2).get("RULE_FIELD").toString()+") ="+xh;
						}else{
							//录入人单位预立库操作
							upNextData = "update "+tableName+" t set t."+pCol+"='"+newDangHaoPartStr.toString()+xuHao+"',t."+dhgz.get(dhgz.size()-2).get("RULE_FIELD").toString()+"='"+xuHao+"' where to_number(t."+dhgz.get(dhgz.size()-2).get("RULE_FIELD").toString()+") ="+xh
									+" and t.CRE_USER_ID='"+nowData.get(0).get("CRE_USER_ID").toString()+"' and t.CRE_CHUSHI_ID='"+nowData.get(0).get("CRE_CHUSHI_ID").toString()+"' ";
				...
				PreparedStatement updateSales =  con.prepareStatement(updateNowData);
				//updateSales.setString(1,newDangHaoStr.toString());
				updateSales.setString(1,newXuHao);
				updateSales.executeUpdate();
				...
				//得到新的档号
				newDangHaoStr.append(newXuHao);
				String updateNowData = "update "+tableName+" t set t."+pCol+"='"+newDangHaoStr.toString()+"',t."+dhgz.get(dhgz.size()-3).get("RULE_FIELD").toString()+" = ? where t.recid='"+nowData.get(0).get("RECID").toString()+"'";
				//2.更新当前表子表：档号和与父表关联字段，默认管理员归档库操作
				String selNowSubData = "select s.* from "+sTable+" s where s."+sCol+"='"+nowData.get(0).get(pCol).toString()+"'";
				if(!tableName.contains("_dak")){
					//录入人单位预立库操作
				...
				if("asc".equals(order)){
					...
					for(int xh=start;xh<=end;xh++){
						...
						if(tableName.contains("_dak")){
							//管理员归档库操作
							upNextData = "update "+tableName+" t set t."+pCol+"='"+newDangHaoPartStr.toString()+xuHao+"',t."+dhgz.get(dhgz.size()-3).get("RULE_FIELD").toString()+"='"+xuHao+"' where to_number(t."+dhgz.get(dhgz.size()-3).get("RULE_FIELD").toString()+") ="+xh;
						}else{
							//录入人单位预立库操作
							upNextData = "update "+tableName+" t set t."+pCol+"='"+newDangHaoPartStr.toString()+xuHao+"',t."+dhgz.get(dhgz.size()-3).get("RULE_FIELD").toString()+"='"+xuHao+"' where to_number(t."+dhgz.get(dhgz.size()-3).get("RULE_FIELD").toString()+") ="+xh
									+" and t.CRE_USER_ID='"+nowData.get(0).get("CRE_USER_ID").toString()+"' and t.CRE_CHUSHI_ID='"+nowData.get(0).get("CRE_CHUSHI_ID").toString()+"' ";
				...
				updateSaless.setString(1,newXuHao);
				updateSaless.executeUpdate();
	 ...
	 **/
	@Override
	public int queryCountAdd(String danghao,String columnName, String tName, String chushiid,String lastColumnName){

		List<Object> para = new ArrayList<>();
		String sql = "select max("+lastColumnName+") from "+tName+" where "+columnName+" like ?  ";
		para.add(danghao+"%");
