...
function readOnly() {
    ...
    layer.open({
        ...
        content: '/modules/dagl/bmgl/readonly.html?type=find',
        area: ['1100px', '660px'],
        offset: '30px',
        resize: false,
        maxmin: true,
	...
	$("#tbody1 tr").each(function(index,value){
		...
		if(m==index){
			if($this.find("input").eq(0).val()==$(e).find("input").val()){
				...
				nodes = $.fn.zTree.getZTreeObj("treeDemo").getSelectedNodes();
                $.get("/dagl/bmgl/adddd",{id:$(e).find("input").val(),tName:tabCurrent,ranking:ranking,all:all},function(data){
                    $("#tbody1").append("<tr>" +
...
function add(e){
    ...
    var waitIndex = layer.msg('加载中，请稍等...', {
        icon: 16,
        shade: [0.1, '#fff'],
        time: false
    });
    ...
    for(var n = 0;n<checkBoxData[tabCurrent].length;n++){
        for(var m = 0;m<tableListData.length;m++){
            var id = tableListData[m].RECID;
            if(tableListData[m].RECID == checkBoxData[tabCurrent][n] ){

                if(tableListData[m].BORROW_STATUS=="1"|| tableListData[m].BORROW_STATUS=="2"){
                    //layer.msg("该档案已借出，禁止添加！",{icon:0});
                    borrowStatusCount++;
                    continue;
                }
