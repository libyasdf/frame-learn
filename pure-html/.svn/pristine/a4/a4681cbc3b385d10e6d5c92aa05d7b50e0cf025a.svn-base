...
public class AttendenceQueryServiceImp implements AttendenceQueryService {
	...
	public PageImpl getPageList(Pageable pageable, PageImpl pageImpl, String startDate, String endDate, String cardNumber,String userId) {
		if(StringUtils.isBlank(UserUtil.getCruChushiId()) && StringUtils.isBlank(userId)){//是局长，列表什么都不显示
	...
	private void setCardNomIntoSql(StringBuilder querySql, JSONObject json, StringBuilder cardSb, int index) {
		...
		if(list.size()>0){
			...
			while(list.size()>0){
				...
				querySql.append(CommonUtils.commomSplit(cardSb.substring(0, cardSb.length()-1)) + " )");
			}
			querySql.append(" ) ");
		}
	}
