        ...
        var arg = {
            ...
            , doneFn: function (e, data, fileListId, pId,download,read,defaultName) {  		//文件上传完成后的回调（不管成功失败）
                ...
                if (data.result.flag === "1") {
                    ...
                    layer.alert(msg, {icon: 0,time:1000});
                    var $aTab = $("a:contains('"+fileName+"')");
            ...
            , progressFn: function (e, data, pId,defaultName) {  //文件上传进度条回调
                ...
                $.each($aTab,function (index,item) {
                    if(item.id==""){
                        //加载等待
                        waitWind = layer.load(1,{shade: [0.5, '#393D49'],content: '请稍候...',success: function(layero){
            ...
            ,changeFn:function (e,data,fileListId,maxfileSize,maxFileCount,fileTypes) {
                ...
                for(var n =0;n < data.files.length;n++){
                    if(data.files[n].size > maxfileSize){
                        layer.alert("文件不能超过"+maxfileSize/1024/1024+"M！", {icon: 0,time:1000});
                        flag = false;
                    }
                }
                ...
                if(maxFileCount !=0){
                    if($aTab.length >= maxFileCount){
                        layer.msg("文件最多上传"+maxFileCount+"个文件！",{icon:0,time:1000});
                        flag = false;
                    }
                ...
                if(fileTypes && fileTypes != ""){
                    ...
                    if(fileTypes.indexOf(",")<0){//如果格式限制当中只有一个格式（eg:docx），那么，就意味着只能上传该格式的文档
                        if(fileTypes != suffix){
                            ...
                            layer.msg("只能上传"+fileTypes+"格式的文件！",{icon:0,time:1000});
                            flag = false;
                        }
                    }else{
                        //如果想允许多个，用英文逗号分隔（eg：
                ...
                $subTableRows.each(function (i, index) {
                    var fjName = $(index).find('a').text().trim();

                    /*if(undefined == fjid){
                        studying.msg += "【"+$(index).find('[name="chushiName"]').val()+"】";
                    }*/
                    if(data.files[0].name == fjName){

                        layer.msg("不能上传重复的附件，"+fjName+"已存在！",{icon:0,time:1000});
                        flag = false;
                        return false;
                    }
                    /*var studyingSubs = {
