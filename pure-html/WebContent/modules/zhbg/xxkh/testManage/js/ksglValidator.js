        ...
        fields: {
			...
			timeRange: {
				...
				validators: {
					...
					callback: {
						message: '开始时间必须大于等于当天时间!',
						callback: function (value, validator, $field) {
							var start = value.substr(0, (value.length + 1) / 2 - 1).trim();
							var startDate = new Date(start.replace("-", "/").replace("-", "/"));
							var td = new Date().getFullYear()+"/"+(new Date().getMonth()+1)+"/"+new Date().getDate();
							console.log("考试开始时间："+startDate+"，今天时间："+new Date(td));
							if (startDate >= new Date(td)) {
								return true;
							}
							return false;
						}
