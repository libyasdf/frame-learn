...
$(document).ready(function(){
	...
	if(!isAuth()){
		//没有权限，提示用户，并隐藏提交按钮
		layer.alert("对不起，您不在本次问卷调查范围之内！",{icon:0});
		$("#surveyForm").attr("action","");
		$("#submitSurvey").hide();
		$('.Loading').remove();
	}else{
		//判断该用户是否已经提交
		if(isComplete()){//已经提交过
			layer.alert("对不起，您已提交本问卷，请勿重复提交！",{icon:0});
			$("#surveyForm").attr("action","");
			$("#submitSurvey").hide();
		}
		$('.Loading').remove();
	...
	$(".submitSurvey").click(function(){
        ...
        var workItemId1 = theRequest.workItemId1;
        if(workItemId1!=undefined){
            var datas = {
                "id" : workItemId1
            };
            httpRequest(
...
function isAuth(){
	var flag = false;
	$.ajax({
		url: "/wenjuan/surveyDirectory/getIsAuth",
		data: {
...
function isComplete(){
	var flag = false;
	$.ajax({
		url: "/wenjuan/surveyDirectory/getIsComplete",
		data: {
			surveyId: $("#surveyId").val()
		},
		async: false,
...
<form id="surveyForm" action="${ctx }/wenjuan/response/save" method="post" >
