var fileTypeId = "b69e9e46320a4a19988e13fcdd20233c";
var workFlowId = "b69e9e46320a4a19988e13fcdd20233c";
                    ...
                    if($("#opertation").val()=="EDIT"){
                        ...
                        DisplayData.playWorkFlow("/flowService/getFlowData",datas,$("#opertation").val(),callback,
                            function returnData(data){
                                ...
                                if(wfleveid =="adf7e9c1868c4a0db8ee2d997f778215" ){
                                    $("#wenjuanBtn").css("display","none");
                                    $("#querySurveyBtn").show();
                                }
                            });
    ...
    if(flag){
        if(flag == "1"){
            ...
            MyLayer.layerOpenUrl({
                url: "/modules/info/authority/authorityInfo.html?contentId=" + id+"&typeCode=04",
                title: "选择通知范围"
            })
        }else{
...
function  commitFlowDept() {
    ...
    if(status =="1"){
        if($("#wenjuanTitle").val() ==""){
            $("#wenjuanTitle").val(getCurrentDate("yyyy年MM月dd日")+getcookie("usernm")+"局级调查问卷审批");
        }
        var bootstrapValidator = $("#form").data('bootstrapValidator');
        bootstrapValidator.validate();
        if(bootstrapValidator.isValid()){
