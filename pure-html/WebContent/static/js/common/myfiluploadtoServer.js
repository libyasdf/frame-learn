var MyFileUpload = function () {
	    		...
	    		if(fileListId == file.fileListId){
		    		$('<div/>').addClass("row m-b-5").append(
		                $('<div/>').addClass("col-md-12").append(
		                	$('<a id=' + file.id + ' href="/system/component/affix/downloadInServer?affixId=' + file.id + '"/>').html(file.name + "&nbsp;&nbsp;&nbsp;")
		                ).append(
    ...
    var echoFileBefore = function (fileListId, data) {
    	...
    	$.each(data.files, function (index, file) {
            $('<div/>').addClass("row m-b-5").append(
                $('<div/>').addClass("col-md-12").append(
                	$('<a id="" href="javascript:void(0);"/>').html(file.name)
                ).append("&nbsp;&nbsp;&nbsp;<span>0%</span>")
            ).appendTo('#' + fileListId);
        });
    };
    ...
    var init = function (json) {
        ...
        var arg = {
            ...
            , uploadUrl: '/system/component/affix/fileuploadToServer'  		//文件上传地址
                ...
                if (data.result.flag === "1") {
                    ...
                    $.each(data.result.data.files, function(index,file){
                        $.each($aTab,function (index,item) {
                            if(item.id == ""){
                                item.id=file.id;
                                item.href="/system/component/affix/downloadInServer?affixId="+file.id ;
                            }
                        })
                    });
                   /* $.each(data.result.data.files, function(index,file){
                    	$aTab.attr("id",file.id);
                    	$aTab.attr("href","/system/component/affix/downloadInServer?affixId="+file.id );
                    });*/
    ...
    var fileDelete = function (json, e) {
        ...
        layer.confirm("确认删除附件【"+fileName+"】吗？", function(index){
	        $.ajax({
	            url: _delArg.url
	            , type: "GET"
    ...
    var saveDocIdToAffix = function(json){
    	...
    	//var aTabs = $("#"+_saveParam.fileListId).find("a");
    	var affixIds = [];
    	if(aTabs.length > 0){
