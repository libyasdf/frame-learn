...
public class DirectorStatsServiceImpl implements DirectorStatsService {
	...
	@Override
	public List<DeptCount> getList(String timeRange, String deptid) throws Exception {
		...
		//获取该用户下的所有处级子部门信息
		StringBuilder deptidsb = new StringBuilder();//存放局部门下所有处级部门的部门id
		StringBuilder userids = new StringBuilder();//存放局部门下处级及处级子部门下的所有人员id
		...
		getChildDeptInfo(deptidsb, map,userids);
		...
		List<KqglAttendenceHandle> kqData = kqglAttendenceHandleService.findByCondition(startDate,endDate,CommonUtils.commomSplit(cardNum));
		changKqData(kqData,kqMap,cardNum);
		//查询某段时间某些人做为同行人的出差记录
		String tempUserids = userids.substring(0, userids.length()-1);
		...
		//把考勤数据转成map格式
		//查询该用户子部门下某段时间内请假数据..
		List<Map<String,Object>> overTimeData = getOverTimeData(startDate,endDate,CommonUtils.commomSplit(tempUserids));
		changeOverTimeData(overTimeData,userOverTimeInfo);
		//统计该用户子部门下某段时间内总的病假次数、事假总次数、出差次数..
		List<Map<String,Object>> totalInfo = getTotalInfo(startDate,endDate,CommonUtils.commomSplit(tempUserids));
		...
		//把转成页面所需的格式，并设置平均
		if(StringUtils.isNotBlank(tempDeptid)){
			String[] deptids = tempDeptid.split(",");
			for (String deptid1 : deptids) {
				DeptCount deptCount = map.get(deptid1);
	...
	private String getCardNums(String uerid,Map<String,DeptCount> map) {
		...
		Map<String, SysFlowUserVo> usersVo = UserUtil.getUserVo(uerid);
		Set<Entry<String, SysFlowUserVo>> entrySet = usersVo.entrySet();
		for (Entry<String, SysFlowUserVo> entry : entrySet) {
			...
			if(StringUtils.isNotBlank(userVo.getCardNumber())){
				cardNum.append(userVo.getCardNumber()+",");
				...
				for (Entry<String, DeptCount> entry2 : DeptCountEntrys) {
					...
					if(index!=-1){
						deptCount.getCardNum().append(userVo.getCardNumber()+",");
						break;
					}
				}
			}
	...
	private List<Map<String, Object>> getOverTimeData(String startDate, String endDate, String userids) {
		...
		sb.append(" from KQGL_LEAVE_INFO t where t.visible='1' and t.subflag='5' and t.cre_user_id in(" + userids + ")  and t.leave_type in ('1','2')");
	...
	//获取该用户下的所有处级子部门信息
	private void getChildDeptInfo(StringBuilder deptidsb,Map<String,DeptCount> map,StringBuilder userids) {
		String juId = UserUtil.getCruJuId();
		Map<String,List<JSONObject>> deptUserInfo = new HashMap<String,List<JSONObject>>();//用于存放部门用户信息，key为pid,value为部门信息或用户信息
		JSONArray jsonArray = service.getDeptAndUserTreeByDeptId(juId);
		setDeptUserInfo(deptidsb,jsonArray,deptUserInfo);
		List<JSONObject> chuDept = deptUserInfo.get(juId);
		for (JSONObject jsonObject : chuDept) {
			if("dept".equals(jsonObject.getString("type"))){
				...
				setPeopleNumAndChidDeptIds(peptCount,deptid,deptUserInfo,userids);
	...
	private void setPeopleNumAndChidDeptIds(DeptCount peptCount, String deptid,
			Map<String, List<JSONObject>> deptUserInfo,StringBuilder userids) {
		...
		if(list!=null && list.size()>0){
			for (JSONObject jsonObject : list) {
				if("dept".equals(jsonObject.getString("type"))){
					...
					setPeopleNumAndChidDeptIds(peptCount,jsonObject.getString("uuid"),deptUserInfo,userids);
				}else{
					//人员
