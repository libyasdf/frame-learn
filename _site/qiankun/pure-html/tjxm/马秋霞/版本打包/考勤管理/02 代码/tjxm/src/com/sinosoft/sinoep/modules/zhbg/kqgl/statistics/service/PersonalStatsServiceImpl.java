...
public class PersonalStatsServiceImpl implements PersonalStatsService {
	...
	public List<PersonalStats> getList(String timeRange) throws Exception {
		...
		StringBuilder sb = getTongJiInfo(startDate, endDate, cruUserId,cardNo);
		Map<String,Object> personMap = jdbcTemplate.queryForMap(sb.toString());
		//把查出来的数据放到实体类里
	 ...
	 * @return
	 */
	private StringBuilder getTongJiInfo(String startDate, String endDate, String cruUserId,String cardNo) {
		...
		if(StringUtils.isNotBlank(cardNo)){
			...
			if(StringUtils.isNotBlank(startDate)){
				sb.append(" and  t.import_date >= '" + startDate + "'");
			}
			if(StringUtils.isNotBlank(endDate)){
				sb.append("  and  t.import_date <= '" + endDate + "'");
			}
			sb.append(" ) comeGoCnt,");
		}else{
		...
		//求出差次数
		sb.append("  (select count(id)   from KQGL_BUSINESS_TRIP_INFO where (cre_user_id='" + cruUserId + "'  or instr(trip_colleague_ids, " + cruUserId +")>0)  and  subflag='" + ConfigConsts.APPROVAL_FLAG + "'  ");
		if (StringUtils.isNotBlank(startDate) && StringUtils.isNotBlank(endDate)) {
