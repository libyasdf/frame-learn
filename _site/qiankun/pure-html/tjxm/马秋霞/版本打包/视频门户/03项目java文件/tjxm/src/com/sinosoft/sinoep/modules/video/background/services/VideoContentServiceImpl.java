...
public class VideoContentServiceImpl implements VideoContentService {
    ...
    public PageImpl getContentPageList(Pageable pageable, PageImpl pageImpl, String columnId, String title, String creTime) {
        ...
        sql.append(" from VideoContent t where t.visible='1'");
        if(StringUtils.isNotBlank(columnId)){
            sql.append(" and t.columnId  = '").append(columnId).append("'");
        }else{
            sql.append(" and t.columnId ='").append(VideoConstants.VIDEOFBONEID).append("'");
        }
        ...
        try{
            Page<VideoContent> page = dao.query(sql.toString(),pageable,para.toArray());
            List<VideoContent> list = page.getContent();
            pageImpl.setFlag("1");
    ...
    public JSONObject queryQx(String columnId, String contentId) {
        ...
        if(StringUtils.isNotBlank(columnId)){
            ...
            if(contentEntity!=null&&VideoConstants.ISSP[0].equals(columnEntity.getIsSp()) && (!"2".equals(contentEntity.getSubflag()))){//如果这个栏目不需要审批则显示发布按钮
                json.put("fbButton","1");//发布
            }else if(contentEntity==null&&VideoConstants.ISSP[0].equals(columnEntity.getIsSp())){
            	json.put("fbButton","1");//发布
            }else{
    ...
    public List<VideoContent> getZdList(String columnId) {
        ...
        list = dao.findByVisibleAndColumnIdAndIsZdIsNotNullOrderByIsZdAsc(CommonConstants.VISIBLE[1],columnId);
        return list;
    }
	...
	public Map<String, Object> getHomePageContent(PageImpl page,String columnId,String flag,String columnName) throws Exception {
		...
		/*	sb.append(" select t.id,t.TITLE,t1.save_path imgPath from ( ");
			...
			sb.append(" select t.id,t.TITLE,t.IMAGE_ID,t.IS_ZD , t.FB_TIME  from VIDEO_CONTENT t where t.visible='1' and t.SUBFLAG='2' and t.column_id='" + VideoConstants.recommendId + "' and (IS_FB_CONTENT='0' or IS_FB_CONTENT='1'   ");
			if(contentIds.length()>0){
			     sb.append("  and t.id in (" + contentIdStr + ") ");
			}
			sb.append(" )");
			...
			sb.append(" left outer join (select content_id,count(id) playTimes from history_record group by content_id) t3 on t.id=t3.content_id  order by t.IS_ZD,t.FB_TIME ");
			...
			//查询每种类别的视频
			StringBuilder classSql = new StringBuilder(" select t.id,t.column_name columnName from video_COLUMN t where  t.visible='1'  and t.IS_SHOW='1' ");
			classSql.append(" and trim(t.super_id) in( select t.id from video_COLUMN t where t.visible = '1' and t.is_first='1'  )");
			...
			for(int i=0;i<columnList.size();i++){
				...
				contentSql.append(" select t.id,t.TITLE,t.IMAGE_ID,t.IS_ZD,t.FB_TIME from VIDEO_CONTENT t where t.visible = '1' and t.column_id='" + columnList.get(i).getId() + "' and t.subflag = '2'  and (IS_FB_CONTENT = '0' or IS_FB_CONTENT = '1'   ");
				if(contentIds.length()>0){
				   contentSql.append(" and t.id in (" + contentIdStr + ") ");
				}
				contentSql.append(" )");
				contentSql.append(" and (( t.SHOW_START_TIME <= '" + nowTime + "' and t.SHOW_END_TIME >= '" + nowTime + "') or t.SHOW_START_TIME is null)  order by t.IS_ZD, t.FB_TIME  ");
				...
				contentSql.append(" left outer join (select content_id,count(id) playTimes from history_record group by content_id) t3 on t.id=t3.content_id order by t.IS_ZD,t.FB_TIME ");
	...
	private void querySql(StringBuilder sb, String columnId,String nowTime,StringBuilder contentIds,String contentIdStr,String columnName,String flag) {
		...
		sb.append("select t.id,t.title,t.IMAGE_ID,t.IS_ZD,t.FB_TIME from VIDEO_CONTENT t where t.visible = '1'   and t.subflag = '" + VideoConstants.SUBFLAG[2] + "' ");
		if("1".equals(flag)){
			//点击搜索获取的数据
		...
		//如果有发布范围，查询发布范围内的数据
		sb.append(" and (t.IS_FB_CONTENT = '0' or (t.IS_FB_CONTENT = '1' ");
		if(contentIds.length()>0){
			sb.append(" and t.id in ("+contentIdStr+") ");
		}
		sb.append(" )) ");
		...
		sb.append(" ) t2 on t.id=t2.info_id  ");
		sb.append(" left outer join (select content_id,count(id) playTimes from history_record group by content_id) t3 on t.id=t3.content_id order by t.IS_ZD,t.FB_TIME desc");
	}
	...
	public List<VideoInfo> getVideoByContentId(String contentId,String fileName) {
		...
		sql.append("  left outer join (select t.video_id,count(1) readcnt from history_record t where state='1' group by video_id )t2  on t.id=t2.video_id ");
		sql.append("  left outer join (select t.video_id,count(1) notfinishcnt from history_record t where state='0' group by video_id )t3  on t.id=t3.video_id ");
