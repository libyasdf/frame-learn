...
$(function () {
    ...
    typeTree.init({isQ2:true,isAdmin:2});
    try{
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.length; ++i) {
            	...
            	if(data[i].ARCHIVE_ENTITY_STATUS=="已移交"){
            		continue;
            	}
                // 渲染表格内容
                var $tr = $('<tr></tr>')
                ...
                for (var j = 0; j < tHeadData.length; ++j) {
                    ...
                    if (tHeadData[j].COLUMN_LIST_ISSHOW === 'T') {
                       ...
                       if("fonds_no" == keyStrMin && "" != valueStr){
                        //这里临时写死全宗号回显列表，后期添加可添加全宗号的时候已改动态获取
                            if("03"==valueStr){
                                valueStr = "全宗1";
                                valueStr2 = valueStr;
                            }else if("04"==valueStr){
...
function showNoData(){
    ...
    {
        // 渲染表格内容
        var $tr = $('<p class="noData">没有找到匹配的记录</p>')
        $('.tab-panel #table #tbody').append($tr);
        $("#total").html("0");
    }
}
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            for( var i=0;i<result.data.rows.length;i++){
                if (null != result.data.rows[i].FHR_ID) {
                    alreadysealUpCount++;
                }
            }
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if ('1' == data.flag) {
                                ...
                                layer.msg("无法删除：选中数据的档案实体状态为已移交或已移交待审核。", {
                                    icon: 5,time:2000
                                });
                            }
                        },
...
function adjustFirstLevel(event){
    ...
    var categoryCode= nodes[0].categoryCode;
    if(checkBoxData[tabCurrent].length != 1){
        layer.msg("请选择一条数据让我们来确认你可被调整的档案！",{icon: 0,time:2000});
        return;
    }else{
        MyLayer.layerOpenUrl({
            url:'/modules/dagl/bmgl/firstLevelDataList.html?tableName='+tabCurrent+'&all='+all+'&selectIds='+selectIds,
            title:"门类第一层档案数据",
            width:"90%",
            height:"90%"
        })
