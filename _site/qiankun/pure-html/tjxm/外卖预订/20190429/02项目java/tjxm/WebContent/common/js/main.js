...
function initYearOptions() {
    ...
    for (var i = 0; i < 10; ++i) {
        $('#year').append($('<option>' + (year+1 - i) + '年</option>'));
    }
}
        ...
        for (var j = 0; j < data[i].length; j++) {
            ...
            if (!data[i][j].validate) {
                ...
                if (data[i][j].hasData && data[i][j].isFinish == '0') {
                    ...
                    trHTML += "<td onclick='check(this)' style='cursor: pointer'> <span class='none hasPlan'>"+ data[i][j].day + type +"<span class='redOfPlan'></span></span> <input type='hidden' value=" + data[i][j].fullDay + " /></td>"
                } else if (data[i][j].hasData && data[i][j].isFinish == '1') {
                    //工作日志
                    trHTML += "<td onclick='check(this)' style='cursor: pointer'><span class='none hasLog'>"+ data[i][j].day + type +"</span> <input type='hidden' value=" + data[i][j].fullDay + " /></td>"
                }else if (data[i][j].hasData && data[i][j].isFinish == '2') {
                    //工作计划和日志
                    trHTML += "<td onclick='check(this)' style='cursor: pointer'><span class='none hasPlanLog'>"+ data[i][j].day + type +"<span class='redOfPlan'></span></span> <input type='hidden' value=" + data[i][j].fullDay + " /></td>"
                } else {//什么都没有
                    trHTML += "<td onclick='check(this)' class='' style='cursor: pointer'><span class='none'>"+ data[i][j].day + type +"</span><input type='hidden' value=" + data[i][j].fullDay + " /></td>"
                }
            } else {
                ...
                if (data[i][j].hasData && data[i][j].isFinish == '0') {
                    //工作计划
                    trHTML += "<td onclick='check(this)' style='cursor: pointer'><span class='hasPlan'>"+ type +"<span class='redOfPlan'></span></span> <input type='hidden' value=" + data[i][j].fullDay + " /></td>"
                } else if (data[i][j].hasData && data[i][j].isFinish == '1') {
                    //工作日志
                    trHTML += "<td onclick='check(this)' style='cursor: pointer'><span class='hasLog'>"+ type +"</span> <input type='hidden' value=" + data[i][j].fullDay + " /></td>"
                } else if (data[i][j].hasData && data[i][j].isFinish == '2') {
                    //工作计划和日志
                    trHTML += "<td onclick='check(this)' style='cursor: pointer'><span class='hasPlanLog'>"+ type +"<span class='redOfPlan'></span></span> <input type='hidden' value=" + data[i][j].fullDay + " /></td>"
                }else {
                	//什么都没有
                    trHTML += "<td onclick='check(this)' class='' style='cursor: pointer' >" + type + "<input type='hidden' value=" + data[i][j].fullDay + " /></td>"
                }
...
function InitInfoTab(){
    var tabLi = $('.gr_block_info_tab_ul').children('.infoWorkFlow');
    var clickIndex=0;//记录被点击下标
    tabLi.each(function(index,item){  // 绑定Tab事件
        $(this).mouseenter(function(){  // 鼠标移入时将线的left为li的宽度*索引index的值
            $(this).css('color','#0073aa');
            $('.gr_block_info_tab_ul .bottom_line').css('left',(index)* $(this).outerWidth())
        });
        $(this).mouseleave(function(){  // 鼠标离开时将线移回到当前显示的tab下
            if(clickIndex !=index){
                $(this).css('color','#000000');
            };
            $('.gr_block_info_tab_ul .bottom_line').css('left',(clickIndex)* $(this).outerWidth()) ;
        })
        $(this).click(function(){  // 用户点击时设置线的left为当前点击tab的位置
            clickIndex = index;
            tabLi.each(function(){
            ...
            $(this).css('color','#0073aa');
            $('.gr_block_info_tab_ul .bottom_line').css('left',(index)* $(this).outerWidth())   // 将线的left设置为当前点击tab的值
        })
...
function orderingMeals() {
    var url = 'modules/mypage/wm/index/index.html'
