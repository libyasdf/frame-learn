...
function checkDept(isReset){
	layer.open({
        ...
        area: ['600px', 'auto'],
        title: ["选择部门", 'font-size:16px;font-weight:bold;'],
        offset:['220px'],
        // btn:["确定","关闭"],
        yes:function(index,layero){
        	// 通过iframe 操作
            var iframeId = $("#chooseSerachall").find('iframe').attr('id')
        	var json = document.getElementById(iframeId).contentWindow.getChooseDept();
            //重置session中用户信息
        ...
        btn2:function(){
        	if(isReset){
                var iframeId = $("#chooseSerachall").find('iframe').attr('id')
                var json = document.getElementById(iframeId).contentWindow.getDefaultDept();
                //重置session中用户信息
                setSession(json.deptid,function(json){
                    if(json.result == "success"){
                        window.location.reload();//刷新当前页面
