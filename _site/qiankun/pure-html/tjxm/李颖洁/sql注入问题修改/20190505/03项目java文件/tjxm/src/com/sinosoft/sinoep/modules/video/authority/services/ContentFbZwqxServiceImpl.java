...
public class ContentFbZwqxServiceImpl implements ContentFbZwqxService {
	...
	public VideoModel saveInfoZwqx(VideoModel model) {
		...
		if(authority.getDeptList().isEmpty() &&authority.getGroupList().isEmpty() && authority.getZwqxList().isEmpty() && authority.getDeptZwqxList().isEmpty()){
			//三个集合都是空的，直接保存
			saveAuth(model);
		}else{
			//权限已存在，先删除，然后重新保存
	...
	public VideoModel getAuthority(String contentId) {
		...
		//查询部门权限
		StringBuilder deptHql = new StringBuilder("from VideoFbDept t where t.contentId = ? and t.visible = '"+ContentFbConstants.VISIBLE[1]+"'");
		TypedQuery<VideoFbDept> deptQuery = deptDao.getEntityManager().createQuery(deptHql.toString(), VideoFbDept.class);
		deptQuery.setParameter(1, contentId);
		List<VideoFbDept> deptList = deptQuery.getResultList();
		model.setDeptList(deptList);
		//查询职务权限
		StringBuilder zwqxHql = new StringBuilder("from VideoFbZwqx t where t.contentId = ? and t.visible = '"+ContentFbConstants.VISIBLE[1]+"'");
		TypedQuery<VideoFbZwqx> zwqxQuery = zwqxDao.getEntityManager().createQuery(zwqxHql.toString(), VideoFbZwqx.class);
		zwqxQuery.setParameter(1, contentId);
		List<VideoFbZwqx> zwqxList = zwqxQuery.getResultList();
		model.setZwqxList(zwqxList);
		//查询部门+职务权限
		StringBuilder deptZwqxHql = new StringBuilder("from VideoFbDeptZwqxList t where t.contentId = ? and t.visible = '"+ContentFbConstants.VISIBLE[1]+"'");
		TypedQuery<VideoFbDeptZwqxList> VideoFbDeptZwqxQuery = deptZwqxListDao.getEntityManager().createQuery(deptZwqxHql.toString(), VideoFbDeptZwqxList.class);
		VideoFbDeptZwqxQuery.setParameter(1, contentId);
		List<VideoFbDeptZwqxList> deptZwqxList = VideoFbDeptZwqxQuery.getResultList();
		model.setDeptZwqxList(deptZwqxList);
		//查询部门+职务list权限
		StringBuilder deptZwqxListHql = new StringBuilder("from VideoFbDeptZwqx t where t.contentId = ? and t.visible = '"+ContentFbConstants.VISIBLE[1]+"'");
		TypedQuery<VideoFbDeptZwqx> deptZwqxListQuery = deptZwqxDao.getEntityManager().createQuery(deptZwqxListHql.toString(), VideoFbDeptZwqx.class);
		deptZwqxListQuery.setParameter(1, contentId);
		List<VideoFbDeptZwqx> deptZwqxListList = deptZwqxListQuery.getResultList();
		model.setDeptZwqxListList(deptZwqxListList);
		//查询群组权限list
		StringBuilder groupListHql = new StringBuilder("from VideoFbGroup t where t.contentId = ? and t.visible = '"+ContentFbConstants.VISIBLE[1]+"'");
		TypedQuery<VideoFbGroup> VideoFbGroupQuery =  groupDao.getEntityManager().createQuery(groupListHql.toString(), VideoFbGroup.class);
		VideoFbGroupQuery.setParameter(1, contentId);
		List<VideoFbGroup> groupList =VideoFbGroupQuery.getResultList();
		model.setGroupList(groupList);
		return model;
	}
	...
	public ProgramaModel saveColumnFbfw(ProgramaModel model) {
		...
		if(columnAuthority.getDeptList().isEmpty() &&columnAuthority.getGroupList().isEmpty() && columnAuthority.getZwqxList().isEmpty() && columnAuthority.getDeptZwqxList().isEmpty()){
			//三个集合都是空的，直接保存
	...
	public ProgramaModel getColumnAuthority(String columnId) {
		...
		//查询部门权限
		StringBuilder deptHql = new StringBuilder("from ProgramaFbDept t where t.columnId = ? ");
		TypedQuery<ProgramaFbDept> deptQuery = programaDeptDao.getEntityManager().createQuery(deptHql.toString(), ProgramaFbDept.class);
		deptQuery.setParameter(1, columnId);
		List<ProgramaFbDept> deptList = deptQuery.getResultList();
		model.setDeptList(deptList);
		StringBuilder zwqxHql = new StringBuilder("from ProgramaFbZwqx t where t.columnId = ? ");
		
		TypedQuery<ProgramaFbZwqx> ProgramaFbZwqxQuery = programaZwqxDao.getEntityManager().createQuery(zwqxHql.toString(), ProgramaFbZwqx.class);
		ProgramaFbZwqxQuery.setParameter(1, columnId);
		List<ProgramaFbZwqx> zwqxList = ProgramaFbZwqxQuery.getResultList();
		model.setZwqxList(zwqxList);
		//查询部门+职务权限
		StringBuilder deptZwqxHql = new StringBuilder("from ProgramaFbDeptZwqx t where t.columnId = ? ");
		TypedQuery<ProgramaFbDeptZwqx> ProgramaFbDeptZwqxQuery = programaDeptZwqxDao.getEntityManager().createQuery(deptZwqxHql.toString(), ProgramaFbDeptZwqx.class);
		ProgramaFbDeptZwqxQuery.setParameter(1, columnId);
		List<ProgramaFbDeptZwqx> deptZwqxList = ProgramaFbDeptZwqxQuery.getResultList();
		model.setDeptZwqxList(deptZwqxList);
		//查询部门+职务list权限
		StringBuilder deptZwqxListHql = new StringBuilder("from ProgramaFbDeptZwqxList t where t.columnId = ? ");
		TypedQuery<ProgramaFbDeptZwqxList> ProgramaFbDeptZwqxListQuery = programaDeptZwqxListDao.getEntityManager().createQuery(deptZwqxListHql.toString(), ProgramaFbDeptZwqxList.class);
		ProgramaFbDeptZwqxListQuery.setParameter(1, columnId);
		List<ProgramaFbDeptZwqxList> deptZwqxListList = ProgramaFbDeptZwqxListQuery.getResultList();
		model.setDeptZwqxListList(deptZwqxListList);
		//查询栏目群组权限list
		StringBuilder groupListHql = new StringBuilder("from ProgramaFbGroup t where t.columnId = ? and t.visible='"+InfoFbConstants.VISIBLE[1]+"'");
		TypedQuery<ProgramaFbGroup> ProgramaFbGroupQuery = columnFbGroupDao.getEntityManager().createQuery(groupListHql.toString(),ProgramaFbGroup.class);
		ProgramaFbGroupQuery.setParameter(1, columnId);
		List<ProgramaFbGroup> groupList =ProgramaFbGroupQuery.getResultList();
		model.setGroupList(groupList);
		return model;
	}
