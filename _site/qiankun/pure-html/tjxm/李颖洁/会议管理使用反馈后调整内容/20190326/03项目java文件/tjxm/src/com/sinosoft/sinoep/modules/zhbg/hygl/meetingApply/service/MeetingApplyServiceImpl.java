...
public class MeetingApplyServiceImpl implements MeetingApplyService {
	...
	public MeetingApplyInfo updateSubFlag(String id, String subflag) throws Exception {
		...
		if ("5".equals(subflag)&&meetingApplyInfo.getMeetingServices()!=null&&!"".equals(meetingApplyInfo.getMeetingServices())){
			...
			for (int i = 0; i < templist.size() ; i++){
	            map.put(templist.get(i).getResponsibleUserId(), templist.get(i).getServiceDeptId());
			}
			for (Map.Entry<String, String> entry : map.entrySet()) { 
				  SysWaitNoflow waitNoflwJu = new SysWaitNoflow();
					...
					waitNoflwJu.setTitle(meetingApplyInfo.getApplyTitle()+"需要提供会务服务");//待办显示标题
	...
	public Map<String,Object> getCount(String flag,String startDate, String endDate, String subflag) throws ParseException{
		...
		if("0".equals(flag)) {
			//按年
			sb1.append("  and substr(t.meeting_time, 0, 4) >= '" + startDate + "' and  substr(t.meeting_time,23,4) < ='" + endDate + "'" );
		}else if("1".equals(flag)) {
			//按月
			sb1.append("  and substr(t.meeting_time, 0, 7) >= '" + startDate + "' and  substr(t.meeting_time,23,7) < ='" + endDate + "'" );
		}else {
			//按周
			sb1.append("  and substr(t.meeting_time, 0, 10) >= '" + startDate + "' and  substr(t.meeting_time,23,10) < ='" + endDate + "'" );
		}
		...
		if("0".equals(flag)) {
			//按年
			sb2.append("  and substr(t.meeting_time, 0, 4) >= '" + startDate + "' and  substr(t.meeting_time,23,4) < ='" + endDate + "'" );
		}else if("1".equals(flag)) {
			//按月
			sb2.append("  and substr(t.meeting_time, 0, 7) >= '" + startDate + "' and  substr(t.meeting_time,23,7) < ='" + endDate + "'" );
		}else {
			//按周
			sb2.append("  and substr(t.meeting_time, 0, 10) >= '" + startDate + "' and  substr(t.meeting_time,23,10) < ='" + endDate + "'" );
		}
		if(UserUtil.getCruUserRoleNo().indexOf("D101")<0){
		...
		if("0".equals(flag)) {
			//按年
			sb3.append("  and substr(t1.meeting_time, 0, 4) >= '" + startDate + "' and  substr(t1.meeting_time,23,4) < ='" + endDate + "'" );
		}else if("1".equals(flag)) {
			//按月
			sb3.append("  and substr(t1.meeting_time, 0, 7) >= '" + startDate + "' and  substr(t1.meeting_time,23,7) < ='" + endDate + "'" );
		}else {
			//按周
			sb3.append("  and substr(t1.meeting_time, 0, 10) >= '" + startDate + "' and  substr(t1.meeting_time,23,10) < ='" + endDate + "'" );
		}
		if(UserUtil.getCruUserRoleNo().indexOf("D101")<0){
	...
	public PageImpl getNoticeList(Pageable pageable, PageImpl pageImpl, String title,
		...
		if (StringUtils.isNotBlank(startDate) && StringUtils.isNotBlank(endDate)) {
			querySql.append("   and substr(t.meeting_time,0,11) <= ? and substr(t.meeting_time,length(f.meeting_time)-18,11) >= ?");
			para.add(endDate);
			para.add(startDate);
		}
		/*if (StringUtils.isNotBlank(endDate)) {
