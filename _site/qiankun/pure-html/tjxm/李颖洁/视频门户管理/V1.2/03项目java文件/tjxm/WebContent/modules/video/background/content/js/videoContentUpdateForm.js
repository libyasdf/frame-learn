...
$(function() {
	...
	if(typeof(msgId)=="undefined"){
		...
		$("#workItemId").val(regVlaue(theRequest.workItemId));
	}
	...
	if (columnId != "") {
	    ...
	    httpRequest("get","/video/authority/contentFbZwqx/getAuthority",datas,function (json){
			if(json.flag == "1"){
				//回显部门权限
				var deptList = json.data.deptList;
				var groupList = json.data.groupList;
				var zwqxList = json.data.zwqxList;
				if(!$.isEmptyObject(deptList) || !$.isEmptyObject(groupList) || !$.isEmptyObject(zwqxList)){
					putStatus(true);
				}
			}
	    	})
	...
	if ($("#id").val() != "") {
		...
		httpRequest("get", "/video/content/edit", datas, function(data) {
			if(data.data.subflag=="2"){//发布状态要把发布按钮隐藏掉
				$("#fbButton").hide();
			}
			if(data.data.showStartTime!=""){
				$("#showTime").val(
...
function fishFileInput(imageUpId,con,path, id) {
	...
	$("#" + imageUpId).fileinput({
        ...
        //  第二次上传图片时 删除第一个上传的图片
        console.error('第二次上传图片时 删除第一个上传的图片');
        var fileSuccess = $('.file-preview-success');
        var kvZoomCache = $('.kv-zoom-cache');
        // console.log($('.file-preview-success').length)
        // console.log($('.kv-zoom-cache').length)
		...
		console.info("上传成功后返回：" + JSON.stringify(data));
		console.info(data)
		// 上传成功给出提示
		if (data) {
			/*var index = layer.msg("图片上传成功", {
				icon : 1
			});*/
...
function checkSaveDictionary(subflag, isTs, status) {
	...
	if (subflag == '2') {
		...
		//alert("file-caption-name:"+filesCount);
		//alert("imageIddfsdfs: "+$(".kv-file-content").val());
		//alert("filePath:"+filePath);
		...
		$.ajax({
					...
					success : function(json) {
						...
						if(subflag != "2" && (filesCount==0 || isUpdataAffix=="0")){
							...
							parent.refreshPage();
							return;
						}else if(subflag == "2" && isUpdataAffix=="0"  && $("#imageId").val()!='' && $("#imageId").val()!=null && $("#imageId").val() != undefined){
							//alert("12")
							$("#noPassButton").hide();
							$("#passButton").hide();
							$("#save").hide();
							var datas = {
								...
								httpRequest(
										...
										function(data) {
											if ('1' == data.flag) {
												if(typeof parent.refreshPage != 'undefined' && parent.refreshPage instanceof Function){
													parent.refreshPage();
												}
												window.top.refreshPage("daiban");
												
												//parent.refreshPage("daiban");
												console
														.log("删除待办成功！");
												parent.refreshPage('right_table');
											}
										});
							if(isSave=="1"){
								layer.msg("发布成功！", {
		...
		$.ajax({
					...
					success : function(json) {
						...
						if ('1' == json.flag) {
							...
							$("#id").val(json.data.id);
							$("#subflag").val(json.data.subflag);
							if(window.parent.$("#right_table1").text()){
								parent.TableInit.refTable('right_table1');
								parent.TableInit.refTable('right_table2');
							}
							//初始化文件上传(为保存后，将tableId放入上传参数中),并保存业务ID到附件表
							...
							var ima = document.getElementsByName("file_id");
							//alert(ima[0]);
							if(isUpdataAffix=='1'){
								$('#image').fileinput('upload');
							}
