...
public class ContrastServiceImpl implements ContrastService{
    ...
    public List<ContrastingRelations> saveContratingRelations(ContrastingRelations contratingRelations){
        ...
        if(StringUtils.isNotBlank(contratingRelations.getId())){
            ...
            //把顶级节点的名称加上，好区分是哪个门类的表；例如：科技档案（请勿删除）- 第三层
                String getCategoryNameSql =
                        "SELECT n.category_name\n" +
                                "  FROM dagl_category_table n\n" +
                                " WHERE n.category_code =\n" +
                                "       (SELECT m.m_table_name\n" +
                                "          FROM (SELECT t.*\n" +
                                "                  FROM (select s.m_table_name\n" +
                                "                          from tables_relation s\n" +
                                "                         start with s.s_table_name = ?\n" +
                                "                        connect by prior s.m_table_name = s.s_table_name\n" +
                                "                         order by rownum desc) t\n" +
                                "                 where instr(t.m_table_name, 'dak') = 0\n" +
                                "                   and rownum = 1) m)";
            String CategoryName = jdbcTemplate.queryForObject(getCategoryNameSql,String.class,contratingRelations.getTargetName());

            //拼接目标表的中文名
            contratingRelations.setTargetChnName(CategoryName+" - "+contratingRelations.getTargetChnName());
     ...
     **/
    @Override
    public void updateColumnInheritByTableAndColumnName(String tableName, String column_name){

        String sql="update table_struct_description t\n" +
                        "   set t.column_inherit = 'F'\n" +
                        " where t.table_name = ?\n" +
                        "   and t.column_name = ?";
        ...
        paraList.add(tableName);
        paraList.add(column_name);
        jdbcTemplate.update(sql,paraList.toArray());
    }
