...
function loadDropdown(data,index) {
    ...
    if (data.COLUMN_NAME === "basefolder_unit") {
       ...
       // $('.app form.row').append($input);
        ...
        var defaultData="";
        if(data.COLUMN_INHERIT == 'T' && lastAddData != null){
            //这里判断显示值继承还是默认值，还是默认日期
            defaultData = lastAddData[0][zujuan.toUpperCase()];
        }
        var $dropdownDom = $(''
            + '<div class="form-group col-xs-6">'
            + '<div class="col-xs-3">'
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                //如果这项为值继承的字段，那么就触发点击事件
                debugger;

                    if(data.COLUMN_INHERIT == 'T' && lastAddData != null){
                        for (var i in result) {
                            debugger;
                            if(result[i] == lastAddData[0][zujuan.toUpperCase()]){
                                debugger;
                                var dom = $dropdownDom.find('.dropdown-item');
                                dom.each(function(){
                                    $(this).text();
                                    if($(this).text()== lastAddData[0][zujuan.toUpperCase()]){
                                        $(this).click();
                                    }
                                });
                                //$dropdownDom.find('.dropdown-item').click();
...
function loadTextArea(data,index) {
    ...
    if(data.COLUMN_INHERIT == 'T' && lastAddData != null){
        //这里判断显示值继承还是默认值，还是默认日期
        defaultData = lastAddData[0][data.COLUMN_NAME.toUpperCase()];
    }else{
        defaultData = data.COLUMN_VALUE_DEFAULT ? data.COLUMN_VALUE_DEFAULT : defaultData
    }
    $('.app form.row').append($(''
