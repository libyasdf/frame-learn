    ...
    $.ajax({
        ...
        success:function (res) {
            ...
            if (res.flag == "1") {
                ...
                parentDom.find('.gr_index_block_ul').empty();
                if (!$.isEmptyObject(rows)) {
                    ...
                    $.each(rows,function (i,row) {
                        ...
                        if(row.type){
                        	if (row.type == "1") {
                                html+= '<li>'+ rowImg +'&nbsp;&nbsp;<a  '+ color +'   href="/gateway/infoDetail.html?columnCode='+ row.columnCode +'&id= '+ row.id +' "   target="_blank" title="'+ row.title +'">【'+ row.columnName +'】'+ row.title +'</a><span class="date">'+ row.time.substring(0, 10) +'</span></li>'
                            } else if (row.type == "2") {
                                html+= '<li>'+ rowImg +'&nbsp;&nbsp;<a  '+ color +'   href="javascript:void(0);"  onclick="openNotice(\''+ row.id +'\')"  title="'+ row.title +'">【'+ row.columnName +'】'+ row.title +'</a><span class="date">'+ row.time.substring(0, 10) +'</span></li>'
                            }
                        }else{
                        	//处理视频发布提醒内容逻辑
                        	//displayFile(id,fileUuid,fileFullName,title,videoId)
                        	html+= '<li>'+ rowImg +'&nbsp;&nbsp;<a  '+ color +'   href="javascript:void(0);"  onclick="displayFile(\''+ row.contentId +'\',\''+ row.firstVideoUuid +'\',\''+ row.firstVideoName +'\',\''+ row.contentTitle +'\',\''+ row.firstVideoId +'\')" title="'+ row.contentTitle +'">【'+ row.columeName +'】'+ row.contentTitle +'</a><span class="date">'+ row.time.substring(0, 10) +'</span></li>'
                        }
                    })
                }else{
                    html += "<li>";
                ...
                parentDom.find('.gr_index_block_ul').append(html);
            }
