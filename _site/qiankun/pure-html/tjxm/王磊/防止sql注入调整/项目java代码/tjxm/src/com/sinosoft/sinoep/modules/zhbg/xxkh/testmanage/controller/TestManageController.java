...
public class TestManageController {
	 ...
	 * @return JSONObject
	 */
	@LogAnnotation(value = "update",opName = "修改考试信息")
	@ResponseBody
	...
	public JSONObject sendWaitNoflow(String id,String user,String subject,String content,String messageURL,String daibanURL,String opName){
		...
		try {
			...
			String[] deptid = user.split(",");
			for (String vo : deptid) {
				...
				waitNoflw.setReceiveDeptId(vo);//接收人部门id 必填
				...
				waitNoflw.setRolesNo(XXKHCommonConstants.REPORT_ROLE[0]);//接受业务角色
				sysWaitNoflowService.saveWaitNoflow(waitNoflw, subject, content, messageURL+"?id="+id);
				SysWaitNoflow waitNoflw1 = new SysWaitNoflow();
				//waitNoflw1.setReceiveDeptName(vo.getDeptname());//接收人部门
				//waitNoflw.setReceiveUserId(vo.getUserid());//接受人id
				//waitNoflw1.setReceiveUserName("");//接受人name
				waitNoflw1.setReceiveDeptId(vo);//接收人部门id 必填
				...
				waitNoflw1.setTableName("xxkh_test_info");//业务表名
				waitNoflw1.setSourceId(id);//业务id
				waitNoflw1.setAttr1("");//预留字段1
				waitNoflw1.setAttr2("");//预留字段2
				waitNoflw1.setAttr3("");//预留字段3
				waitNoflw1.setRolesNo(XXKHCommonConstants.REPORT_ROLE[1]);//接受业务角色
				sysWaitNoflowService.saveWaitNoflow(waitNoflw1, subject, content, messageURL+"?id="+id);
			}
    ...
    public JSONObject deleteWaitNoflow(String testId,String receiveDeptId){
      ...
      //李颖洁20190329改动，按照角色发送待办，改为按照角色删除待办
        int num =sysWatiNoflowService.deleteWaitNoflow("",testId,receiveDeptId, "", XXKHCommonConstants.REPORT_ROLE[0]);
        int num1 =sysWatiNoflowService.deleteWaitNoflow("",testId,receiveDeptId, "", XXKHCommonConstants.REPORT_ROLE[1]);
        if (num !=0 && num1!=0){
            json.put("flag","1");
        }
        return json;
	 ...
	 * @return
	 */
	@LogAnnotation(value = "update",opName = "修改评卷状态 修改主观题的分数")
