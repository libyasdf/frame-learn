...
var $examNote = function() {
    var $dom = $(''
        ...
        +'温馨提醒：<br> 1. 答完试卷后请务必点击<b style="color:red">（我要交卷）</b>按钮，否则考试结果无法录入到系统。<br> 2. 请在规定时间内完成考试，考试时间结束系统会自动提交试卷。<br> 3. 考试结束后可查看客观题分数，主观题分数需要人工评卷完成之后，方可查看。'
        +'</div>');
 ...
 * @return
 **/
function submitTest(event,type){
        ...
        success:function(data){
            if ('1' == data.flag) {
                ...
                if("1"==isShowAnswer){
                    ...
                    layer.close(index);
                    if("js" == type){
                        layer.alert('考试时间结束，系统已为您自动交卷',{icon: 1}, function(){
                    ...
                    layer.close(index);
                    if("js" == type){
                        layer.alert('考试时间结束，系统已为您自动交卷,您可前往综合办公-学习考核-个人管理-成绩查询去查询本次考试详情',{icon: 1}, function(){
                            closeIfram();
                        })
                    }else{
