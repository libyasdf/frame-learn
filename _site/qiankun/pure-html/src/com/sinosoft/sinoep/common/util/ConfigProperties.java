 ...
 * <B>模块名称:</B><BR>
 * <B>中文类名:</B><BR>
 * <B>概要说明:</B>ConfigProperties 读取配置文件类<BR>
 * 
 * @author 中科软 kjx
 * @since 2016年5月16日
 */
public class ConfigProperties {
    ...
    private static transient Log log = LogFactory.getLog(ConfigProperties.class);

    //属性配置文件名称
    public static final String ENCODING = "utf-8";
    public static final String CONFIG_FILENAME = "configs/configs.properties";
    ...
    static {
        ...
        if (url != null) {
            try {
                ...
                log.info(new StringBuffer().append("加载属性配置文件[").append(CONFIG_FILENAME).append("]成功!").toString());
            }catch (ConfigurationException e) {
            	log.error(new StringBuffer().append("加载属性配置文件[").append(CONFIG_FILENAME).append("]出错!").toString(), e);
            }
        }
        else {
        	log.warn(new StringBuffer().append("属性配置文件[").append(CONFIG_FILENAME).append("]不存在!").toString());
        }
    }
    ...
    public static Properties getProperties(Class<?> class1, String propertiesFileName) {
        ...
        try {
        	...
        	log.error("加载类[" + class1.getName() + "]相关的属性配置文件[" + propertiesFileName + "]失败！", e);
        }
        finally {
            IOUtils.close(is);
        }
     ...
     * @return double双浮点型
     */
    public static double getDoublePropertyByKey(PropertiesConfiguration configuration, String strKey,
            double defaultValue) {
        if (configuration.containsKey(strKey)) {
     ...
     * @return boolean类型
     */
    public static boolean getBooleanPropertyByKey(PropertiesConfiguration configuration, String strKey,
    ...
    public static void saveProperties(String propertiesFileName, Map<String, Object> map) throws ConfigurationException {
        if (map == null || map.size() == 0) {
            throw new ConfigurationException("属性配置文件[" + propertiesFileName + "]无需要保存的属性列表!");
        }
     ...
     * @throws ConfigurationException
     */
    public static void savePropertiesAsNew(String propertiesFileName, Map<String, Object> map,
            String newPropertiesFileName) throws ConfigurationException {
        if (map == null || map.size() == 0) {
            throw new ConfigurationException("属性配置文件[" + propertiesFileName + "]无需要保存的属性列表!");
        }
        if (StringUtils.isBlank(newPropertiesFileName)) {
            throw new ConfigurationException("另存为的属性配置文件名不能为空!");
        }
        PropertiesConfiguration configuration = new PropertiesConfiguration(propertiesFileName);
        for (Entry<String, Object> entry : map.entrySet()) {
            configuration.setProperty(entry.getKey(), entry.getValue());
