...
public class SeatServiceImp implements SeatService{
	...
	public List<Seat> getSeatsByNoticeId(String meetingNoticeId,String attendDeptId){
		...
		StringBuilder sb = new StringBuilder("select t.id,t.seat_num,t.row1,t.col,t.owner,t.ownerid,t.state,t.auto_arrange,t.owner,t.attend_dept,t.attend_dept_id,t.fankui_type from hygl_seat t where t.noticeid= ?");
		para.add(meetingNoticeId);
		if(StringUtils.isNotBlank(attendDeptId)) {
			sb.append(" and t.attend_dept_id= ? ");
			para.add(attendDeptId);
		}
	...
	public void deleteByNoticeId(String flag,String meetingNoticeId,String seatListJson) {
		if("0".equals(flag)){
			//重置所有座位
			String sql = "delete from hygl_seat where noticeId= ?";
			jdbcTemplate.update(sql,meetingNoticeId);
		}else{
			//重置某些座位
	...
	public void cancelReserve(String meetingNoticeId,String seatListJson) {
		JSONArray jsonArry = JSONArray.fromObject(seatListJson);
		List<Object> para = new ArrayList<>();
		StringBuilder sb = new StringBuilder("delete from hygl_seat where noticeId = ? and (");
		para.add(meetingNoticeId);
		int i=1;
		for (Object object : jsonArry) {
			JSONObject jsonObj = JSONObject.fromObject(object);
			if(i==1){
				sb.append("  row1= ?  and col= ? ");
				para.add(jsonObj.getString("row"));
				para.add(jsonObj.getString("col"));
			}else{
				sb.append("  or row1= ?  and col= ? ");
				para.add(jsonObj.getString("row"));
				para.add(jsonObj.getString("col"));
			}
			i++;
	...
	private void feedbackByNum(String meetingNoticeId,String fankuiType) {
		...
		//根据会议通知id删除自动排座的那些座位
		String deleteSql = "delete from hygl_seat where state='2' and auto_arrange='0' and noticeId= ?";
		//jdbcTemplate.execute(deleteSql);
		jdbcTemplate.update(deleteSql,meetingNoticeId);
	...
	private void setBackInfoByNoticeId(String meetingNoticeId, final StringBuilder judeptid,final StringBuilder chudeptid,
		...
		String noticeBackSql = " select t.attend_person_num,t.attend_dept_id,t.attend_dept_type,t.attend_dept from  hygl_notice_back t where t.visible='1' and t.STATE='1' and t.notice_id= ?  ";
...
/*		for(int i=rowflag;i<=rowNum;i++){
	...
	private List<MeetingNoticeBack> attendeePersion(String meetingNoticeId) {
		...
		sb.append(" where t.notice_id= ? and t.visible = '1' and t.state = '1' and  t.ATTEND_DEPT_TYPE = '0' ");
		sb.append("  union ");
		...
		sb.append("  where t.notice_id= ? and t.visible = '1' and t.state = '1' and  t.ATTEND_DEPT_TYPE = '1' ");
		sb.append("  union ");
	...
	private void getOWnerIds(String meetingNoticeId,Map<String,Object> jsonMap) {
		...
		StringBuilder sb = new StringBuilder("select t.ownerid,t.row1,t.col,t.state,t.auto_arrange from hygl_seat t where t.auto_arrange!='0' and t.noticeid= ?  ");
	...
	private Map<String,Seat> getPeopleSeatInfo(String meetingNoticeId, List<UserSeat> list) {
        ...
        List<Object> para = new ArrayList<>();
		StringBuilder sql = new StringBuilder("select t.id,t.ownerid,t.row1,t.col from hygl_seat t where t.NOTICEID = ?  ");
		para.add(meetingNoticeId);
		if(StringUtils.isNotBlank(ownerids.toString())){
			String inString = CommonUtils.solveSqlInjectionOfIn(ownerids.toString(), para);
			sql.append(" and t.OWNERID in (" + inString + ") ");
		}
	...
	public List<Seat> manualSeatSave(String meetingNoticeId,String seatDataJson,String fankuiType) {
		...
		List<Object> para = new ArrayList<>();
		StringBuilder delSql = new StringBuilder(" delete from hygl_seat where noticeid= ? ");
		para.add(meetingNoticeId);
		...
		for (Object object : jSONArray) {
			...
			if(jsonObj.getString("ownerid")!=null && StringUtils.isNotBlank(jsonObj.getString("ownerid"))){
				...
				i++;
				if(i==1){
					delSql.append("  and ( row1= ? and col = ?  ");
					para.add(jsonObj.getInt("row"));
					para.add(jsonObj.getInt("col"));
				}else{
					delSql.append("  or row1= ? and col = ? ");
					para.add(jsonObj.getInt("row"));
					para.add(jsonObj.getInt("col"));
				}
			}
		...
		if(i>0){
			...
			jdbcTemplate.update(delSql.toString(),para.toArray());
		}
		dao.save(list);
		return list;
	...
	private List<Seat> getBySeatId(String meetingNoticeId,String commomSplit) {
		List<Object> para = new ArrayList<>();
		String sql = "  select t.id,t.row1,t.col,t.auto_arrange from hygl_seat t where t.noticeId = ? ";
		para.add(meetingNoticeId);
		String inString = CommonUtils.solveSqlInjectionOfIn(commomSplit, para);
		sql +=  "	and id in (" + inString + ") ";
	...
	public void publish(String meetingNoticeId,String fankuiType) {
		...
		//修改会议通知座位的状态
		StringBuilder noticeSql = new StringBuilder(" update HYGL_MEETING_NOTICE set publish_state='1' where id= ? ");
		StringBuilder seatSql = new StringBuilder(" update hygl_seat set publish_state='1' where  noticeid= ? ");
		/*jdbcTemplate.execute(noticeSql.toString());
		jdbcTemplate.execute(seatSql.toString());*/
		jdbcTemplate.update(noticeSql.toString(),meetingNoticeId);
		jdbcTemplate.update(seatSql.toString(),meetingNoticeId);
	}
	...
	private List<MeetingNoticeBack> attendeeInfo(String meetingNoticeId) {
		StringBuilder sb = new StringBuilder("select id,attend_dept,attend_dept_id,attend_dept_type  from hygl_notice_back where visible = '1' and state='1'  and notice_id= ?  ");
	...
	private void sendPersonDaiban(MeetingNoticeInfo notice,String meetingNoticeId) {
		...
		//查询此次参会的所有人的座位
		String sql = "select t.ownerid,t.attend_dept_id attendDeptId from hygl_seat t where t.noticeid= ? and  t.state='2' and t.owner is not null";
		List<Seat> list =jdbcTemplate.query(sql, new RowMapper<Seat>() {
					@Override
					public Seat mapRow(ResultSet result, int arg1) throws SQLException {
						Seat seat = new Seat();
		...
		for (Seat seat : list) {
			...
			waitNoflwJu.setTitle(now+UserUtil.getCruChushiName()+"发送座位图和座位票");//待办显示标题
			waitNoflwJu.setTableId(notice.getId());//业务表id
			waitNoflwJu.setTableName("HYGL_MEETING_NOTICE");//业务表名
			waitNoflwJu.setSourceId(notice.getId());//业务id
			waitNoflwJu.setAttr1("");//预留字段1
	...
	public JSONObject ticketList(String meetingNoticeId,String attendDeptType,String attentionItem,String attendDeptId,String userid) {
		...
		//更新通知里的注意事项
			String selectSql = " select t.attention_item from HYGL_MEETING_NOTICE t where t.id= ? ";
			attentionItem = jdbcTemplate.queryForObject(selectSql, String.class,meetingNoticeId);
		...
		sb.append(" where t.noticeid= ? and t.state!=4 ");
		para.add(meetingNoticeId);
		if(StringUtils.isNotBlank(attendDeptId)) {
			sb.append(" and t.ATTEND_DEPT_ID = ?");
			para.add(attendDeptId);
	...
	private void setPostion1(List<List<Seat>> list, Map<Integer, List<Seat>> seatMap, String userid) {
		...
		private void sendDeptDaiban1(Map<String, Map<String, List<String>>> deptRoleUserMap, MeetingNoticeInfo notice,
			...
			if(juDeptmap.size()>0) {
				...
				for (Entry<String, MeetingNoticeBack> entry : entrySet) {
					...
					waitNoflwJu.setDaibanUrl("/modules/zhbg/hygl/meetingNotice/arrangeSeat/daiban.html?flg=1&publishState=1&meetingNoticeId=" + notice.getId() + "&attendDeptId="+deptid+"&backId="+backinfo.getId()+"&isJu=1");//待办url  必填
					waitNoflwJu.setTitle(now+UserUtil.getCruChushiName()+"发送会议座位图和座位票");//待办显示标题
					waitNoflwJu.setTableId(backinfo.getId());//业务表id
					waitNoflwJu.setTableName("HYGL_MEETING_NOTICE");//业务表名
					waitNoflwJu.setSourceId(backinfo.getId());//业务id
					waitNoflwJu.setAttr1("");//预留字段1
					...
					waitNoflwJu1.setDaibanUrl("/modules/zhbg/hygl/meetingNotice/arrangeSeat/daiban.html?flg=1&publishState=1&meetingNoticeId=" + notice.getId() +"&attendDeptId="+deptid+"&backId="+backinfo.getId()+"&isJu=1");//待办url  必填
					waitNoflwJu1.setTitle(now+UserUtil.getCruChushiName()+"发送会议座位图和座位票");//待办显示标题
			...
			if(chuDeptmap.size()>0) {
				...
				for (Entry<String, MeetingNoticeBack> entry : entrySet) {
					...
					waitNoflwJu.setDaibanUrl("/modules/zhbg/hygl/meetingNotice/arrangeSeat/daiban.html?flg=1&publishState=1&meetingNoticeId=" + notice.getId() +"&attendDeptId="+deptid+"&backId="+backInfo.getId()+"&isJu=0");//待办url  必填
					waitNoflwJu.setTitle(now+UserUtil.getCruChushiName()+"发送会议座位图和座位票");//待办显示标题
					waitNoflwJu.setTableId(backInfo.getId());//业务表id
					waitNoflwJu.setTableName("HYGL_MEETING_NOTICE");//业务表名
					waitNoflwJu.setSourceId(backInfo.getId());//业务id
					...
					waitNoflwJu1.setOpName("会议的座位通知");//操作类型
					waitNoflwJu1.setDaibanUrl("/modules/zhbg/hygl/meetingNotice/arrangeSeat/daiban.html?flg=1&publishState=1&meetingNoticeId=" + notice.getId() +"&attendDeptId="+deptid+"&backId="+backInfo.getId()+"&isJu=0");//待办url  必填
					waitNoflwJu1.setTitle(now+UserUtil.getCruChushiName()+"发送会议座位图和座位票");//待办显示标题
					waitNoflwJu1.setTableId(backInfo.getId());//业务表id
					waitNoflwJu1.setTableName("HYGL_MEETING_NOTICE");//业务表名
					waitNoflwJu1.setSourceId(backInfo.getId());//业务id
		 ...
		 *  TODO 
		 * @author 李颖洁  
		 * @date 2019年4月3日下午3:49:03
		 * @param sourceId
		 * @param receiveDeptId
		 * @return
		 */
		@Override
		public int deleteWaitNoflow(String sourceId, String receiveDeptId,String isJu) {
			String rolesNo1 = "";
			String rolesNo2 = "";
			if("1".equals(isJu)){
				rolesNo1 = HyglConstants.REPORT_ROLE[0];
				rolesNo2 = HyglConstants.REPORT_ROLE[1];
			}else if("0".equals(isJu)){
				rolesNo1 = HyglConstants.REPORT_ROLE[2];
				rolesNo2 = HyglConstants.REPORT_ROLE[3];
			}
			int num = 0;
			num = sysWaitNoflowservice.deleteWaitNoflow("", sourceId, receiveDeptId, "", rolesNo1);
			int num1 = sysWaitNoflowservice.deleteWaitNoflow("", sourceId, receiveDeptId, "", rolesNo2);
			if(num>0&&num1>0){
				num = 1;
			}
			return num;
