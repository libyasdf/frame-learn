...
public class VideoContentController {
    ...
    public JSONObject sendFlow(VideoContent entity){
        JSONObject json = new JSONObject();
        json.put("flag","0");
        String cruDate = new SimpleDateFormat("yyyy年MM月dd日").format(new Date());
        ...
        try{
           ...
           service.saveFromBySql(entity);
            ...
            //JSONObject shDept = shDeptArray.getJSONObject(shDeptArray.size()-1);
            //waitNoflowEntity.setReceiveDeptName((String)shDept.get("deptname"));
            waitNoflowEntity.setReceiveDeptId(fbUserEntity.getShDeptId());
            ...
            waitNoflowEntity.setTitle(cruDate+userName+chuShiName+"视频申请");
            waitNoflowEntity = waitNoflowService.saveWaitNoflow(waitNoflowEntity,"视频发布",cruDate+userName+chuShiName+"发来的视频申请","/modules/video/background/content/videoContentUpdateForm.html?columnId="+entity.getColumnId()+"&id="+entity.getId());
            json.put("flag","1");
            json.put("data",entity);
    ...
    public JSONObject noPass(VideoContent entity,String workItemId){
        ...
        try{
            ...
            waitNoflowEntity.setOpName("视频发布");
            waitNoflowEntity.setDaibanUrl("/modules/video/background/content/videoContentUpdateForm.html?columnId="+entity.getColumnId());
            waitNoflowEntity.setTitle(cruDate+userName+chuShiName+"视频申请退回");
    ...
    public JSONObject updateFlag(String id,String subflag){
        ...
        json.put("flag","0");
        try{
            VideoContent videoContent =service.updateFlag(id,subflag);
            json.put("flag",subflag);
            if(videoContent.getSubflag().equals("5")){
                //发送消息通知
     ...
     * 查询个人门户最新发布的视频提醒
     * @author 王磊
     * @Date 2019年10月12日 下午3:29:18
     * @param id
     * @param subflag
     * @return
     */
    @LogAnnotation(value = "query",opName = "查询个人门户最新发布的视频提醒")
    @ResponseBody
    @RequestMapping("getVideoContentNoticeList")
    public PageImpl getVideoContentNoticeList(PageImpl page){
        try{
            page = service.getVideoContentNoticeList(page);
            page.setFlag("1");
        }catch (Exception e){
            e.printStackTrace();
            log.error(e.getMessage());
