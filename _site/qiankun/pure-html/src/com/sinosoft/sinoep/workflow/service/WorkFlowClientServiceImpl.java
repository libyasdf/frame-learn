...
public class WorkFlowClientServiceImpl implements WorkFlowClientService {
    ...
    public JSONObject completeFlow(String workitemid, String subflag, String recordid, String idea,String flag) {
        ...
        if (ConfigConsts.DUBBO_TYPE.equals(ConfigConsts.SERVICE_TYPE)) {
            // 本地服务调用
//            res = processControlService.cleanUp(json.toString());
            res = processControlService.normalCleanUp(json.toString());
        }else if (ConfigConsts.REST_TYPE.equals(ConfigConsts.SERVICE_TYPE)) {
            // http服务调用
            String url = WorkFlowConfigConsts.WORKFLOW_SERVICE_ROOT_URL + "/proceecontrol/normalCleanUp";
    ...
    public JSONObject deleteFlowInfo(String workflowId,String recordId) throws Exception{
        ...
        if (StringUtils.isNotBlank(recordId) && StringUtils.isNotBlank(workflowId)) {
            ...
            tempIdeaSql.append(" and t.filetypeid = ? ");
            jdbcTemplate.update(tempIdeaSql.toString(),workflowId);
            json.put("flag", "1");
            json.put("msg","执行成功");
        }else{
            json.put("msg","缺少参数");
