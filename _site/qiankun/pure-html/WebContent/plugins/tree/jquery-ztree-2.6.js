...
(function($) {
	...
	$.fn.zTree = function(zTreeSetting, zTreeNodes) {
		...
		if (setting.root[setting.nodesCol] && setting.root[setting.nodesCol].length > 0) {
			...
			if(setting.root.nodes[0].open==true){
			  asyncGetNode(setting, setting.root.nodes[0]);
			}
	...
	function bindTreeNodes(setting, treeObj) {
		...
		treeObj.unbind(ZTREE_DBCLICK);		
		treeObj.bind(ZTREE_DBCLICK, function (event, treeId, treeNode) {
		  tools.apply(setting.callback.dblclick, [event, treeId, treeNode]);
		});
