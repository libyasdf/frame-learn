	...
	$("#publishBtn").click(function(){
	    if($("#deptState").val() ==1){
            MyLayer.layerOpenUrl({
                url: "/modules/wenjuan/content/diaowen-design/surveyDeptEditForm.html?surveyId=" + questionBelongId+"&dwSurveyName="+$("#dwSurveyName").html(),
                title: "编辑调查问卷"
            })
        }
        if($("#deptState").val() ==0){
            MyLayer.layerOpenUrl({
                url: "/modules/wenjuan/content/diaowen-design/surveyGenalsEditForm.html?surveyId=" + questionBelongId+"&dwSurveyName="+$("#dwSurveyName").html(),
                title: "编辑调查问卷"
            })
        }
        publish(questionBelongId);
	});
...
function saveSurvey(callback){
	isSaveProgress=true;
	//保存问卷级别信息--之后财保存问卷中的题
	var svyNmSaveTag=$("input[name='svyNmSaveTag']").val();//欢迎页、题目
	var svyNoteSaveTag=$("input[name='svyNoteSaveTag']").val();//欢迎页，参考样例
	var svyAttrSaveTag=$("input[name='svyAttrSaveTag']").val();//选项设置
	if( svyNmSaveTag==="0" || svyNoteSaveTag==="0" || svyAttrSaveTag==="0"){
		var url = "/wenjuan/surveyDesign/ajaxSaveSet?";
		...
		//规则属性
		//收集规则 
//		var $checkBox = $("#modelUIDialogContent .dwSurveyAttrSetDialog :checkbox:checked");
 ...
 * 发布
 */
function publish(questionBelongId){
	debugger;
	curEditCallback();
	dwCommonDialogHide();
	resetQuItemHover(null);
	saveSurvey(function(){
		...
		$.ajax({
			url: "/wenjuan/surveyDirectory/publish",
			type: "POST",
			data: {
				surveyId: questionBelongId
			},
			dataType: "json",
			success: function(json){
				console.log(json);
...
function bindQuHoverItem(){
	...
	$(".dwQuDelete").click(function(){
		...
		if(confirm("确认要删除此题吗？")){
			...
			if(quId!=""){
				//var url="/design/question!ajaxDelete.action";
				var url="/wenjuan/surveyDirectory/ajaxDelete";
				var data="quId="+quId;
				$.ajax({
					url:url,
					data:data,
	...
	$(".addOption,.addColumnOption,.addRowOption").click(function(){
		...
		if(quType=="RADIO"){
			//添加单选选项
			editAble(addRadioItem(quItemBody,"新选项"));
			//editAble(quItemBody.find(".quCoItem table .editAble").last());
		}else if(quType=="CHECKBOX"){
			editAble(addCheckboxItem(quItemBody, "新选项"));
...
function saveRadio(quItemBody,callback){
	...
	if(saveTag==0){
		var url=ctx+"/wenjuan/quRadio/ajaxSave";
		var quType=quItemBody.find("input[name='quType']").val();
...
function deleteRadioOption(){
	...
	if(quOptionId!="" && quOptionId!="0" ){
		...
		var url="/wenjuan/quRadio/ajaxDelete";
...
function saveCheckbox(quItemBody,callback){
	...
	if(saveTag==0){
		...
		var url=ctx+"/wenjuan/quCheck/ajaxSave";
		var quType=quItemBody.find("input[name='quType']").val();
		var quId=quItemBody.find("input[name='quId']").val();
		var orderById=quItemBody.find("input[name='orderById']").val();;
		var isRequired=quItemBody.find("input[name='isRequired']").val();
...
function deleteCheckboxOption(){
	...
	if(quOptionId!="" && quOptionId!="0" ){
		...
		var url=ctx+"/wenjuan/quCheck/ajaxDelete";
		var data="quItemId="+quOptionId;
		$.ajax({
			url:url,
			data:data,
...
function saveFillblank(quItemBody,callback){
	var saveTag=quItemBody.find("input[name='saveTag']").val();
	if(saveTag==0){
		var url=ctx+"/wenjuan/quFillblank/ajaxSave";
		var quType=quItemBody.find("input[name='quType']").val();
		var quId=quItemBody.find("input[name='quId']").val();
		var orderById=quItemBody.find("input[name='orderById']").val();;
		var isRequired=quItemBody.find("input[name='isRequired']").val();
...
function saveScore(quItemBody,callback){
	var saveTag=quItemBody.find("input[name='saveTag']").val();
	if(saveTag==0){
		var url=ctx+"/wenjuan/quScore/ajaxSave";
		var quType=quItemBody.find("input[name='quType']").val();
		var quId=quItemBody.find("input[name='quId']").val();
		var orderById=quItemBody.find("input[name='orderById']").val();
		var isRequired=quItemBody.find("input[name='isRequired']").val();
...
function deleteScoreOption(){
	...
	if(quOptionId!="" && quOptionId!="0" ){
		...
		var url=ctx+"/wenjuan/quScore/ajaxDelete";
		var data="quItemId="+quOptionId;
		$.ajax({
			url:url,
			data:data,
...
function saveOrderqu(quItemBody,callback){
	var saveTag=quItemBody.find("input[name='saveTag']").val();
	if(saveTag==0){
		var url=ctx+"/wenjuan/quOrderqu/ajaxSave";
		var quType=quItemBody.find("input[name='quType']").val();
		var quId=quItemBody.find("input[name='quId']").val();
		var orderById=quItemBody.find("input[name='orderById']").val();
		var isRequired=quItemBody.find("input[name='isRequired']").val();
...
function deleteOrderquOption(){
	...
	if(quOptionId!="" && quOptionId!="0" ){
		...
		var url=ctx+"/wenjuan/quOrderqu/ajaxDelete";
		var data="quItemId="+quOptionId;
		$.ajax({
			url:url,
			data:data,
...
function savePagetag(quItemBody,callback){
	var saveTag=quItemBody.find("input[name='saveTag']").val();
	if(saveTag==0){
		var url=ctx+"/wenjuan/quPagetag/ajaxSave";
		var quType=quItemBody.find("input[name='quType']").val();
		var quId=quItemBody.find("input[name='quId']").val();
		var orderById=quItemBody.find("input[name='orderById']").val();;
		var isRequired=quItemBody.find("input[name='isRequired']").val();
...
function saveParagraph(quItemBody,callback){
	var saveTag=quItemBody.find("input[name='saveTag']").val();
	if(saveTag==0){
		var url=ctx+"/wenjuan/quParagraph/ajaxSave";
		var quType=quItemBody.find("input[name='quType']").val();
		var quId=quItemBody.find("input[name='quId']").val();
		var orderById=quItemBody.find("input[name='orderById']").val();;
		var isRequired=quItemBody.find("input[name='isRequired']").val();
...
function saveMultiFillblank(quItemBody,callback){
	var saveTag=quItemBody.find("input[name='saveTag']").val();
	if(saveTag==0){
		var url=ctx+"/wenjuan/quMultiFillblank/ajaxSave";
		var quType=quItemBody.find("input[name='quType']").val();
		var quId=quItemBody.find("input[name='quId']").val();
		var orderById=quItemBody.find("input[name='orderById']").val();
		var isRequired=quItemBody.find("input[name='isRequired']").val();
...
function deleteMultiFillblankOption(){
	...
	if(quOptionId!="" && quOptionId!="0" ){
		...
		var url=ctx+"/wenjuan/quMultiFillblank/ajaxDelete";
		var data="quItemId="+quOptionId;
		$.ajax({
			url:url,
			data:data,
...
function notify(msg,delayHid,callback) {
	$(".notification").remove();
	if(delayHid==null){
		delayHid=5000;
	}
