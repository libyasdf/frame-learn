$(function() {
	...
	if($("#id").val() != ""){
		...
		httpRequest("get","/djgl/ddjs/shyk/zbdydh/edit",datas,function (data){
...
function saveForm(){
	...
	if(bootstrapValidator.isValid()){
		...
		alert(check_val);
		$("#meetingType").val(check_val);
		alert($("#meetingType").val());*/
		$.ajax({
			type: "POST",
			url: "/djgl/ddjs/shyk/zbdydh/saveZbdydh",
			data:$("#form").serialize(),
			async: false,
...
function iniFileUpload(){
	...
	//初始化文件上传
	MyFileUpload.init({
		viewParams: {"tableId":$("#id").val(),"tableName":"DDJS_SHYK_PARTYCONGRESS"},
