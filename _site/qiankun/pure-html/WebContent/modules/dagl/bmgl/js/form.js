...
var treePid = "";
var chushiid = "";
var where = "";//记录了是从哪里打开的新增，用于回填立卷单位
var basefolderUnit = "";//父页面的立卷单位
$(function () {
	  //当前用户的角色编号
...
//	}
    ...
    basefolderUnit = GetRequest().basefolderUnit;
    // 获取表名，当前标签页层级，标签页总数
...
function getParentData() {
    $.ajax({
        ...
        error: function (err) {
            layer.msg('父级数据加载失败，请刷新页面',{icon: 2,time:1000});
        }
    });
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var timer = setInterval(function () {
                var dropdownMenuAll = $('.dropdown-menu');
                var dropdownMenuLength = (function (allDom) {
                    var length=0;
                    allDom.each(function () {
                        if($(this).children('li').length >  0) length++;
                    })
                    return length
                })(dropdownMenuAll);
                if (dropdownMenuLength == dropdownMenuAll.length){
                    xunifenlei();//回填虚拟分类
        ...
        error: function (err) {
            layer.msg('表单加载失败，请刷新页面',{icon: 2,time:1000});
        },
        complete: function () {
//
...
//        }
        ...
        for(var i in fileNumberRule){
        	...
        	if($("[name='" + name + "']").hasClass("dropdown-toggle")){
                ...
                $("[name="+name).css({"border-color":"rgb(204, 204, 204)","background-color":"rgb(238, 238, 238)"})
                $("[name="+name).next("ul").find("li").each(function(){
                    if($(this).html()==val){
                        $("[name="+name).attr("data-value",$(this).attr("data-value"));
                    }
                })
...
function loadTextInput(data,index) {
	...
	if(data.COLUMN_TYPE==5){
		if("pigeonhole_date"==data.COLUMN_NAME){
            defaultDay = getNowFormatDate();
        }
	}else if(data.COLUMN_INHERIT == 'T'&& lastAddData != null){
        //这里判断显示值继承还是默认值，还是默认日期
        defaultDay = lastAddData[0][data.COLUMN_NAME.toUpperCase()];
    }else{
        defaultDay = data.COLUMN_VALUE_DEFAULT ? data.COLUMN_VALUE_DEFAULT : defaultDay
    }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if(count == 1){
                            ...
                            var defaultData = "";
                            /*if("danweiyuliku" == where){
                                //单位预立库回填左侧树上方的立卷单位
                                defaultData = basefolderUnit;
                            }else if("yulikuLjdw" == where){
                                //预立库（按立卷单位）回填左侧下方树的立卷单位
                            }else if("yulikuMl" == where){
                                //预立库（按门类）回填左侧树上方的立卷单位
                            }*/

                            //获取已经选中的做回显
                            var dom =  $dropdownDom.find('.dropdown-item');
                            dom.each(function(){
                                //$(this).text();
                                if($(this).attr("data-value")== basefolderUnit){
                                    $(this).click();
                                }
                            });
...
function loadTextArea(data,index) {
    //如果是日期类型，默认设置为当前日期yyyymmdd 王磊 20190403
    var defaultData = data.COLUMN_VALUE_DEFAULT ? data.COLUMN_VALUE_DEFAULT : defaultData
    $('.app form.row').append($(''
        ...
        + '</div>'
        + '<div class="col-xs-9">'
        + '<textarea onkeyup="InputChange('+ index +')">' + (defaultData) + '</textarea>'
        + '<em class="input-em text-info"></em>'
        + '</div>'
        + '</div>'
    ));
...
function getlastAddData(){
    ...
    $.ajax({
        url: '/dagl/bmgl/findUserLastOne',
        data: {
            tName: tName, // 表名
            userId: getcookie("userid"),
            chushiId:$("button[name='basefolder_unit']").attr("data-value"),
            danghao : valM,
            columnName: mColName
        ...
        success: function (data) {
            if("1" == data.flag){
                lastAddData = data.data;
                //判断值继承的字段，赋值
                for (var i = 0; i < form.length; i++) {
                    //loadInput(form[i],i);
                    // T text S select A textarea(只会占一行) D treelayer F
                    var defaultDay = "";
                    if(form[i].COLUMN_INHERIT == 'T'&& lastAddData != null){
                        //这里判断显示值继承还是默认值，还是默认日期
                        defaultDay = lastAddData[0][form[i].COLUMN_NAME.toUpperCase()];
                        switch (form[i].COLUMN_INPUT_TYPE) {
                            case "T":
                                $("[name= '"+form[i].COLUMN_NAME+"']").val(defaultDay);
                                break;
                            case "S":
                                //获取已经选中的做回显
                                var dom = $("[name='"+form[i].COLUMN_NAME+"']").parent().find('.dropdown-item');
                                dom.each(function(){
                                    //$(this).text();
                                    if($(this).text()== defaultDay){
                                        $(this).click();
                                    }
                                });
                                break;
        ...
        error: function (err) {
            layer.msg('上次新增的数据加载失败，请刷新页面',{icon: 2,time:1000});
        }
    });
}
