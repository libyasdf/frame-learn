...
function initPage() {
    ...
    $('.dropdown-menu .dropdown-item').each(function(){
        if($(this).text() == '新增') {
            $(this).click(newlyAdded);
        }
        if($(this).text() == '替换') {
            $(this).click(replacelyAdded);
        }
        if($(this).text() == '简单查询') {
            $(this).click(easyFind);
        }
        ...
        if($(this).text() == '组合查询') {
            $(this).click(complexFind);
        }
        if($(this).text() == '移交') {
            $(this).click(submitData);
        }
        if($(this).text() == '删除') {
            $(this).click(dels);
        }
        ...
        if($(this).text() == '打印条形码') {
            $(this).click(printerCode);
        }
        if($(this).text() == '汇总') {
            $(this).click(gatherData);
        }
        if($(this).text() == '生成条码号') {
            $(this).click(saveCode);
        }
        //归档库去掉列表排序功能，单位预立库添加第一层表顺序调整的功能 王磊 20190423
        /*if($(this).text() == '排序') {
            $(this).click(orderBy);
        }*/
        if($(this).text() == '顺序调整') {
            $(this).click(adjustFirstLevel);
        }
    })
}
