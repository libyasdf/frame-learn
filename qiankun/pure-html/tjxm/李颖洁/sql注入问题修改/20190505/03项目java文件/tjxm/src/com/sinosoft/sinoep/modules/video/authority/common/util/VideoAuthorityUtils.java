...
public class VideoAuthorityUtils {
	...
	public static Map<String,List<Map<String, Object>>> getAuthListByContentId(String contentId){
		...
		StringBuilder sBuilder = new StringBuilder("");
		sBuilder.append("select t.content_id contentId,t.dept_id deptId from video_fb_dept t where t.content_id = ? and t.visible = '"+InfoFbConstants.VISIBLE[1]+"'");
		List<Map<String, Object>> deptList = jdbcTemplate.queryForList(sBuilder.toString(),contentId);
		authMap.put("deptAuth", deptList);
		sBuilder.setLength(0);
		sBuilder.append("select t.content_id contentId,t.position_id positionId from video_fb_zwqx t where t.content_id = ? and t.visible = '"+InfoFbConstants.VISIBLE[1]+"'");
		List<Map<String, Object>> positionList = jdbcTemplate.queryForList(sBuilder.toString(),contentId);
		authMap.put("positionAuth", positionList);
		sBuilder.setLength(0);
		sBuilder.append("select t.content_id contentId,t.dept_id deptId,t.position_id positionId from video_fb_dept_zwqx t where t.content_id = ? and t.visible = '"+InfoFbConstants.VISIBLE[1]+"'");
		List<Map<String, Object>> deptPositionList = jdbcTemplate.queryForList(sBuilder.toString(),contentId);
		authMap.put("deptPositionAuth", deptPositionList);
		sBuilder.setLength(0);
		sBuilder.append("select t.content_id contentId,t.group_id groupId from video_fb_group t where t.content_id = ? and t.visible = '"+InfoFbConstants.VISIBLE[1]+"'");
		List<Map<String, Object>> groupList = jdbcTemplate.queryForList(sBuilder.toString(),contentId);
