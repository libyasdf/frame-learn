...
public class HonorServiceImp implements HonorService{
	...
	public PageImpl getPageList(PageImpl pageImpl, Honor honor){
		...
		if(StringUtils.isNotBlank(honor.getYear())){
			sb.append(" and t.year = '" + honor.getYear() + "' ");
		}
		...
		Page<Honor> page = honorDao.query(sb.toString(), pageable);
	...
	public void changeNumber(String id, Integer number, String exchangeId, Integer exchangeNumber) {
		...
		sb.append(" update sys_honor_details set order_number = '"+exchangeNumber+"' where id = '"+id+"' ");
		StringBuffer sb1 = new StringBuffer();
		sb1.append(" update sys_honor_details set order_number = '"+number+"' where id = '"+exchangeId+"' ");
	...
	public List<Honor> getList(Honor honor) {
		...
		sb.append(" order by t.year desc ");
		return  honorDao.getEntityManager().createQuery(sb.toString()).getResultList();
	}
