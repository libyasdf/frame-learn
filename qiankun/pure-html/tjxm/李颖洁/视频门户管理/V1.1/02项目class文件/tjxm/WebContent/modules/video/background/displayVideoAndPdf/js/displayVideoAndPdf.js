	...
	if(showFileAreaId.indexOf("videos") != -1){
		...
		$.ajax({
			...
			success:function(data){
				...
				window.open("/modules/video/background/displayVideoAndPdf/playVideo.html?videoUrl="+videoUrl+"&infoId="+$("#id").val()+"&infoName="+encodeURIComponent($("span#dataName").text())+"&isRecord="+isRecord+"&fileFullName="+encodeURIComponent(fileFullName));
			},
			error:function(data){
...
function getUrlAndUpload(fileAreaId,showFileAreaId){
	...
	//限制一个内容只能上传一个视频
	//if($("#"+showFileAreaId).find("a").length==undefined || $("#"+showFileAreaId).find("a").length==0){ 
		//从参数中获得类别全路径
...
function saveFile(fileFullName,fileUuid,isVideo,fileId){
	...
	$.ajax({
		type: "post",
		url:"/video/content/videoAndPdf/saveFile",
...
function getFilesByInfoId(infoId){
	...
	$.ajax({
		type:"get",
		url:"/video/content/videoAndPdf/getByInfoId?infoId="+infoId,
...
function deleteFileDiv(obj){
	...
	if(videoLen==1 && $("#subflag").val()=='2'){
		layer.msg("请至少发布一个视频！",{icon: 0});
		return;
	}else{
		layer.confirm('确定删除吗？',{
			icon:3
			...
			,btn1:function(index){
		    	...
		    	$.ajax({
		    		type:"post",
		    		url:"/video/content/videoAndPdf/deleteById?fileId="+fileId,
		    		async: true,
		    		dataType:"json",
		    		success:function(data){
		    			if(data.flag == 1){
