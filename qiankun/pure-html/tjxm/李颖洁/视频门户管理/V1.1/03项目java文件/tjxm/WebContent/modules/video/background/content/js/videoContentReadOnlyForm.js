...
$(function() {
	...
	if ($("#id").val() != "") {
		...
		httpRequest("get", "/video/content/edit", datas, function(data) {
			if($('span[name="showTime"]').length>0){
				if(data.data.showStartTime!=""){
					$('span[name="showTime"]').html(data.data.showStartTime + " - " + data.data.showEndTime);
				}
			}
	...
	//if (bootstrapValidator.isValid()) {
		if (isFbfw == "1") {//存在发布范围
			...
			MyLayer.layerOpenUrl({
				url : "/modules/video/authority/authorityInfoReadOnly.html?contentId="
