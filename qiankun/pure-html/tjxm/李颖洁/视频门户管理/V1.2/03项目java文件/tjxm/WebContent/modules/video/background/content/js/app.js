	 ...
	 $.ajax({
	        ...
	        success: function (data) {
	            ...
	            if(data.length>0){
	            	...
	            	// 显示竖线
                    $('head').append('<style>#cd-timeline::before{ opacity:1 }</style>');
	            	$('.cd-timeline-block').eq(0).addClass('menu-playing');
	            }else{
	            	$('#cd-timeline').empty();
	            	console.log('清除竖线')
                    $('head').append('<style>#cd-timeline::before{ opacity:0 }</style>');
	            }
	            // 给当前播放的视频添加样式
 ...
 * 加载历史记录
 */
function loadHistory(flag,isLoadParentHistory){
	//console.info("历史记录")
	var historyStrs;
    $.post("/video/background/historyRecord/getLastestHistoryRecord", { },function(data){
    	...
    	if(isLoadParentHistory=="1"){
    		//加载视频门户的历史记录
    		window.opener.loadHistory("1","1",data)
    	}
    	if(data.length>0){
    		$("#historyList").empty();
    		if(flag=="0"){
...
function loadSeries(data, i) {
  ...
  if(i=="0" && playClass == "cd-play"){
    ...
    $dom.click(function () {
        ...
        timer ? clearInterval(timer) : '';  // 清除播放下一个视频定时器
...
function addVideoEvent() {
    ...
    $('#player').on('ended', function () {
        ...
        playerPlay()  // 修改暂停按钮状态
        document.getElementById('player').pause();  // 修改视频状态
...
function initVideoCon(){
    ...
    video.on('pause', playerPlay);
    video.on('play', playerPause);

    // 时间
    video.on('loadedmetadata', function() {
        // 计算分钟和秒
        var  durationArr = (video[0].duration/120).toFixed(2).split('.')
        // 计算小时
        var durationHours = parseInt(video[0].duration/3600 );
        var joinHours = durationHours  ? durationHours+':'  :  '';
        $('.duration').text(joinHours + durationArr[0] + ':' + durationArr[1])
        console.log(video[0].duration,video[0].duration/3600,video[0].duration/120,durationArr)
    });
    video.on('timeupdate', function() {
        var  current = (video[0].currentTime/120).toFixed(2).split('.')
        // 计算小时
        var currentHours = parseInt(video[0].currentTime/3600 );
        var jointHours = currentHours  ? currentHours+':'  :  '';
        $('.current').text(jointHours + current[0] + ":" + current[1]);
    });
    ...
    myVideo.onerror = function (){
        layer.msg("视频加载失败，请刷新页面重试！", {icon: 2});
    }
