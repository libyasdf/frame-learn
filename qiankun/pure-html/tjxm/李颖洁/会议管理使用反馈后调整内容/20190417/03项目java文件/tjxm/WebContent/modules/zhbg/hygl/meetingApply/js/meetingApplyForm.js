...
$(function(){
	...
	if($("#id").val() != ""){
		...
		//alert(getcookie("chuShiName"))
		//$("#creChuShiName1").val(getcookie("chuShiName"));
	...
	$.post("/zhbg/hygl/meetingRoomUseInfo/getByApplyId", data, function(data){
		if(data.flag=="1"){
			...
			for(var i=0;i<list.length;i++){
				...
				if($("#opertation").val() != "VIEW"){
					dom=dom+'<td width="15%" ><span class="delect glyphicon glyphicon-trash p-l-10" title="删除" style="cursor: pointer" data-index="'+ (i+1) +'" id="'+ list[i].id +'"></span></td> '
				}
				dom=dom+'</tr>'
...
function bindDelect(){
    $('.delect').on('click', function(){
        ...
        $(tr).remove();
        // 更新序号
        $('#table_list').find('tr').each(function(index,dom){
   	 ...
   	 $.ajax({
   		   ...
   		   success: function(data){
   		      if(data.flag=="0"){
   		    	  ...
   		    	  var msg="已选择的会议室中包含不可申请状态，请重新选择！"
   		    	  layer.msg(msg, {
   						icon : 0
   				 });
   		    	 return;
   		    	  ...
   		    	  //没有不可设置的会议室
   		    	  var curDate1=getCurrentDate("yyyy年MM月dd日");
   		    	 
   		    	  var title=curDate1+creUserName+creChuShiName+"会议室申请";
   		    	  ...
   		    	  var plan = saveForm();
   		    		if(plan!=""){
   		    			...
   		    			submitToFlow(oper,this,title,idea,$("#id").val(),"attr","attr1",$("#workitemid").val(),$("#workflowid").val());
   		    		}
   		      }
   		   }
   	});
...
function commitFlow1(){
	...
	arrStr=arr.join(",")
	$.post("/zhbg/hygl/meetingApply/checkServiceInfo", {meetingServiceIds:arrStr},function(data){
		var data1=data.data;
		if(data.flag=='1'){
			if(data1.length>0){
				var msg="提供";
				for(var i=0;i<data1.length;i++){
						msg=msg+"【"+data1[i].meetingService+"】"
				}
				msg=msg+"的负责人无权限，请联系会议管理员进行设置！"
				//msg=msg+"的负责人没有权限，请重新设置！"
	...
	$.post("/zhbg/hygl/meetingApply/checkServiceInfo", {meetingServiceIds:arrStr},function(data){
		...
		if(data.flag=='1'){
			if(data1.length>0){
				...
				completeFlow('',Config.approvalflag)
			}
		}
