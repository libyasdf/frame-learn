...
//		}
		...
		if(StringUtils.isNotBlank(meetingServices)){
			...
			if(meetingServices.indexOf(",")!=-1){
				...
				resSql+="where 1=1 and ((b.meeting_service_id like ? )";
				para.add("%"+mtServices[0]+"%");
				for(int i=1;i<mtServices.length;i++){
					resSql+=" or (b.meeting_service_id like ? )";
					para.add("%" + mtServices[i] + "%");
				...
				resSql+="where b.meeting_service_id like ? ";	
				para.add("%" + meetingServices + "%");
...
//		}
	 ...
	 * @author 李颖洁  
	 * @date 2020年1月1日上午3:47:24
	 * @param applyId 会议申请id
	 * @return
	 */
	@Override
	public String getResponseUserIds(String applyId){
		String userIds = "";
		String sql = "select to_char(wm_concat(distinct t.contact_id)) responseUserIds from hygl_meeting_service_task t where t.meetingroom_apply_id = ?";
		userIds = jdbcTemplate.queryForObject(sql, String.class,applyId);
		return userIds;
	}
}
