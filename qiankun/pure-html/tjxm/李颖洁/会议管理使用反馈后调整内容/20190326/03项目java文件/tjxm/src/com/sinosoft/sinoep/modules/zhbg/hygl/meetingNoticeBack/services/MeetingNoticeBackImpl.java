...
public class MeetingNoticeBackImpl implements MeetingNoticeBackService{
		...
		public MeetingNoticeBack saveForm(MeetingNoticeBack meetingNoticeBack) {
			...
			try {
				if (StringUtils.isBlank(meetingNoticeBack.getId())) {
					...
					if("1".equals(oldNoticeBack.getState())){
					...
					sysWaitNoflowservice.deleteWaitNoflow("",oldNoticeBack.getId(),oldNoticeBack.getAttendDeptId(),"","");
		...
		public PageImpl getPageListDraft1(Pageable pageable, PageImpl pageImpl, String startDate, String endDate,
			...
			if (StringUtils.isNotBlank(startDate) && StringUtils.isNotBlank(endDate)) {
				querySql.append("   and substr(f.meeting_time,0,10) <= '"+endDate+"' and substr(f.meeting_time,23,10)>='"+startDate+"'");
			}
			// 会议名称
			if (StringUtils.isNotBlank(meetingName)) {
				querySql.append("   and f.apply_title like '%"+meetingName+"%'");
			}
			// 状态
			if (StringUtils.isNotBlank(status)) {
				querySql.append("   and b.state = '"+status+"'");
			}
			// 拼接排序语句
			if (StringUtils.isBlank(pageImpl.getSortName())) {
			...
			content = jdbcTemplate.query(sb1.toString(),new BeanPropertyRowMapper<>(MeetingNoticeBack.class));
			//总个数
	    	StringBuilder sb2 = new StringBuilder("select count(1) from (");
	    	sb2.append(querySql.toString());
	    	sb2.append(")");
	    	Integer total=jdbcTemplate.queryForObject(sb2.toString(), Integer.class);
			//Page<MeetingNoticeBack> page = meetingNoticeBackDao.query(querySql.toString(), pageable, para.toArray());
			// 草稿列表，添加操作列
		...
		public PageImpl getBackedPageList(Pageable pageable, PageImpl pageImpl, String status, String id) {
			...
			String sql = " select sum(to_number(t.attend_person_num)) attendNum,sum(to_number(t.real_attend_num)) realAttendNum,sum(to_number(t.should_attend_num)) shouldAttendNum "
					+ "from hygl_notice_back t where t.notice_id = '"+id+"' and t.state = '"+status+"'";
			CountNoticeBack countNoticeBack = jdbcTemplate.queryForObject(sql,new BeanPropertyRowMapper<CountNoticeBack>(CountNoticeBack.class));
			// 草稿列表，添加操作列
			List<MeetingNoticeBack> content = page.getContent();
			for (MeetingNoticeBack meetingNoticeBack : content) {
		...
		public Map<String,List<MeetingNoticeBack>> findByNoticeId(String meetingNoticeId,String isJu,String isChuZhang,Map<String,String> attendDeptMap) {
			...
			if("0".equals(isJu) || "0".equals(isChuZhang)){
				...
				if("0".equals(isJu) && "0".equals(isChuZhang)){
					...
					//局长复选框被选中
					sb.append(" select t.ATTEND_PERSON_ID,t.attend_dept_id,t.attend_dept,'0' flag from hygl_notice_back t where t.visible = '1' and state='1' and t.ATTEND_DEPT_TYPE='0' and t.NOTICE_ID ='" + meetingNoticeId + "' ");
					sb.append(" union ");
					sb.append(" select t.attend_person_chu_id ATTEND_PERSON_ID,t.attend_dept_id,t.attend_dept,'0' flag from hygl_notice_back t where t.visible = '1' and state='1' and t.ATTEND_DEPT_TYPE='0' and t.NOTICE_ID ='" + meetingNoticeId + "' ");
					sb.append(" union ");
					sb.append(" select t.ATTEND_PERSON_LEAVE_ID ATTEND_PERSON_ID,t.attend_dept_id,t.attend_dept,'0' flag from hygl_notice_back t where t.visible = '1' and state='1' and t.ATTEND_DEPT_TYPE='0' and t.NOTICE_ID ='" + meetingNoticeId + "' ");
				}else if("1".equals(isChuZhang)){
					//处长长复选框被选中
					sb.append(" select t.ATTEND_PERSON_CHU_ID ATTEND_PERSON_ID,t.attend_dept_id,t.attend_dept,'1' flag from hygl_notice_back t where t.visible = '1' and state='1' and t.ATTEND_DEPT_TYPE='1' and t.NOTICE_ID ='" + meetingNoticeId + "' ");
				}
			}else{
				//局长和处长都选中了
				sb.append(" select t.ATTEND_PERSON_ID,t.attend_dept_id,t.attend_dept,'0' flag from hygl_notice_back t where t.visible = '1' and state='1' and t.ATTEND_DEPT_TYPE='0' and t.NOTICE_ID ='" + meetingNoticeId + "' ");
				...
				sb.append(" select t.ATTEND_PERSON_LEAVE_ID ATTEND_PERSON_ID,t.attend_dept_id,t.attend_dept,'0' flag from hygl_notice_back t where t.visible = '1' and state='1' and t.ATTEND_DEPT_TYPE='0' and t.NOTICE_ID ='" + meetingNoticeId + "' ");
				sb.append(" union ");
				sb.append(" select t.ATTEND_PERSON_CHU_ID,t.attend_dept_id,t.attend_dept,'1' flag from hygl_notice_back t where t.visible = '1' and state='1' and t.ATTEND_DEPT_TYPE='1' and t.NOTICE_ID ='" + meetingNoticeId + "' ");
			}
			list = jdbcTemplate.query(sb.toString(), new BeanPropertyRowMapper<MeetingNoticeBack>(MeetingNoticeBack.class));
