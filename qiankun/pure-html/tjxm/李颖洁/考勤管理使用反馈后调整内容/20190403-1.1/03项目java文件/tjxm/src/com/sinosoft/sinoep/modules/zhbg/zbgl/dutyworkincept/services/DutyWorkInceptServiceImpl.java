...
public class DutyWorkInceptServiceImpl implements DutyWorkInceptService {
	...
	public int updateVisible(String id,String deptId) throws SysException {
		...
		if(StringUtils.isBlank(id))
			throw new SysException("参数为空，删除数据失败,请重试！");
		String jpql = "update DutyWorkIncept t set t.visible = ? where t.zbNoticeId = ? and t.zbDeptId in ("+deptId+")";
		return dutyWorkInceptDao.update(jpql, CommonConstants.VISIBLE[1], id);
	}
	...
	public List<DutyWorkIncept> getList(PageImpl pageImpl,String zbNoticeId, String state) {
		...
		String sql = "select * from zbgl_duty_notice_incept t where t.visible = '"+state+"' ";
		if (StringUtils.isNotBlank(zbNoticeId)){
            sql += "	and t.zb_notice_id = '"+zbNoticeId+"'";
        }
		//拼接排序语句
		if (StringUtils.isBlank(pageImpl.getSortName())) {
		...
		list = jdbcTemplate.query(sql,new BeanPropertyRowMapper<>(DutyWorkIncept.class));
		return list;
	}
