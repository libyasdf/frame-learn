...
;(function ($, win) {
    ...
    function AddTr(arg) {
        ...
        this.deleteFn = arg.deleteFn || function () {
            return true
        };
        this.addTrFn = arg.addTrFn || function () {
            return true
        };
        // 调用初始化
        this.init();
    ...
    AddTr.prototype.init = function () {
        ...
        $('#' + that.deleteId).unbind('click').bind('click', function () {
            ...
            if ($("#isInnewOrReborrow").val() && $("#isInnewOrReborrow").val() == "1" && (rows.length - _objs.length) < 1) {
                layer.msg("至少保留一条数据！", {icon: 0});
                return false;
            }
            if (_objs.length == 0) {
