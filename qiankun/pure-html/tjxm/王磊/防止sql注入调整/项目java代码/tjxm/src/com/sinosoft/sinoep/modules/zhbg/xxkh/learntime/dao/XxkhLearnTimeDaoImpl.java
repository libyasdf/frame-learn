...
public class XxkhLearnTimeDaoImpl implements XxkhLearnTimeRepository {
	...
	public List<XxkhLearnTime> findDeptUserList(XxkhLearnTime learnTime) {
		...
		String sql = " select  t.cre_user_name,sum(t.minutes) as minutes " + " from" + " (select x.*,d.type "
				+ " from xxkh_learn_time x,xxkh_zi_liao d " + " where x.cre_dept_id = ? "
				+ " and x.zl_id = d.id) t" + " where t.type = ? group by t.cre_user_name";
		return jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(XxkhLearnTime.class),learnTime.getCreDeptId(),learnTime.getInfoId());
	}
}
