	...
	public JSONObject hasReportUserByDeptId(String deptIds,String deptNames) throws Exception {
		...
		try {
			...
			String sql = "select sfd.deptname,sfd.deptid,duser.userid" 
					...
					+"	on d.id = f.u_dept_id"
					+"	where f.roleid in (select t.roleid from sys_flow_role t where t.role_no in('"+ZBGLCommonConstants.AB_ROLE_NO[0]+"','"+ZBGLCommonConstants.AB_ROLE_NO[1]+"'))and d.status = '"+ZBGLCommonConstants.STATE[1]+"') t"
