		...
		layer.alert('确定要提交吗？点击确定按钮后将不能再修改考试人员范围和试卷信息！',
				...
				function(){
					...
					if(json.flag=="1"){
						...
						if(count==0){
							...
							if(res){
								...
								var params = {
										...
										"content":"请及时上报本次应考人员！",//消息内容
										"messageURL":"/modules/zhbg/xxkh/testManage/reportPersonsEditForm.html",//消息的URL
						...
						layer.alert('提交失败！'+names+'处室没有配置考试人员上报员，请联系系统管理员进行配置后再试！',
								{icon:0,title:'警告'});
					}
		});
	}
	...
	$.ajax({
		...
		success:function(json){
			if (json.flag == '1') {
				...
				for(var i in data1){
					$('#position').append('<label  class="checkbox-inline"><input name="dutyIds" type="checkbox" value="'+data1[i].value+'">'+data1[i].name+'</label>');
				}*/
				//创建职级
				$('#positionLevel').append('<label  class="checkbox-inline"><input id="inlineCheckboxAll2" type="checkbox" ">全选</label>');
				for(var i in data2){
					$('#positionLevel').append('<label class="checkbox-inline"><input type="checkbox"  name="levelIds" value="'+data2[i].value+'">'+data2[i].name+'</label>');
				}
				 //点击全选改变所有复选框状态
