...
var $markQuestion = function () {
    var $markQuestionListItem = $(''
        ...
        + '<div class="col-md-3">'
        + '<input class="form-control" type="number"  onkeydown="if(event.keyCode==13)event.keyCode=9" onKeypress="if((event.keyCode>=48 && event.keyCode<=57)) event.returnValue=true else event.returnValue=false" />'
        + '</div>'
        + '</div>'
        + '</div>');
...
function clickSaveButton(o) {
    ...
    for (var i = 0; i < $('.mark-content .mark-paper [type="number"]').length; ++i) {
       ...
       /* var min = $score.attr('min');
        ...
        sumScore += markItem.score==""?0:(markItem.score-0);
...
function clickSubmitButton() {
    ...
    $.ajax({
        type: "get",
        url:"/zhbg/xxkh/testmanage/updatePingJuan",
        data:{paperId:paperId,testId:testId,userId:userId,paperSubjectiveScore:sumScore,artificialMarkingState:1},
        dataType:"json",
        success:function(data){
            if ('1' == data.flag) {
                //parent.TableInit.refTable('right_table');
              ...
              $(window.parent.$(".left")).each(function(){
            	  $(this)[0].contentWindow.shuaxing();
              })
