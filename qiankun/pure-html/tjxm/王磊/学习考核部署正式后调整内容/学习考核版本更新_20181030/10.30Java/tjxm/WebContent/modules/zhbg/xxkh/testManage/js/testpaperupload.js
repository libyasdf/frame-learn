var MyPaperUpload = function () {
    ...
    var echoFile = function (fileListId, data, editOrView) {
        ...
        if (editOrView != "VIEW") {
    		...
    		$.each(data.rows, function (index, result) {
    			$('<div/>').addClass("col-md-10").append($('<a title="查看试卷信息" style= "color:blue" id=' + result.id + ' href = "javascript:void(0)" onclick="MyLayer.layerOpenUrl({url:\'/modules/zhbg/xxkh/paperManage/paperReadonly.html?id='+result.paperId+'\',width:\'90%\',height:\'98%\'});"/>').html(result.name + "&nbsp;&nbsp;&nbsp;")
                ).appendTo('#' + fileListId);
	        });
        }
    };
            	...
            	if(data.data.rows.length==0){
            		...
            		if(data.data.rows[0].paperState=="0"){
            			...
            			if(data.data.rows.length==1){
            				$("#isMoreChance1").prop("checked",true);
                        	$("#isMoreChance2").prop("checked",false);
            				$("#isMoreChance2").prop("disabled",true);
            			}else{
            				$("#isMoreChance2").prop("disabled",false);
            			}
            			//渲染试卷总分
            			if(_viewArg.isChange=="NO"){
