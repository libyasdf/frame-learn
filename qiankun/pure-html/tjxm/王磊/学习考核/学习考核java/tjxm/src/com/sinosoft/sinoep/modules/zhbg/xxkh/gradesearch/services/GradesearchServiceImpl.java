...
public class GradesearchServiceImpl implements GradesearchService {
	...
	public List<XxkhTestInfo> getScoreInquiry(PageImpl pageImpl,XxkhTestInfo testInfo,String timeRange) {
		...
		String sql = "select s.* from "+
				...
				" left join (select u.test_id,max(to_number(u.paper_subjective_score)+to_number(u.paper_objective_score)) as maxScore,min(to_number(u.paper_subjective_score)+to_number(u.paper_objective_score)) as minScore from xxkh_user_paper u where u.artificial_marking_state = '1' group by  u.test_id) u on t.id=u.test_id) s  " + 
				" left join (select u.test_id,count(1) as passPeople from xxkh_user_paper u where u.is_submit ='1'  group by  u.test_id) u on s.id =u.test_id ) s " + 
				...
				" left join (select u.test_id,count(1) as passPeople from xxkh_user_paper u,xxkh_test_info t where u.test_id = t.id and to_number(u.paper_subjective_score)+to_number(u.paper_objective_score)>= t.pass_score group by u.test_id) u on s.id = u.test_id) s "+
				" left join (select u.test_id,count(1) as sumPeople from xxkh_user_paper u   group by  u.test_id) u on s.id =u.test_id ) s " ;
	...
	public List<XxkhUserPaper> getbyTestContent(PageImpl pageImpl, XxkhTestInfo testInfo,String isJiGe) {
		...
		String sql = "select u.*,t.pass_score passScore,"
				...
				+"( select count(1) from xxkh_question_group  q where q.paper_id =   (select t.paper_id from xxkh_test_paper t where t.test_id='" +testInfo.getId()+"') and q.question_type = '5' " + 
				") as isZhuGuan"
				+ " from "
	...
	public List<XxkhTestInfo> getPersonalTestScore(PageImpl pageImpl, XxkhTestInfo info, String isPass) {
		...
		if (!StringUtils.isBlank(isPass)){
			if("1".equals(isPass)){
				conditionSql += "   and p.score >= d.pass_score";
			}else if("0".equals(isPass)){
				conditionSql += "   and p.score < d.pass_score";
			}
		}
		String Sql = "select t.user_id,t.test_id,t.paper_subjective_score,t.paper_objective_score,t.paper_id,t.begin_test_time,sum(t.paper_subjective_score+t.paper_objective_score) as score 	"	
				+ "		from xxkh_user_paper t  where t.user_id = "+UserUtil.getCruUserId()+" and t.is_submit = "+XXKHCommonConstants.IS_SUBMIT[1]+"  group by t.user_id ,t.test_id,t.paper_subjective_score,t.paper_objective_score,t.paper_id,t.begin_test_time 	";
		totalSql += "select count(distinct p.test_id) from ("+Sql+") p,xxkh_test_info d where p.test_id = d.id "+conditionSql;
		//全部列表查询
		querySql += "	select d.id,d.name,d.type,d.full_score,d.pass_score,d.start_time,d.end_time,p.paper_id as paperId,p.score	,p.paper_subjective_score as subjectiveScore,p.paper_objective_score as objectiveScore,p.begin_test_time as beginTestTime,	";
		querySql += "	("+totalSql+") as total	";
		querySql += "	from ("+Sql+") p,xxkh_test_info d 	";
		querySql += "	where p.test_id = d.id "+conditionSql;
		//拼接排序语句
		if (StringUtils.isBlank(pageImpl.getSortName())) {
			querySql += "  order by p.begin_test_time desc";
		}else{
