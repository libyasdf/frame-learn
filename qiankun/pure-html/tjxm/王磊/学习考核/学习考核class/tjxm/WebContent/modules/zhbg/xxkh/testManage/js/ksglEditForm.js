...
function saveTestPerson(){
	...
	var params = {
		...
		$.ajax({
			url:"/zhbg/xxkh/testmanage/saveTestPerson",
			data:params,
			dataType:"json",
			async: false,
			success:function(json){
        ...
        readOnlyFn: function () {
				...
				if(data.length==0){
					...
					if(chushi.length!=data.length){
						...
						if(chushi.length!=0){
							layer.alert(chushi+'，没有匹配的人员，请删除此处室！',{icon:0,title:'警告'});
						}
					}
			}
        }
...
function refreshTable(tableId){
	...
	TableInit.init({
        domId:tableId,
        url:"/zhbg/xxkh/testmanage/getTestPersonCountInUserPaper?rdm="+Math.random(),
        columns:right_table_echo_col,
        queryParams:function(params){
        	params.resId = $("#resId").val();
        	params.testId = $("#id").val();
        	//params.candidateChushiId = $("#unitId").val();
        	//params.candidateJuId = $("#juId").val();
            return params;
        },
        rowStyle: function (row, index) {	//默认样式：居中，
        ...
        readOnlyFn:function(){
        	if($("#inlineRadio2").is(":checked")){
        		...
        		$('#right_table2 tr.readOnly').each(function(){
        			var opt = $(this).find('td:eq(2)').find("span").html();
        			if (optChushi.indexOf(opt) > -1) {
        				optChushi.splice(optChushi.indexOf(opt), 1);
        			}
        		})
