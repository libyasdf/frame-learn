...
$(function () {
    ...
    if ($('#flowType input[name="flowType"]:checked ').val() == "1" && "formBorrow" !=formWhere) {//本单位
        $('#searchId').hide();
    }
});
...
function addTabFn(obj) {
    ...
    trCustom += '</td>';
    trCustom += '<td class="text-center" style="width: 12%">';
    trCustom += '	<select CK_info="立卷单位"  placeholder="请输入立卷单位" class="form-control" id="ljdwMark' + subTableLineNum + '" name="ljdwMark">';
