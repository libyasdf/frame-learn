        ...
        $('#tb_customer').bootstrapTable({
            ...
            onLoadSuccess:function(){
                ...
                $.ajax({
                    ...
                    data: {
                        tName: findTreeParent($.fn.zTree.getZTreeObj("treeDemo").getSelectedNodes()[0]).id//表名
                    },
                    success: function (data) {
                        if(data.flag =='1' && data.data.rows.length>0){
                            //根据获取的门类名称，获取档号规则
                            var category_id = data.data.rows[0].id;
                            $.ajax({
                                type: "get",
                                url: "dagl/xtpz/filenumberrule/getRule",
                                data: {
                                    category_id: category_id //表名
                                },
                                success: function (data) {
                                    if(data.flag =='1'){
                                        //根据获取的门类名称，获取档号规则
                                        fileNumberRule = data.data.rows;
                                        for(var i=0;i<fileNumberRule.length;i++){
                                            //档号规则的项不能设置为值继承
                                            if($('#tb_customer tr[data-uniqueid='+fileNumberRule[i].ruleField+']').find('a[data-name=COLUMN_INHERIT]').length=1){
                                                $('#tb_customer tr[data-uniqueid='+fileNumberRule[i].ruleField+']').find('a[data-name=COLUMN_INHERIT]').off('click');
                                                $('#tb_customer tr[data-uniqueid='+fileNumberRule[i].ruleField+']').find('a[data-name=COLUMN_INHERIT]').on('click',function () {
                                                    layer.msg("档号规则不可设置为值继承项！",{icon:0,time:2000});
                                                })
                                            }
                                        }
                                    }
                ...
                //档号规则的项不能设置为值继承
                if($('#tb_customer tr[data-uniqueid=basefolder_unit]').find('a[data-name=COLUMN_INHERIT]').length=1){
                    $('#tb_customer tr[data-uniqueid=basefolder_unit]').find('a[data-name=COLUMN_INHERIT]').off('click');
                    $('#tb_customer tr[data-uniqueid=basefolder_unit]').find('a[data-name=COLUMN_INHERIT]').on('click',function () {
                        layer.msg("立卷单位不可设置为值继承项！",{icon:0,time:2000});
                    })
                }
