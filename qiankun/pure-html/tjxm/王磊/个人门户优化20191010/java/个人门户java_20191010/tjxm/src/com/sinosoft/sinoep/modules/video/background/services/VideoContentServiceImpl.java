...
public class VideoContentServiceImpl implements VideoContentService {
	...
	public PageImpl getVideoContentNoticeList(PageImpl page) throws Exception{
		...
		StringBuilder sql = new StringBuilder();
		sql.append("select * from (select a.*,rownum rn from (select vcol.column_name as columeName,vcon.id as contentId,vcon.title as contentTitle,vcon.fb_time as contentFbTime from video_content vcon,video_column vcol where vcon.column_id=vcol.id and vcon.visible = '1' and vcon.subflag = '2' and (vcon.is_fb_content = '0' ");
		if(contentIds.length()>0){
		...
		for(Map<String,Object> map : contentList){
			...
			if(null != map.get("contentId")){
				...
				if(null != vMap.get("file_name")){
					vo.setFirstVideoId(String.valueOf(vMap.get("file_name")));
				}
				if(null != vMap.get("file_name")){
