...
public class UserUtil {
	...
	public static Map<String, SysFlowUserVo> getUserVo(String userIds){
        ...
        RedisClusterService redisService = (RedisClusterService)SpringBeanUtils.getBean("com_sinosoft_sinoep_common_jedis_services_RedisClusterService");
		Map<String, SysFlowUserVo> map = new HashMap<>();
		try {
			if(StringUtils.isNotBlank(userIds)){
                Object redisData = redisService.get(CommonConstants.USER_INFO_ALL_REDIS+ DigestUtils.md5Hex(userIds));
                if(redisData != null){
                    Map<String, String> redisMap = JSONUtils.getJsonAsObject(String.valueOf(redisData),Map.class);
                    for(Map.Entry<String, String> entry:redisMap.entrySet()){
                        SysFlowUserVo vo = (SysFlowUserVo)JSONObject.toBean(JSONObject.fromObject(entry.getValue()),SysFlowUserVo.class);
                ...
                if(map.size() <= 0){
                    ...
                    if(map.size() > 0){
                        redisService.set(CommonConstants.USER_INFO_ALL_REDIS+ DigestUtils.md5Hex(userIds),JSONObject.fromObject(map).toString());
                        //设置redis超时时间（1800秒 = 30分钟）
                        redisService.expire(CommonConstants.USER_INFO_ALL_REDIS+ DigestUtils.md5Hex(userIds),CommonConstants.EXPIRE_REDIS);
                    }
                }
			}
		} catch (Exception e) {
	 ...
	 */
	public static List<String[]> subUser(String userIds){
	...
	public static void setSession(HttpServletRequest request,String deptId) {
		...
		//设置登录信息（包括：登录类型、登录时间、ticket）
		String loginType = (String)request.getAttribute("loginType");
		if(StringUtils.isNotBlank(loginType) && CommonConstants.LOGIN_TYPE[1].equals(loginType)){
            //客户端登录
            session.setAttribute(UserConfigConsts.LOGIN_TYPE,CommonConstants.LOGIN_TYPE[1]);
        }else{
            //系统登录，非消息客户端登录
            session.setAttribute(UserConfigConsts.LOGIN_TYPE,CommonConstants.LOGIN_TYPE[0]);
            //设置ticket
            String ticket = (String)request.getAttribute("ticket");
            session.setAttribute(UserConfigConsts.TICKET,ticket);
        }
		//设置证书信息
		session.setAttribute(UserConfigConsts.COOKIE_KEY_CODE, CookieUtil.getKeyCode(request));
		//设置用户信息
		UserInfoService userInfoService = (UserInfoService)SpringBeanUtils.getBean("com_sinosoft_sinoep_user_service_UserInfoService");
		String userId = request.getAttribute("userid").toString();
		log.info("1.【dubbo】getUserInfoById 获取用户基本信息。");
		long aa = System.currentTimeMillis();
		UserInfo userInfo = userInfoService.getUserBase(userId, null);
		long bb = System.currentTimeMillis();
		log.info("  getUserInfoById 用时：" + (bb-aa)+" 毫秒");
		...
		if(StringUtils.isBlank(deptId)) {
		    log.info("2.【dubbo】getDept 获取用户所在部门ID。");
            long cc = System.currentTimeMillis();
			deptId = userInfoService.getDeptId(userId);
            long dd = System.currentTimeMillis();
            log.info("  getDept 用时：" + (dd-cc)+" 毫秒");
		}
		if(!deptId.contains(",")){
            log.info("3.【dubbo】getFlowDept 根据deptId获取所有父部门信息。（循环调用）");
            long cc = System.currentTimeMillis();
			List<Dept> deptList = userInfoService.getAllSuperDeptById(deptId);
            long dd = System.currentTimeMillis();
            log.info("  getFlowDept 用时：" + (dd-cc)+" 毫秒");
			if (deptList.size() > 0) {
			...
			//业务角色ID
            log.info("4.【rest】getFlowRoles 获取用户业务角色ID。");
            ...
            log.info("  getFlowRoles 用时：" + (ff-ee)+" 毫秒");
			//业务角色编号
            log.info("5.【rest】getFlowRoles 获取用户业务角色编号。");
            ...
            log.info("  getFlowRoles 用时：" + (ff-ee)+" 毫秒");
			//系统角色ID
            log.info("6.【rest】getFlowRlsy 获取用户系统角色ID。");
            ee = System.currentTimeMillis();
			session.setAttribute(UserConfigConsts.USER_SYSROLE_ID, RecourseUtils.getSysRoleIdByDept(userId,deptId));
            ff = System.currentTimeMillis();
            log.info("  getFlowRlsy 用时：" + (ff-ee)+" 毫秒");
			//系统角色编号
            log.info("7.【rest】getFlowRlsy 获取用户系统角色编号。");
            ee = System.currentTimeMillis();
		...
		log.info("设置session结束!耗时：" + (end-start) + " 毫秒");
	}
