...
public class DutyTableController {
    ...
    @ResponseBody
    public JSONObject getList(String timeRange,DutyTable dut){
        JSONObject json = new JSONObject();
        try {
            List<DutyTable> list = dutyTableService.getList(timeRange,dut);
            json.put("flag", "1");
     ...
     * @return net.sf.json.JSONObject
     **/
    @LogAnnotation(value = "query",opName = "值班查询")
    @RequestMapping(value = "getListOfZongDutySearch", method = RequestMethod.GET)
    @ResponseBody
    public PageImpl getListOfZongDutySearch(PageImpl pageImpl, DutyTable dut){
        try {
            Pageable pageable = new PageRequest(pageImpl.getPageNumber()-1, pageImpl.getPageSize());
            pageImpl = dutyTableService.getListOfZongDutySearch(pageable,pageImpl,dut);
        } catch (Exception e) {
            e.printStackTrace();
            log.error(e.getMessage(),e);
        }
    ...
    @ResponseBody
    public PageImpl getListOfDutySearch(PageImpl pageImpl, String year_month, DutyTable dut){
        try {
            Pageable pageable = new PageRequest(pageImpl.getPageNumber()-1, pageImpl.getPageSize());
            pageImpl = dutyTableService.getListOfDutySearch(pageable,pageImpl,year_month,dut);
        } catch (Exception e) {
            e.printStackTrace();
            log.error(e.getMessage(),e);
        }
     ...
     * @return void
     **/
    @LogAnnotation(value = "query",opName = "导出值班表模版")
     ...
     * @return net.sf.json.JSONObject
     **/
    @LogAnnotation(value = "save",opName = "导入值班表")
...
//            msg = dutyTableService.importInfo(file);
     ...
     * @Param [dut, request, response]
     * @return void
     **/
    @LogAnnotation(value = "query",opName = "导出excel")
    @RequestMapping(value = "/export", method = RequestMethod.POST)
    public void exportDutyLog(String year_month,String timeRange,DutyTable dut, HttpServletRequest request, HttpServletResponse response) {
        try {
            ExportDataToExcelOfMergeCells exportData = new ExportDataToExcelOfMergeCells();
            List<ExportBean> list = new ArrayList<ExportBean>();
            ExportBean exportBean = new ExportBean();
            List<DutyTable> DutyTablelist = dutyTableService.getList(timeRange,dut);//查询数据
            exportBean.setExportList(DutyTablelist);
            exportBean.setGetMethod(EmConstants.GET_METHOD);
            exportBean.setHeader(EmConstants.HEADER);
            exportBean.setSheetTitle(EmConstants.SHEET_TITLE[0]);
            exportBean.setSheetColWidth(EmConstants.SHEET_COL_WIDTH);// 列宽默认20，可以自己指定
            ...
            list.add(exportBean);
            String ChushiName = "";
            if(DutyTablelist.size()>0){
                ChushiName = DutyTablelist.get(0).getCreChushiName();
            }
            List<MergeCells> mergeCells = new ArrayList<>();
     ...
     * @return com.sinosoft.sinoep.common.util.PageImpl
     **/
    @LogAnnotation(value = "query",opName = "查询值班查询的数据（list）")
    @RequestMapping(value = "getListOfSearch", method = RequestMethod.GET)
    @ResponseBody
    public JSONObject getListOfSearch(DutyTable dut){
        JSONObject json = new JSONObject();
        try {
            List<DutyTable> list = dutyTableService.getListOfSearch(dut);
            json.put("flag", "1");
            JSONObject data = new JSONObject();
            data.put("total", list.size());
            JSONArray array = new JSONArray();
     ...
     **/
    @LogAnnotation(value = "query",opName = "导出excel")
    @RequestMapping(value = "/exportZzbcx", method = RequestMethod.POST)
    public void exportZzbcx(DutyTable dut, PageImpl pageImpl, HttpServletRequest request, HttpServletResponse response) {
        try {
            ExportDataToExcelOfMergeCells exportData = new ExportDataToExcelOfMergeCells();
            List<ExportBean> list = new ArrayList<ExportBean>();
            ExportBean exportBean = new ExportBean();
            List<DutyTable> DutyTablelist = dutyTableService.getListOfSearch(dut);//查询数据
            exportBean.setExportList(DutyTablelist);
            exportBean.setGetMethod(EmConstants.ZZBS_GET_METHOD);
            exportBean.setHeader(EmConstants.ZZBS_HEADER);
            exportBean.setSheetTitle(EmConstants.SHEET_TITLE[0]);
            exportBean.setSheetColWidth(EmConstants.SHEET_COL_WIDTH);// 列宽默认20，可以自己指定
            exportBean.setFontName(EmConstants.FONT_NAME);// 默认宋体
            exportBean.setTextFontSize(EmConstants.TEXT_FONT_SIZE);// 文本字体大小默认11
            ...
            MergeCells mergeCell2 = new MergeCells();
            mergeCell2.setCellLine(3);//需要合并的列
            mergeCell2.setStarRow(1);//从第几行开始合并
            mergeCells.add(mergeCell2);

            MergeCells mergeCell3 = new MergeCells();
            mergeCell3.setCellLine(4);//需要合并的列
            mergeCell3.setStarRow(1);//从第几行开始合并
            mergeCells.add(mergeCell3);

            MergeCells mergeCell4 = new MergeCells();
            mergeCell4.setCellLine(10);//需要合并的列
            mergeCell4.setStarRow(1);//从第几行开始合并
            mergeCells.add(mergeCell4);

            MergeCells mergeCell5 = new MergeCells();
            mergeCell5.setCellLine(11);//需要合并的列
            mergeCell5.setStarRow(1);//从第几行开始合并
            mergeCells.add(mergeCell5);

            exportData.excelProject(response, list, dut.getWatchDate()+"值班表.xls", mergeCells);
        } catch (Exception e) {
            e.printStackTrace();
            log.error(e.getMessage(), e);
        }
    ...
    /**
     * @Author 王富康
     * @Description //TODO 值班查询导出excel
     * @Date 2018/11/27 11:08
     * @Param [dut, pageImpl, request, response]
     * @return void
     **/
    @LogAnnotation(value = "query",opName = "值班查询导出excel")
    @RequestMapping(value = "/exportZbcx", method = RequestMethod.POST)
    public void exportZbcx(DutyTable dut, PageImpl pageImpl, HttpServletRequest request, HttpServletResponse response) {
        try {
            ExportDataToExcelOfMergeCells exportData = new ExportDataToExcelOfMergeCells();
            List<ExportBean> list = new ArrayList<ExportBean>();
            ExportBean exportBean = new ExportBean();
            List<DutyTable> DutyTablelist = dutyTableService.getListOfSearch(dut);//查询数据
            exportBean.setExportList(DutyTablelist);
            exportBean.setGetMethod(EmConstants.ZBCX_GET_METHOD);
            exportBean.setHeader(EmConstants.ZBCX_HEADER);
            exportBean.setSheetTitle(EmConstants.SHEET_TITLE[0]);
            exportBean.setSheetColWidth(EmConstants.SHEET_COL_WIDTH);// 列宽默认20，可以自己指定
            exportBean.setFontName(EmConstants.FONT_NAME);// 默认宋体
            exportBean.setTextFontSize(EmConstants.TEXT_FONT_SIZE);// 文本字体大小默认11
            ...
            MergeCells mergeCell4 = new MergeCells();
            mergeCell4.setCellLine(7);//需要合并的列
            mergeCell4.setStarRow(1);//从第几行开始合并
            mergeCells.add(mergeCell4);

            MergeCells mergeCell5 = new MergeCells();
            mergeCell5.setCellLine(8);//需要合并的列
            mergeCell5.setStarRow(1);//从第几行开始合并
            mergeCells.add(mergeCell5);
     ...
     * @return net.sf.json.JSONObject
     **/
    @LogAnnotation(value = "query",opName = "查询部门是否有日常值班上报员")
    @ResponseBody
    @RequestMapping("hasReportUser")
    public JSONObject hasReportUserByDeptId(String deptIds,String deptNames) {
        JSONObject json = new JSONObject();
        try {
            json = dutyTableService.hasReportUserByDeptId(deptIds,deptNames);
        } catch (Exception e) {
            e.printStackTrace();
            log.error(e.getMessage(),e);
            json.put("flag", "0");
