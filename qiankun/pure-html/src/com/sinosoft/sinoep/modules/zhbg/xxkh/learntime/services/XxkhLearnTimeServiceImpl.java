...
public class XxkhLearnTimeServiceImpl implements XxkhLearnTimeService {
	...
	public PageImpl getPageList(Pageable pageable, PageImpl pageImpl, String startTime, String endTime)
		...
		if (StringUtils.isNotBlank(startTime) && StringUtils.isNotBlank(endTime)) {
			querySql.append("   and (t.startTime <= ?  and t.overTime >= ? ");
			querySql.append("   or t.startTime <= ?  and t.overTime >= ? ");
			querySql.append("	or t.startTime >= ?  and t.overTime<= ? )");
			para.add(startTime);
			para.add(startTime);
			para.add(endTime);
	...
	public PageImpl getLearnTimeById(Pageable pageable, PageImpl pageImpl, String timeRange, String nodeIds) {
		...
		if (StringUtils.isNotBlank(nodeIds)) {
			String inString = CommonUtils.solveSqlInjectionOfIn(nodeIds,para);
			querySql.append(" and t.infoId in (" + inString + ")");
		}
		// 拼接排序语句 根据学习日期排序
		if (StringUtils.isBlank(pageImpl.getSortName())) {
	...
	public List<XxkhLearnTime> getLearnTotalTime(String startTime, String endTime, String nodeIds) {
		...
		if (StringUtils.isNotBlank(nodeIds)) {
			...
			querySql1.append("select info_id as infoId from xxkh_learn_time t where" + " t.cre_user_id='" + cruUserId
					...
					+ inString + ") order by t.learn_date desc");
			list1 = jdbcTemplate.query(querySql1.toString(), new BeanPropertyRowMapper<>(XxkhLearnTime.class),para.toArray());
		}
		if (list1 != null && list1.size() > 0) {
			...
			if (StringUtils.isNotBlank(startTime) && StringUtils.isNotBlank(endTime)) {
				querySql.append("	and (t.start_time <= ?  and t.over_time>= ? )");
				thisPara.add(endTime);
				thisPara.add(startTime);
			}

			if (StringUtils.isNotBlank(nodeIds)) {
				String inString = CommonUtils.solveSqlInjectionOfIn(nodeIds,thisPara);
				querySql.append(" and t.info_id in (" + inString + ")");
			}
			querySql.append(" order by t.start_time desc");
			List<XxkhLearnTime> list = jdbcTemplate.query(querySql.toString(),
					new BeanPropertyRowMapper<>(XxkhLearnTime.class),thisPara.toArray());
			return list;
		} else {
			return list1;
		}
	...
	public PageImpl getLearnDeptTotalTime(PageImpl pageImpl,Pageable pageable,String deptIds, String chushiIds, String positions,
		...
		if (StringUtils.isNotBlank(deptIds)) {
			String inString = CommonUtils.solveSqlInjectionOfIn(deptIds,para);
			querySql.append(" and t.cre_ju_id in (" + inString + ")");
		}
		if (StringUtils.isNotBlank(chushiIds)) {
			String inString = CommonUtils.solveSqlInjectionOfIn(chushiIds,para);
		...
		//这里的positions原来没有用，现在当做最少学习时间来用 王磊2019-03-01
		if(StringUtils.isNotBlank(positions)){
			querySql.append(" group by t.cre_ju_name,t.cre_chushi_name,t.cre_user_name,t.cre_user_id having sum(t.learn_time_h) >= ? ");
			para.add(Float.parseFloat(positions));
		}else{
		...
		List<XxkhLearnTime> list1 = jdbcTemplate.query(querySql.toString(),new BeanPropertyRowMapper<>(XxkhLearnTime.class),para.toArray());
		List<XxkhLearnTime> list = jdbcTemplate.query(resSql.toString(),
				new BeanPropertyRowMapper<>(XxkhLearnTime.class),para.toArray());
		pageImpl.setFlag("1");
