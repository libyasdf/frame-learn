...
import com.sinosoft.util.tool.DateUtils;
...
public class MeetingApplyServiceImpl implements MeetingApplyService {
	...
	public MeetingApplyInfo saveForm(MeetingApplyInfo meetingApplyInfo) throws Exception {
		...
		if (StringUtils.isBlank(meetingApplyInfo.getId())) {
			...
			oldMeetingApplyInfo.setMeetingPhone(meetingApplyInfo.getMeetingPhone());
		
			meetingApplyInfo = meetingApplyDao.save(oldMeetingApplyInfo);
		}
		//保存会议室信息
		String delSql = "delete from hygl_meetingroom_useinfo where apply_id= ? ";
		jdbcTemplate.update(delSql,meetingApplyInfo.getId());
	...
	public PageImpl getPageListDraft(Pageable pageable, PageImpl pageImpl, String title,String startDate, String endDate, String subflag,String noticeFlag, String applyTitle, String serviceDeptName, String serviceDeptId) throws Exception {
    	...
    	querySql(sb,title,startDate,endDate,subflag,noticeFlag,applyTitle,serviceDeptName,serviceDeptId,para);
    	sb.append(")");
    	Integer total=jdbcTemplate.queryForObject(sb.toString(), Integer.class,para.toArray());
    	//查询分页数据
    	para.clear();
		String pageSql = pageSql(pageable,title,startDate,endDate,subflag,noticeFlag,applyTitle,serviceDeptName,serviceDeptId,para);
	...
	private void querySql(StringBuilder sb, String title,String startDate, String endDate, String subflag,String noticeFlag, String applyTitle, String serviceDeptName, String serviceDeptId,List<Object> para) {
		...
		sb.append(" where t.cre_user_id = '" + UserUtil.getCruUserId() +"' and t.visible='1' and t.subflag = ?");
		para.add(subflag);
		...
		if (StringUtils.isNotBlank(noticeFlag)) {
			sb.append(" and t.notice_flag = ?");
			para.add(noticeFlag);
		}
		if (StringUtils.isNotBlank(applyTitle)) {
			sb.append(" and t.apply_title like ?");
			para.add("%"+applyTitle+"%");
		}
		if (StringUtils.isNotBlank(serviceDeptId)) {
			sb.append(" and t.host_dept_id= ?");
			para.add(serviceDeptId);
		}
		if (StringUtils.isNotBlank(startDate)) {
			sb.append(" and SUBSTR(t.meeting_time,23,10)>= ?");
			para.add(startDate);
		}
		if (StringUtils.isNotBlank(endDate)) {
			sb.append(" and SUBSTR(t.meeting_time,0,10)<= ?");
			para.add(endDate);
		}
	...
	private String pageSql(Pageable pageable,  String title,String startDate, String endDate, String subflag,String noticeFlag, String applyTitle, String serviceDeptName, String serviceDeptId,List<Object> para) {
	...
	public Map<String,Object> getCount(String flag,String startDate, String endDate, String subflag) throws ParseException{
		...
		sb.append(" where  t.visible = '"+CommonConstants.VISIBLE[1]+"'  and t.subflag = ?");
		para.add(subflag);
		if("0".equals(flag)) {
			//按年
			//sb.append("  and substr(t.meeting_time, 0, 4) >= '" + startDate + "' and  substr(t.meeting_time,23,4) < ='" + endDate + "'" );
			sb.append("  and substr(t.meeting_time, 0, 4) >= ? and  substr(t.meeting_time,0,4) < = ?" );
			para.add(startDate);
			para.add(endDate);
			...
			sb.append("  and substr(t.meeting_time, 0, 7) >= ? and  substr(t.meeting_time,0,7) < = ?" );
			para.add(startDate);
			para.add(endDate);

		}else {
			//按周
			sb.append("  and substr(t.meeting_time, 0, 10) >= ? and  substr(t.meeting_time,0,10) < = ?" );
			para.add(startDate);
			para.add(endDate);
		...
		sb1.append(" from hygl_meeting_apply t where   t.visible = '"+CommonConstants.VISIBLE[1]+"'  and t.subflag = ?");
		para.add(subflag);
		if("0".equals(flag)) {
			//按年
			sb1.append("  and substr(t.meeting_time, 0, 4) >= ? and  substr(t.meeting_time,0,4) < = ?" );
			para.add(startDate);
			para.add(endDate);
		}else if("1".equals(flag)) {
			//按月
			sb1.append("  and substr(t.meeting_time, 0, 7) >= ? and  substr(t.meeting_time,0,7) < = ?" );
			para.add(startDate);
			para.add(endDate);
		}else {
			//按周
			sb1.append("  and substr(t.meeting_time, 0, 10) >= ? and  substr(t.meeting_time,0,10) < = ?" );
			para.add(startDate);
			para.add(endDate);
		}
		...
		List<Map<String,Object>> list1 = jdbcTemplate.queryForList(sb1.toString(),para.toArray());
		
		para.clear();
		//按照主办单位获取会议次数
		StringBuilder sb2 = new StringBuilder("select count(id) meetCount ,t.host_dept_name meetType from hygl_meeting_apply t " );
		sb2.append("  where  t.visible = '"+CommonConstants.VISIBLE[1]+"'  and t.subflag = ?");
		para.add(subflag);
		if("0".equals(flag)) {
			//按年
			sb2.append("  and substr(t.meeting_time, 0, 4) >= ? and  substr(t.meeting_time,0,4) < = ?" );
			para.add(startDate);
			para.add(endDate);
		}else if("1".equals(flag)) {
			//按月
			sb2.append("  and substr(t.meeting_time, 0, 7) >= ? and  substr(t.meeting_time,0,7) < = ?" );
			para.add(startDate);
			para.add(endDate);
		}else {
			//按周
			sb2.append("  and substr(t.meeting_time, 0, 10) >= ? and  substr(t.meeting_time,0,10) < = ?" );
			para.add(startDate);
			para.add(endDate);
		}
		if(UserUtil.getCruUserRoleNo().indexOf("D101")<0){
		...
		List<Map<String,Object>> list2 = jdbcTemplate.queryForList(sb2.toString(),para.toArray());
		...
		sb3.append("  where  t1.visible = '"+CommonConstants.VISIBLE[1]+"'  and t1.subflag = ?");
		para.add(subflag);
		if("0".equals(flag)) {
			//按年
			sb3.append("  and substr(t1.meeting_time, 0, 4) >= ? and  substr(t1.meeting_time,0,4) < = ?" );
			para.add(startDate);
			para.add(endDate);
		}else if("1".equals(flag)) {
			//按月
			sb3.append("  and substr(t1.meeting_time, 0, 7) >= ? and  substr(t1.meeting_time,0,7) < = ?" );
			para.add(startDate);
			para.add(endDate);
		}else {
			//按周
			sb3.append("  and substr(t1.meeting_time, 0, 10) >= ? and  substr(t1.meeting_time,0,10) < = ?" );
			para.add(startDate);
			para.add(endDate);
		}
		if(UserUtil.getCruUserRoleNo().indexOf("D101")<0){
		...
		List<Map<String,Object>> list3 = jdbcTemplate.queryForList(sb3.toString(),para.toArray());
	 ...
	 */
	@Override
	public MeetingApplyInfo removedByMeetingAdmin(MeetingApplyInfo info, String ifRemovedByadmin,String subflag) throws Exception {
		//20190524李颖洁修改，增加更改撤办的会议的状态为撤办
		updateSubFlag(info.getId(), subflag);
		//撤办会议标记为已撤办
		MeetingApplyInfo meetingApplyInfo = getById(info.getId());
		meetingApplyInfo.setIfRemovedByadmin(ifRemovedByadmin);
		meetingApplyInfo = meetingApplyDao.save(meetingApplyInfo);
		List<String> userIds = new ArrayList<>();
		List<String> userIds1 = new ArrayList<>();
		//查找会务服务人员
		String responseUserIds = meetingServiceNoticeService.getResponseUserIds(info.getId());
		if(StringUtils.isNotBlank(responseUserIds)){
			for (String userId : responseUserIds.split(",")) {
				userIds.add(userId);
			}
		}
		//查找参会人员
		String attendPersonIds = meetingNoticeBackService.getAllAttendPersonIds(info.getId());
		if(StringUtils.isNotBlank(attendPersonIds)){
			for (String userId : attendPersonIds.split(",")) {
				userIds.add(userId);
			}
		}
		//给以上人员发消息
		if(userIds.size()>0){
			for (String userId : userIds) {
    			if(!userIds1.contains(userId)){
    				userIds1.add(userId);
    				sysWaitNoflowservice.sendMsgsByUid(userId, "会议取消通知", info.getMeetingTime()+"期间"+info.getHostDeptName()+"举办的【"+info.getApplyTitle()+"】会议已经取消", "", 
    						com.sinosoft.sinoep.common.util.DateUtil.CHINESE_FULL.getDateText(new Date()), new SysWaitNoflow());
    			}
    		}
		}
		return meetingApplyInfo;
	...
	public List<MeetingServiceInfo> checkServiceInfo(String meetingServiceIds) throws Exception {
		...
		if (StringUtils.isNotBlank(meetingServiceIds)){
			List<Object> para = new ArrayList<>();
			String inString = CommonUtils.solveSqlInjectionOfIn(meetingServiceIds, para);
			StringBuilder sb = new StringBuilder("select t.id,t.responsible_user_id responsibleUserId,t.meeting_service meetingService,t.responsible_dept_id responsibleDeptId from HYGL_MEETING_SERVICE t where t.id in(" + inString + ")");
			list = jdbcTemplate.query(sb.toString(), new RowMapper<MeetingServiceInfo>() {
				...
				public MeetingServiceInfo mapRow(ResultSet result, int arg1) throws SQLException {
					...
					serviceInfo.setResponsibleDeptId(result.getString("responsibleDeptId"));
					return serviceInfo;
				}
			},para.toArray());
		}
		...
		//查询服务负责人的详细信息20190419李颖洁修改
		if(list.size()>0){
			String userRole = "";
			for (MeetingServiceInfo info : list) {
				userRole = RecourseUtils.getFlowRoleNoByDept(info.getResponsibleUserId(),info.getResponsibleDeptId());
				if(StringUtils.isNotBlank(userRole) && userRole.contains(HyglConstants.SERVICE_ROLE)) {
					//记录拥有会务服务角色的用户id
					userIds.add(info.getResponsibleUserId());
				}
			}
			if(userIds.size()>0) {
				for (String userId : userIds) {
