...
import com.sinosoft.sinoep.modules.zhbg.zbgl.config.source.Task;
...
public class ConfigController {
    ...
    /**
     * @Author 王富康
     * @Description //TODO 手动触发定时器，执行生成下月的基础配置
     * @Date 2019/1/11 15:23
     * @Param []
     * @return void
     **/
    @LogAnnotation(value = "save",opName = "手动触发定时器，执行生成下月的基础配置")
    @ResponseBody
    @RequestMapping("HandcreJcpzOnFistDay")
    public void HandcreJcpzOnFistDay(){

        try{
            task.creJcpzOnFistDay();
        }catch (Exception e){
            e.printStackTrace();
            log.error(e.getMessage(),e);
        }
