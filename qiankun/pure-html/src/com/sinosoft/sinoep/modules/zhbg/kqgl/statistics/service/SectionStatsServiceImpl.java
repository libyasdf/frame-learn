...
public class SectionStatsServiceImpl implements SectionStatsService {
	...
	@Override
	public PageImpl getList(Pageable pageable,PageImpl pageImpl,String timeRange, String userids,String deptid,String deptname,String flg,String isAll,Integer total,String from) throws Exception {
		...
		if((!StringUtils.isNotBlank(userids)) && "0".equals(from)) {//不需要查询任何人(从领导查询统计点击过来）
			return new PageImpl(total,personalStatsList);
		}
		/*if(roleNo.contains(KqglConstant.KQMANGE_ROLE_ID) && (!StringUtils.isNotBlank(userids)) && "0".equals(flg) && "0".equals(isAll)) {
			//如果当前用户是考勤管理员，并且没有查询条件(并且不是从领导查询统计列表的只读页面点击过来)，不返回任何数据
			 return new PageImpl(total,personalStatsList);
			//return personalStatsList;
		}*//*else if(roleNo.contains(KqglConstant.KQMANGE_ROLE_ID) && "0".equals(flg) && "1".equals(isAll) && (!StringUtils.isNotBlank(userids))) {
			//如果当前用户是考勤管理员，（点击的是领导查询列表），查询的是所有用户 (此处应该现在不会用到)
		...
		if(StringUtils.isNotBlank(deptid)){
			...
			if(StringUtils.isNotBlank(userids)){
				//设置用户展示的顺序
				setUserIdOrder(userids,userOrder);
			}else {
		...
		if("1".equals(from)) {
			...
			for(int i=start;i<=end;i++) {
				...
				if(userIds.length>index) {
					if(StringUtils.isNotBlank(userIds[index])) {
						userids=userids+userIds[index]+",";
					}
				}
			}
		}
		//某些用户某段时间的请假数据
		List<Map<String,Object>> leaveData=new ArrayList<Map<String,Object>>();
		if(StringUtils.isNotBlank(userids)) {
			 leaveData = getOverTimeData(startDate,endDate,userids);
		...
		if(StringUtils.isNotBlank(userids)) {
			...
			String sql = getSql(startDate,endDate,userids,para);
			System.out.print(sql);
			List<Map<String,Object>> list = jdbcTemplate.queryForList(sql,para.toArray());
			 personalStatsList = setEntity(list,userInfo,BusinessTrips,userOrder,userids,from,start);
		}
		//查询某些人的考勤卡号
		//getCardNoms(userids,cardNumAndUserInfo);
		//查询某段时间某些人的考勤数据
		List<KqglAttendenceHandle> kqInfo = new ArrayList<KqglAttendenceHandle>();
		if(cardNumAndUserInfo.size()>0){
			kqInfo = getKqInfo(startDate, endDate, CommonUtils.commomSplit(cardNumAndUserInfo.getString("cardNum")));
		}
	...
	private void setUserIdOrder(String userids, Map<String, Integer> userOrder) {
		String[] userIds = userids.split(",");
		int index=0;
		for (String userid : userIds) {
			userOrder.put(userid, index);
			index++;
		}
	...
	private String getUserIds(String cruDeptId,Map<String,Integer> userOrder) {
		...
		for (Object object : deptUserList) {
			...
			if("user".equals(type)){
				if(!userids.contains(jsonObj.getString("uuid"))) {
					userids=userids+jsonObj.getString("uuid")+",";
	 ...
	 * @return
	 */
	private List<PersonalStats> setEntity(List<Map<String, Object>> list,Map<String,SysFlowUserVo>  userInfo,List<Map<String,Object>> BusinessTrips,Map<String,Integer> userOrder,String userids,String from,Integer index) {
		List<PersonalStats> personalStatsList = new ArrayList<PersonalStats>();
		...
		if("1".equals(from)) {
			//从查询统计的只读页面点击过来，由于领导查询统计是在前端分的页，而查询统计的只读页面是从后端分的页，需要重新初始化序号
			for (PersonalStats personStats : personalStatsList) {
				personStats.setSequenceNum(index);
				index++;
			}
		}
	...
	private String getSql(String startDate,String endDate,String userids,List<Object> para) {
		StringBuilder sb = new StringBuilder("");
		...
		String temp1 = useridIntoSql1(useridList,para);
		sb.append(temp1).append(")");
		sb.append("union");
		sb.append("(select  distinct cre_user_id from KQGL_BUSINESS_TRIP_INFO where subflag='5' ");
		useridList = new ArrayList<String>(Arrays.asList(userids.split(",")));
		String temp2 = useridIntoSql1(useridList,para);
		sb.append(temp2);
		sb.append(")");
		sb.append("union");
		sb.append("(select  distinct cre_user_id from kqgl_go_out_info where subflag='5' ");
		useridList = new ArrayList<String>(Arrays.asList(userids.split(",")));
		String temp3 = useridIntoSql1(useridList,para);
		sb.append(temp3);
		sb.append(")");
		sb.append("union");
		sb.append("(select  distinct cre_user_id from KQGL_SUPPLEMENT_SIGN_INFO where subflag='5' ");
		useridList = new ArrayList<String>(Arrays.asList(userids.split(",")));
		String temp4 = useridIntoSql1(useridList,para);
		sb.append(temp4);
		sb.append(")");
		sb.append("union");
		sb.append("(select  distinct cre_user_id from KQGL_COMELATE_LEAVEEARLY_INFO where subflag='5' ");
		useridList = new ArrayList<String>(Arrays.asList(userids.split(",")));
		String temp5 = useridIntoSql1(useridList,para);
		sb.append(temp5);
		sb.append(")");
		sb.append("union");
		sb.append("(select  distinct cre_user_id from KQGL_OVER_TIME_INFO where subflag='5' ");
		useridList = new ArrayList<String>(Arrays.asList(userids.split(",")));
		String temp6 = useridIntoSql1(useridList,para);
		sb.append(temp6);
		sb.append(")");
		sb.append(") t ");
		/**每个人事假总次数*/
	...
	private String useridIntoSql1(ArrayList<String> useridList,List<Object> para ) {
		StringBuilder sb = new StringBuilder();
		StringBuilder useridSb = new StringBuilder();
		int index = 0;
		String inString = "";
