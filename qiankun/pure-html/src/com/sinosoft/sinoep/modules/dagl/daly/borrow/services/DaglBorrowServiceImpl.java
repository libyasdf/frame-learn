...
public class DaglBorrowServiceImpl implements DaglBorrowService {
    ...
    public DaglBorrow saveForm(DaglBorrow daglBorrow) throws Exception {
        ...
        if (StringUtils.isBlank(daglBorrow.getId())) {
            String titleTime = new SimpleDateFormat("yyyy年MM月dd日").format(new Date());
            ...
            if (StringUtils.isNotBlank(daglBorrow.getFlowType()) && daglBorrow.getFlowType().equals("1")) {
                daglBorrow.setTitle(titleTime + daglBorrow.getBorrowUnitName() + daglBorrow.getBorrowUserName() + "借阅本单位档案申请");
            } else {
                daglBorrow.setTitle(titleTime + daglBorrow.getBorrowUnitName() + daglBorrow.getBorrowUserName() + "借阅非本单位档案申请");
            }
            ...
            if (StringUtils.isNotBlank(daglBorrow.getFlowType()) && daglBorrow.getFlowType().equals("1")) {
                oldDaglBorrow.setTitle(titleTime + daglBorrow.getBorrowUnitName() + daglBorrow.getBorrowUserName() + "借阅本单位档案申请");
            } else {
                oldDaglBorrow.setTitle(titleTime + daglBorrow.getBorrowUnitName() + daglBorrow.getBorrowUserName() + "借阅非本单位档案申请");
            }
        ...
        if (newFileList.size() > 0) {
            ...
            for (DaglFile daglFile : newFileList) {
                if (StringUtils.isBlank(daglFile.getId())) {
                    ...
                    oldDaglFile.setSecurityClass(daglFile.getSecurityClass());
                    oldDaglFile.setPageNum(daglFile.getPageNum());
    ...
    public int deleteDaglBorrow(String id) throws Exception {
        ...
        if(!"".equals(daglBorrow.getShouldReturnDate())){
            //借阅记录中，删除已经办理借出的档案时，先把档案归还；
            returnThisBorrow(id,"");
        }
    ...
    */
    ...
    public PageImpl recordList(Pageable pageable, PageImpl pageImpl, DaglFile daglFile) {
        ...
        // 拼接排序语句
        if (StringUtils.isBlank(pageImpl.getSortName())) {
            //档案借阅（按档案），按照借阅日期排序-王富康-2019-09-23
            querySql.append("  order by t.creTime desc ) ");
        } else {
            querySql.append("  order by t." + pageImpl.getSortName() + " " + pageImpl.getSortOrder() + " ");
        }
     ...
     **/
    @Override
    public JSONObject returnThisBorrow(String id,String lockData) {
        ...
        try {
            ...
            String[] lockDataIds = lockData.split(",");
            String sql = "select * \n" +
            ...
            if (daglBorrow.size() > 0) {
                String thisBorrowSid = "";
                int thisBorrowSidCount = 0;
                //分情况
                //这个和下面的修改子表状态顺序的不能变，否则因为事物原因不起作用
                List<DaglFile> daglFiles = getFileList(daglBorrow.get(0));

                for (int i = 0; i < daglFiles.size(); i++) {
                    //先判断锁定不归还的id是否为本次借阅的子表数据
                    if(null != lockDataIds && Arrays.asList(lockDataIds).contains(daglFiles.get(i).getId())){
                        //如果包含，记录id，并记录是否全部归还
                        thisBorrowSid += "'"+daglFiles.get(i).getId()+"',";
                        thisBorrowSidCount++;
                    }
                    if(null != daglFiles.get(i).getReturnDate()){
                        thisBorrowSidCount++;
                    }
                    //排除已归还,线下借阅不在数据库的的数据
                ...
                //不为空去除逗号
                if(!("".equals(thisBorrowSid))){
                    thisBorrowSid = thisBorrowSid.substring(0,thisBorrowSid.length()-1);
                    sql1 += "   and t.id not in ("+thisBorrowSid+")";
                }
                int num = jdbcTemplate.update(sql1,id);
                //再次查询子表，更新父表状态
                List<DaglFile> daglFilesOfSecond = getFileList(daglBorrow.get(0));
                int notReturnNum = 0;
                for (int i = 0; i < daglFilesOfSecond.size(); i++) {//遍历子表数据是否含有未归还的档案
                    if (null == daglFilesOfSecond.get(i).getReturnDate()) {
                        notReturnNum++;
                    }
                }
                String updateSql = "";
                if (0 == notReturnNum) {
                    ...
                    //未归还；当然这里这种情况不应该存在的，既然归还了一个，至少为部分归还
                }
