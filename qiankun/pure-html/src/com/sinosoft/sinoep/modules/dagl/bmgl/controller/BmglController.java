...
public class BmglController {
	...
	@ResponseBody
	public JSONObject dynamicUpdate(String jsonStr,String tName,String key,String value, String all, String ranking, String fileNumberRule){
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("flag", 0);
		int dynamicAdd = bmglService.dynamicUpdate(jsonStr, tName, key, value, all, ranking, fileNumberRule);
		if (dynamicAdd>0) {
			jsonObject.put("flag", 1);
		}
		return jsonObject;
	...
	@ResponseBody
	public JSONObject dynamicList(String jsonStr,String tName,PageImpl pageImpl,String conditions,String fRecIds,String parameters,String complexParam,String danweihao,String danweiku,Integer all,String ljdanweiAndRenyuan){
		JSONObject json = bmglService.dynamicList(tName, pageImpl, jsonStr, conditions, fRecIds,parameters,complexParam,danweihao,danweiku,all,ljdanweiAndRenyuan);
		return json;
	}
	 ...
	 * 整理编目、档案检索门类树
	 * @author 颜振兴
	 * @param isQ2 是否展示q2档案，true:展示，false:不展示；目前三个预立库都配置的不显示Q2档案
	...
	@ResponseBody
	public JSONObject replace(String tName, String fids, String column,String categoryCode,String fileNumberRule,String replaceInputType, String oldReplaceValue,String oldReplaceCode, String nowReplaceValue, String nowReplaceCode, String all, String ranking, String type) {
		JSONObject json = bmglService.replace(tName, fids, column, categoryCode,fileNumberRule,replaceInputType,oldReplaceValue,oldReplaceCode, nowReplaceValue, nowReplaceCode, all, ranking, type);
		return json;
	...
	@ResponseBody
	public JSONObject findDHGZ(String tName,String categoryCode,int all,int ranking) {
		JSONObject json = new JSONObject();
		...
		if (all==ranking) {
			...
			if(ranking ==3){
				map.put("item_folder_no", bmglService.findZHbycolnum(tName, "item_folder_no"));
			}
			data.put("colsz", map);
			json.put("data", data);
	...
	@ResponseBody
	public JSONObject openOrSetVolume(String tName,String ids,String archiveFlag,String conCol,String conVal, String fid, String all, String ranking) {
		JSONObject json = new JSONObject();
		json.put("flag", "0");
		try {
			int num = bmglService.openVolume(tName,ids,archiveFlag,conCol,conVal, fid, all, ranking);
			json.put("flag", "1");
			json.put("data", num);
		} catch (Exception e) {
			log.error(e.getMessage(),e);
	...
	@ResponseBody
	public JSONObject isRepetition(String column, String relevancyId,String tName,String chushiId, String userid, String recId,String ranking) {
		JSONObject json = new JSONObject();
		int repetition = bmglService.isRepetition(column, relevancyId, tName, chushiId, userid, recId, ranking);
		if(repetition>0) {
			json.put("flag", "0");
		}else {
			json.put("flag", "1");
  ...
  */
	...
	public Map<String,Object> adddd(String id,String tName,String ranking,String all) {
		Map<String,Object> list = bmglService.adddd(id,tName,ranking,all);
		return list;
	}
	...
	@ResponseBody
	public JSONObject queryCountAdd(String danghao,String columnName, String tName, String chushiid, String userid, String lastColumnName,String ranking) {
		JSONObject json = new JSONObject();
		int i = bmglService.queryCountAdd(danghao, columnName,tName,chushiid,userid,lastColumnName, ranking);
		json.put("data", i);
		return json;
	}
	...
	/**
	 * @Author 王富康
	 * @Description //TODO 是否能调入，防止确认调整之后导致档号跟游离的档案重复，0可以调入，大于0不可调入
	 * @Date 2019/6/12 14:50
	 * @Param [column, relevancyId, tName, chushiId, recId, ranking]
	 * @return net.sf.json.JSONObject
	 **/
	@LogAnnotation(value = "query",opName = "是否能调入，防止确认调整之后导致档号跟游离的档案重复，0可以调入，大于0不可调入")
	@RequestMapping(value="canDiaoru",produces = "application/json;charset=utf-8")
	@ResponseBody
	public JSONObject canDiaoru(String count, String tName, String selectFRecid, String categoryCode, String recid, String all,String ranking) {
		JSONObject json = new JSONObject();
		int repetition = bmglService.canDiaoru(count, tName, selectFRecid, categoryCode, recid, all,ranking);
		if(repetition>0) {
			json.put("flag", "0");
		}else {
			json.put("flag", "1");
	 ...
	 **/
	@LogAnnotation(value = "query",opName = "根据门类code和用户查询上一条等级的数据")
	@RequestMapping(value="findUserLastOne")
	@ResponseBody
	public JSONObject findUserLastOne(String tName,String userId, String chushiId, String danghao, String columnName) {
		JSONObject json = new JSONObject();
		List<Map<String, Object>> list = bmglService.findUserLastOne(tName,userId,chushiId,danghao,columnName);
		String replaceAll = com.alibaba.fastjson.JSONObject.toJSONString(list,SerializerFeature.WriteMapNullValue).replaceAll("null", "\"\"");
		if(list.size()>0) {
			json.put("flag", "1");
			json.put("data", replaceAll);
	 ...
	 * @return
	 */
	@LogAnnotation(value = "query",opName = "退回档案")
	@RequestMapping(value="backRecords")
	@ResponseBody
	public JSONObject backRecords(String tName, String ids, String entityStatus, String backReason) {
		JSONObject json = new JSONObject();
		try {
			json = bmglService.backRecords(tName, ids, entityStatus, backReason);
		} catch (Exception e) {
			log.error("退回档案失败："+e.getMessage());
			json.put("flag","0");
			json.put("msg",entityStatus+"异常，请稍后再试！");
		}
		return json;
	}
	 ...
	 * @return net.sf.json.JSONObject
	 **/
	@LogAnnotation(value = "query",opName = "中转库根据录入人和立卷单位和档号查询案卷号")
	@RequestMapping(value="getYearFolderNo")
	@ResponseBody
	public JSONObject getYearFolderNo(String tName, String baseFolderUnit, String lrrId, String folderNo) {
		JSONObject json = new JSONObject();
		try {
			List<Map<String, Object>> list = bmglService.getYearFolderNo(tName, baseFolderUnit, lrrId, folderNo);
			String replaceAll = com.alibaba.fastjson.JSONObject.toJSONString(list,SerializerFeature.WriteMapNullValue).replaceAll("null", "\"\"");
			json.put("flag", "1");
			json.put("data",replaceAll);
		} catch (Exception e) {
	 ...
	 * @return java.util.List<java.util.Map<java.lang.String,java.lang.Object>>
	 **/
	@LogAnnotation(value = "query",opName = "查询父表数据")
	@RequestMapping(value="queryPData")
	@ResponseBody
	public JSONObject queryPData(String tName,String jsonStr, String ranking, String id){
		JSONObject json = new JSONObject();
		try {
			List<Map<String, Object>> findById = bmglService.findById(tName, id);
			String archiveFlag = "";
			...
			if(Integer.parseInt(ranking)==1 || "未组卷".equals(archiveFlag)){
				//层数大于1或者未组卷，无父级，直接返回
				json.put("flag", "1");
			}else{
				List<Map<String, Object>> list = bmglService.queryPData(tName,jsonStr);
				String replaceAll = com.alibaba.fastjson.JSONObject.toJSONString(list,SerializerFeature.WriteMapNullValue).replaceAll("null", "\"\"");
				if(list.size()>0){
					json.put("flag", "1");
				}else{
			...
			json.put("flag", "0");
			log.error("查询父表数据失败："+e.getMessage());;
		}
		return json;
	}
	 ...
	 * @return void
	 **/
	@LogAnnotation(value = "update",opName = "做了数据导入之后执行此方法，可填充立卷单位的code值，即各表的cre_chushi_id")
	@RequestMapping(value="synchronousUpdate")
	public void synchronousUpdate(){
		bmglService.synchronousUpdate();
	}
}
