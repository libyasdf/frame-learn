...
import com.sinosoft.sinoep.modules.video.background.entity.MenHuContentVo;
...
public class VideoContentServiceImpl implements VideoContentService {
    ...
    public PageImpl getContentPageList(Pageable pageable, PageImpl pageImpl, String columnId, String title, String creTime) {
    	...
    	List<Object> param = querySql(sb,columnId,title,creTime);
    	sb.append(")");
    	Integer total=jdbcTemplate.queryForObject(sb.toString(), Integer.class,param.toArray());
    	//查询分页数据
		 String pageSql = pageSql(pageable,columnId,title,creTime,param);
    ...
    private String pageSql(Pageable pageable, String columnId, String title, String creTime,List<Object> param) {
	...
	private List<Object> querySql(StringBuilder sb, String columnId, String title, String creTime) {
    ...
    public PageImpl getFbPageList(Pageable pageable, PageImpl pageImpl, String columnId, String title, String creTime) {
    	//总个数
    	StringBuilder sb = new StringBuilder("select count(1) from (");
    	List<Object> param = fbQuerySql(sb,columnId,title,creTime);
    	sb.append(")");
    	Integer total=jdbcTemplate.queryForObject(sb.toString(), Integer.class,param.toArray());
    	//查询分页数据
		 String pageSql = fbPageSql(pageable,columnId,title,creTime,param);
		List<VideoContent> listData =jdbcTemplate.query(pageSql.toString(), new RowMapper<VideoContent>(){
			@Override
			public VideoContent mapRow(ResultSet result, int arg1) throws SQLException {
				VideoContent content = new VideoContent();
    ...
    private String fbPageSql(Pageable pageable, String columnId, String title, String creTime,List<Object> param) {
    	StringBuilder sb=new StringBuilder("select * from ( select a1.*,ROWNUM RN from (");
	...
	private List<Object> fbQuerySql(StringBuilder sb, String columnId, String title, String creTime) {
		List<Object> param = new ArrayList<Object>();
		sb.append(" select t.id,t.title,t.cre_time creTime,t.fb_time fbTime,t.subflag subflag,t.IS_ZD isZd from video_content t  ");
    ...
    public PageImpl getSpList(Pageable pageable, PageImpl pageImpl, String columnName, String title) {
        ...
        if(StringUtils.isNotBlank(columnName)){
            sql.append(" and l.column_name like ? ");
	...
	public PageImpl getContentByColumn(PageImpl page, String columnCode, VideoContent info) throws Exception {
		...
		if(StringUtils.isNotBlank(columnCode)){
			hql.append(" and c.columnCode  = ? ");
			param.add(columnCode);
        }
		//如果有发布范围，查询发布范围内的数据
		hql.append(" and (t.isFBContent = '0' or (t.isFBContent = '1' and t.id in (" + CommonUtils.solveSqlInjectionOfIn(contentIdStr,param) + ")))");
		if(StringUtils.isNotBlank(info.getTitle())){
        	hql.append(" and t.title like ?");
        	param.add("%" + info.getTitle() + "%");
        }
        if(StringUtils.isNotBlank(info.getCreTime())){
        	hql.append(" and t.creTime = ? ");
        	param.add(info.getCreTime());
        }
	...
	public List<BannerImgVo> getImgsByColumn(String columnCode) throws Exception{
		...
		sBuilder.append(" where c.column_code = ? and i.visible = '1' and i.subflag = '"+VideoConstants.SUBFLAG[2]+"' and i.column_id = c.id and t.table_id = i.id ");
		...
		query = jdbcTemplate.query(sBuilder.toString(), new BeanPropertyRowMapper<>(BannerImgVo.class),columnCode);
		return query;
	}
            ...
            if(StringUtils.isNotBlank(contentIds)){
                ...
                for(String id:contentIdsArray){
                	...
                	if(StringUtils.isNotBlank(isZd)) {
                    	sb.append(" update video_content set IS_ZD= ?,ZD_TIME='" + cruTime + "',ZD_ORDER='" + index + "' where id= ? ");
                    	para.add(isZd);
                    	para.add(id);
                	}else {
                    	sb.append(" update video_content set IS_ZD='',ZD_TIME='',ZD_ORDER='' where id=? ");
                    	para.add(id);
                	}
                	jdbcTemplate.update(sb.toString(),para.toArray());
    ...
    public List<VideoContent> getZdList(String columnId) {
       List<Object> param = new ArrayList<Object>();
        StringBuilder sb = new StringBuilder(" select t.id,t.title,t.IS_ZD isZd from video_content t where t.COLUMN_ID=? ");
	...
	public Map<String, Object> getHomePageContent(PageImpl page,String columnId,String flag,String columnName) throws Exception {
		...
		/*	sb.append(" select t.id,t.TITLE,t1.save_path imgPath from ( ");
			...
			//总个数
			StringBuilder sb = new StringBuilder("select count(1) from (");
			List<Object> param = querySql(sb,columnId,nowTime,contentIds,contentIdStr,columnName,flag);
			sb.append(")");
			Integer total=jdbcTemplate.queryForObject(sb.toString(), Integer.class,param.toArray());
			//查询分页数据
			String pageSql = pageSql(pageable,columnId,nowTime,contentIds,contentIdStr,columnName,flag,param);
	...
	private List<Object> querySql(StringBuilder sb, String columnId,String nowTime,StringBuilder contentIds,String contentIdStr,String columnName,String flag) {
		List<Object> param = new ArrayList<Object>();
		...
		sb.append(" and (( t.SHOW_START_TIME <= ? and t.SHOW_END_TIME >= ? )");
		param.add(nowTime);
		param.add(nowTime);
	...
	private String pageSql(Pageable pageable, String columnId,String nowTime,StringBuilder contentIds,String contentIdStr,String columnName,String flag,List<Object> param) {
		StringBuilder sb=new StringBuilder("select * from ( select a1.*,ROWNUM RN from (");
	...
	public List<VideoContent> getContentByColumnId(String columnId, String title) throws Exception {
		...
		contentIdStr = contentIds.substring(0, contentIds.length() - 1);
		StringBuilder sb = new StringBuilder(" select t.*,t1.info_id uuid from (select t.id,t.title from video_content t where t.column_id= ? ");
		param.add(columnId);
		if(StringUtils.isNotBlank(title)){
			sb.append(" and t.title like ? ");
			param.add("%" + title + "%");
	...
	public void updateImageId(String id, String imageid) {
		...
		String updateSql = "update video_content set image_id= ? where id= ? ";
		param.add(imageid);
		param.add(id);
		jdbcTemplate.update(updateSql,param.toArray());
	...
	public void saveFromBySql(VideoContent entity) {
		...
		sb.append("set SUBFLAG= ? ");
		sb.append(", TITLE= ? ");
		sb.append(", SUBTITLE= ? ");
		sb.append(", SOURCE= ? ");
		sb.append(", AUTHOR= ? ");
		sb.append(", SHOW_START_TIME= ? ");
		sb.append(", SHOW_END_TIME= ? ");
		sb.append(", IS_FB_CONTENT= ? ");
		sb.append("  where id= ? ");
		
		param.add(VideoConstants.SUBFLAG[1]);
		param.add(entity.getTitle());
		param.add(entity.getSubtitle());
		param.add(entity.getSource());
		param.add(entity.getAuthor());
		param.add(entity.getShowStartTime());
		param.add(entity.getShowStartTime());
		param.add(entity.getIsFBContent());
     ...
     */
    @Override
    public VideoContent updateFlag(String id, String flag) throws Exception {
        VideoContent videoContent = dao.findOne(id);
        videoContent.setSubflag(flag);
        if (flag.equals(SysNoticeConstants.FLAG[3])) {
            videoContent.setFbTime(DateUtil.getDateText(new Date(), "yyyy-MM-dd"));
        }
        videoContent = dao.save(videoContent);
        return videoContent;
    }
	...
	public PageImpl getVideoContentNoticeList(PageImpl page) throws Exception{
		...
		StringBuilder sql = new StringBuilder();
		sql.append("select * from (select a.*,rownum rn from (select vcol.column_name as columeName,vcon.id as contentId,vcon.title as contentTitle,vcon.fb_time as contentFbTime,vcon.IS_ZD as isTop from video_content vcon,video_column vcol where vcon.column_id=vcol.id and vcon.visible = '1' and vcon.subflag = '5' and (vcon.is_fb_content = '0' ");
		if(contentIds.length()>0){
			sql.append(" or vcon.is_fb_content = '1' and vcon.id in (" + contentIdStr + ")");
		}
		sql.append(")");
		sql.append(" and ((vcon.show_start_time <= '" + nowTime + "' and vcon.show_end_time >= '" + nowTime + "') or vcon.show_start_time is null)  order by vcon.is_zd asc, vcon.zd_time desc, vcon.zd_order,vcon.fb_time desc ");
		sql.append(" )a where rownum <=" + (pageSize * pageNumber) + ") where rn>=" + ((pageNumber-1) * pageSize+1));
		List<Map<String,Object>> contentList = jdbcTemplate.queryForList(sql.toString());
		List<MenHuContentVo> list = new ArrayList<MenHuContentVo>();
		for(Map<String,Object> map : contentList){
			MenHuContentVo vo = new MenHuContentVo();
			if(null != map.get("columeName")){
				vo.setColumeName(String.valueOf(map.get("columeName")));
			}
			if(null != map.get("contentId")){
				vo.setContentId(String.valueOf(map.get("contentId")));
				//查询内容下第一条数据
				String sel = "select * from (select a.*,rownum rn from (select t.id,t.file_name,t.file_id,t.cre_time from video_video_and_pdf t where t.info_id ='"+String.valueOf(map.get("contentId"))+"' and t.visible='1' order by t.cre_time )a ) where rn=1";
				try {
					Map<String,Object> vMap = jdbcTemplate.queryForMap(sel);
					//依次设置第一条视频的业务id，视频名称（带后缀），视频在音视频服务器中的uuid
					if(null != vMap.get("id")){
						vo.setFirstVideoId(String.valueOf(vMap.get("id")));
					}
					if(null != vMap.get("file_name")){
						vo.setFirstVideoName(String.valueOf(vMap.get("file_name")));
					}
					if(null != vMap.get("file_id")){
						vo.setFirstVideoUuid(String.valueOf(vMap.get("file_id")));
					}
				} catch (Exception e) {
					System.out.println("查询内容第一条视频异常sql："+sel);
					System.out.println("查询内容第一条视频异常原因："+e.getMessage());
				}
			}
			if(null != map.get("contentTitle")){
				vo.setContentTitle(String.valueOf(map.get("contentTitle")));
			}
			if(null != map.get("contentFbTime")){
				vo.setTime(String.valueOf(map.get("contentFbTime")));
			}
            if(null != map.get("isTop")){
                vo.setIsTop(String.valueOf(map.get("isTop")));
            }
			list.add(vo);
		}
		page.getData().setRows(list);
		page.getData().setTotal(list.size());
		return page;
	}
}
