...
public class MeetingNoticeController {
	...
	@LogAnnotation(opType = OpType.UPDATE,opName = "根据会议申请id查询通知内容")
	@ResponseBody
	@RequestMapping("edit1")
	public JSONObject edit1(String applyId) {
		JSONObject json = new JSONObject();
		MeetingNoticeInfo meetingNotice = null;
		try {
			meetingNotice = meetingNoticeService.getByApplyId(applyId);
			json.put("flag", "1");
			json.put("data", meetingNotice);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public JSONObject getList(PageImpl pageImpl, String timeRange, String meeetingTitle,String meetingType,String hostDeptId,String meetingRoomId,String flg,String ifInputByhand) {
		...
		JSONObject json =  meetingNoticeService.getPageListMeetingAllInfo(pageable, pageImpl, startDate, endDate,meeetingTitle,meetingType,hostDeptId,meetingRoomId,flg,ifInputByhand);
