$(function(){
    ...
    $("#positionDeptId").val(getcookie("juId"));
 /*   var rolesNos = getcookie("rolesNo");
    if(rolesNos.indexOf("C616") != -1 || rolesNos.indexOf("D616") != -1 || rolesNos.indexOf("C612") != -1 || rolesNos.indexOf("D612") != -1){//部门级管理员

    }else{
        $("#positionDeptId").val(getcookie("orgid"));
	}*/
	...
	httpRequest("get","/infoFbZwqx/getAuthority",datas,function (json){
		if(json.flag == "1"){
			var data = json.data;
			//回显部门权限
            ...
            if(!$.isEmptyObject(userGroupList)){
                var sysUserIds = "";
                var sysUserNames = "";
                var sysGrupIds ="";
                var sysGrupNames ="";
                for (var i = 0;i < userGroupList.length; i++){
                    sysUserIds += userGroupList[i].sysUserId + ",";
                    sysUserNames += userGroupList[i].sysUserName + ",";
                    sysGrupIds += userGroupList[i].sysGrupId+",";
                    sysGrupNames += userGroupList[i].sysGrupName+",";
                }
                sysUserIds = sysUserIds.substring(0,sysUserIds.length-1);
                sysUserNames = sysUserNames.substring(0,sysUserNames.length-1);
                sysGrupIds = sysGrupIds.substring(0,sysGrupIds.length-1);
                sysGrupNames = sysGrupNames.substring(0,sysGrupNames.length-1);
                $("#sysUserId").val(sysUserIds);
                $("#sysUserName").val(sysUserNames);
                $("#sysGrupId").val(sysGrupIds);
                $("#sysGrupName").val(sysGrupNames);
            }
			//回显职务权限
...
function checkForm(){
    ...
    //校验人群权限
    var userGroupFlag = false;
    var sysUserId = $("#sysUserId").val();
    var sysUserName = $("#sysUserName").val();
    if(sysUserId && sysUserName){
        userGroupFlag = true;
    }
    console.log(userGroupFlag);
	//校验职务权限
	...
	if(!deptFlag && !groupFlag  && !userGroupFlag && !positionFlag && (!tableFlag || tableFlag==null)){
		msg = msg ? msg : "请至少选择一种权限范围";
	}
	//部门、职务、群组只要有一个不为false  并且  表格为true或者为空
	if((deptFlag || positionFlag || groupFlag || userGroupFlag) && (tableFlag==null ||tableFlag)){
		res = true;
	}
	//部门、职务、群组均为空，并且  表格有填写完整的一行
	if(!deptFlag && !groupFlag && !userGroupFlag && !positionFlag && tableFlag != null && tableFlag){
		res = true;
	}
	if(!res){
...
function getFormData(){
    ...
    //组装人群权限集合
    var sysGrupIds = $("#sysUserId").val();
    if(sysGrupIds){
        var groups = sysGrupIds.split(",");
        var sysUserNames = $("#sysUserName").val().split(",");
        var sysGrupnames = $("#sysGrupName").val().split(",");
        var sysGrupIds = $("#sysGrupId").val().split(",");
        var sysDeptIds = $("#sysDeptId").val().split(",");
        $(groups).each(function(i,n){
            var group = {
                "contentId":$("#contentId").val(),
                "sysUserId":n,
                "sysUserName":sysUserNames[i],
				"sysGrupName":sysGrupnames[i],
				"sysGrupId":sysGrupIds[i],
				"sysDeptId":sysDeptIds[i]
            };
            userGroupList.push(group);
        })
    }
    //组装职务权限集合
...
function openTree(){
   ...
   /* if(rolesNo.indexOf("C616") != -1 || rolesNo.indexOf("D616") != -1 ){//部门级管理员
        juId = getcookie("juId");
    }*/
	openSelectZtree({'id':'deptId','name':'deptName','type':'2','deptId':juId,'asyn':true,'level':level,'url':'/system/component/tree/deptTree','select':true});
}
...
function openDeptTree(subTableLineNum){
  ...
  /*  var rolesNo = getcookie("rolesNo");*/
	...
	openSelectZtree({'id':'deptId'+subTableLineNum+'','name':'deptName'+subTableLineNum+'','deptId':juId,'select':true,'asyn':true,'isMulti':'1','check':'','cancle':'','type':'2','level':level,'url':'/system/component/tree/deptTree'});
}
 ...
 * 打开人员树
 */
function openNoticeZtree(){
    openNoticeSelectZtree({'id':'sysUserId','name':'sysUserName','parentId':'sysDeptId','groupid':'sysGrupId','groupname':'sysGrupName','check':'s','cancle':'s','url':'/system/noticeGrup/getGroupTree','subId':Config.sysId})
}