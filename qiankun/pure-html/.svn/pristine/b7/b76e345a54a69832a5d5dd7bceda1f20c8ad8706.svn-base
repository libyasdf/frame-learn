var fileTypeId = "51b9d806df394ced85a573908b916fc7";
var workFlowId = "51b9d806df394ced85a573908b916fc7";
	...
	if (bootstrapValidator.isValid()) {
		if (isFbfw == "1") {//存在发布范围
			...
			MyLayer.layerOpenUrl({
                url: "/modules/info/authority/authorityInfo.html?contentId=" + contentId+"&columnId="+columnId+"&typeCode=03",
				title : "选择发布范围"
			});
		}
	}
...
function  commitFlowDept() {
    debugger;
    if($("#videoTitle").val() ==""){
        $("#videoTitle").val(getCurrentDate("yyyy年MM月dd日")+getcookie("usernm")+"视频发布审核");
    }
    var bootstrapValidator = $("#form").data('bootstrapValidator');
    bootstrapValidator.validate();
    if(bootstrapValidator.isValid()){
        ...
        if(flag!=""){
            ...
            submitToFlow(oper, this, $("#videoTitle").val(), idea, $("#id").val(), "attr", "attr1", $("#workitemid").val(), $("#workflowid").val());
...
function updateBusiData(subflag) {
    ...
    $.ajax({
        type: "POST",
        url: basePath + "/video/content/updateFlag?resId=" + resId,
        data: {
            id: $("#id").val(),
            subflag: subflag
        },
