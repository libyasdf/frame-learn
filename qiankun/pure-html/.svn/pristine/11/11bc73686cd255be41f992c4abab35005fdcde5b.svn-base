$(function(){
    ...
    $("#typeCode").val(theRequest.typeCode);
    ...
    httpRequest("get","/infoFbZwqx/getGenarals",datas,function (json){
    	if(json.flag == "1"){
    		if(json.type =="1"){
    			$("#generals1").attr("checked","checked");
			}
		}
...
function saveData(){
	debugger;
	var genarals = $("input[name=generals]:checked").val();
	if(genarals == 1){
        var datas = {
            contentId: $("#contentId").val(),
            genarals:$("input[name=generals]:checked").val()
        }
        httpRequest("get","/infoFbZwqx/saveGenarals",datas,function (json){
            if(json.flag == "1"){
...
function checkForm(){
    ...
    var sysGrupName = $("#sysGrupName").val();
    if(sysUserId && sysGrupName){
        groupFlag = true;
    }
...
function openTree(){
   ...
   /* if(rolesNo.indexOf("C616") != -1 || rolesNo.indexOf("D616") != -1 ){//部门级管理员
	...
	openSelectZtree({'id':'deptId','height':'90%','name':'deptName','type':'2','deptId':juId,'asyn':true,'level':level,'url':'/system/component/tree/deptTree','select':true});
}
...
function openDeptTree(subTableLineNum){
  ...
  /*  var rolesNo = getcookie("rolesNo");*/
	...
	openSelectZtree({'id':'deptId'+subTableLineNum+'','height':'90%','name':'deptName'+subTableLineNum+'','deptId':juId,'select':true,'asyn':true,'isMulti':'1','check':'','cancle':'','type':'2','level':level,'url':'/system/component/tree/deptTree'});
}
...
function openGenaralsZtree(){
    openGenaralsSelectZtree({'id':'sysUserId','name':'sysUserName','parentId':'sysDeptId','groupid':'sysGrupId','groupname':'sysGrupName','typeCode':$("#typeCode").val(),'check':'s','cancle':'s','url':'/system/noticeGrup/getGenaralsGroupTree','subId':Config.sysId})
}
