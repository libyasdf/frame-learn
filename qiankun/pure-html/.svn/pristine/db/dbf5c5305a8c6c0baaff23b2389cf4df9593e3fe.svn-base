...
public class DutyTableServiceImpl implements DutyTableService{
     ...
     **/
    @Override
    public List<DutyTable> getList(String timeRange,DutyTable dut){
        ...
        if (StringUtils.isNotBlank(timeRange)) {
            ...
            sql +=" and t.watch_date >= ? and t.watch_date <= ? ";
        }

        if (StringUtils.isNotBlank(dut.getWatchName())) {
            sql +=" and t.watch_name like ? ";
            paraList.add("%"+dut.getWatchName()+"%");
        }
        sql +=" order by t.watch_Date Asc,decode(t.classes,'早',1,'中',2,'晚',3,'昼',4,'夜',5) ";
        log.info(sql);
        List<DutyTable> list = jdbcTemplate.query(sql,new BeanPropertyRowMapper<>(DutyTable.class),paraList.toArray());
        return list;
    }
     ...
     **/
    @Override
    public PageImpl getListOfZongDutySearch(Pageable pageable, PageImpl pageImpl, DutyTable dut) {

        StringBuilder querySql = new StringBuilder();
        List<Object> para = new ArrayList<>();
        querySql.append(" from DutyTable t where t.visible = "+CommonConstants.VISIBLE[1]+" and  t.reportId is not null ");
        ...
        if(StringUtils.isNotBlank(dut.getCreJuId())){
            querySql.append(" and t.creJuId in("+CommonUtils.solveSqlInjectionOfIn(dut.getCreJuId().replaceAll("'", ""),para)+")");
        }
     ...
     **/
    @Override
    public PageImpl getListOfDutySearch(Pageable pageable, PageImpl pageImpl, String year_month,DutyTable dut) {

        String sql ="select * from ( select distinct t.cre_ju_name, t.cre_chushi_name, t.private_phone, t.common_phone\n" +
                        "  from zbgl_duty_detail t\n" +
                        " where t.visible = "+CommonConstants.VISIBLE[1]+"\n" +
                        "   and t.report_id is not null";

        if (StringUtils.isNotBlank(year_month)) {
            //选择了查询项
            String[] temp = year_month.split("-");
            ...
            sql += " and t.watch_date >= '"+firstDay+"' and t.watch_date <= '"+lastDay+"'";

            //根据月份查询出基础配置的单位
            List<Config> cf = configService.queryOne(year_month);
            String reportIds ="";
            if(cf.size()>0){
                //得到基础配置的单位的上报表的数据
                List<ShangBao> sb = shangBaoService.queryOneByMonthAndUnitId(year_month, cf.get(0).getUnitId());
            ...
            if(reportIds.length()>0){
                int n = reportIds.length();
                reportIds = reportIds.substring(0,reportIds.length()-1);
                sql += " and t.report_id in ( "+reportIds+" )";
            }else{
                sql += " and t.report_id in ('')";
            }
        ...
        if(StringUtils.isNotBlank(dut.getCreJuId())){
            sql += " and t.cre_ju_id in("+CommonUtils.solveSqlInjectionOfIn(dut.getCreJuId().replaceAll("'", ""), list)+")";
        }

        //处室
        if (StringUtils.isNotBlank(dut.getCreChushiId())) {
            sql += " and t.cre_chushi_id = ? ";
            list.add(dut.getCreChushiId());
        }

        sql += " ) order by cre_ju_name desc, cre_chushi_name desc";
        Page<DutyTable> page = dutyTableDao.nativeQuery(sql, pageable,list.toArray());
        return new PageImpl((int)page.getTotalElements(),page.getContent());
    }
     ...
     **/
    @Override
    public List<DutyTable> getListOfSearch(DutyTable dut){
        String sql = " select * from zbgl_duty_detail t where t.visible = "+CommonConstants.VISIBLE[1]+" and  t.report_id is not null ";
        ...
        List<Object> list = new ArrayList<>();
        if (StringUtils.isNotBlank(dut.getWatchDate())) {
            sql += " and t.watch_date =  ? ";
            list.add(dut.getWatchDate());
        ...
        List<DutyTable> dutyTableList= jdbcTemplate.query(sql,new BeanPropertyRowMapper<>(DutyTable.class),list.toArray());
        return dutyTableList;
    }
    ...
    public JSONObject hasReportUserByDeptId(String deptIds,String deptNames) throws Exception {
        ...
        try {
            ...
            String sql = "select sfd.deptname,sfd.deptid,duser.userid"
                    ...
                    +"	on d.id = f.u_dept_id"
                    +"	where f.roleid in (select t.roleid from sys_flow_role t where t.role_no in('"+ZBGLCommonConstants.RC_ROLE_NO[0]+"','"+ZBGLCommonConstants.RC_ROLE_NO[1]+"'))and d.status = '"+ZBGLCommonConstants.STATE[1]+"') t"
