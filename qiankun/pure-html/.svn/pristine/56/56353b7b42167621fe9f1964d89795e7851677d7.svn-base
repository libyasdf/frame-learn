...
public class BmglController {
	...
	public JSONObject dynamicAdd(String jsonStr,String tName, String fids){
		...
		com.alibaba.fastjson.JSONObject dynamicAdd = bmglService.dynamicAdd(jsonStr, tName,fids);
	...
	@ResponseBody
	public JSONObject isRepetition(String column, String relevancyId,String tName,String chushiId,String recId) {
		JSONObject json = new JSONObject();
		
		int repetition = bmglService.isRepetition(column, relevancyId, tName, chushiId,recId);
		if(repetition>0) {
			json.put("flag", "0");
		}else {
			json.put("flag", "1");
  ...
  */
	...
	@ResponseBody
	public JSONObject dynamicPlAdd(String jsonStr,String tName,String addCount, String categoryCode, String fids){
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("flag", 0);
		int dynamicAdd = bmglService.dynamicPlAdd(jsonStr, tName, addCount, categoryCode,fids);
		if (dynamicAdd>0) {
			jsonObject.put("flag", 1);
		}
		return jsonObject;
	...
	public PageImpl getFirstLevelData(PageImpl pageImpl,String tableName,String liJuanDanWei,String luRuRen,String mainTitle,String dangHao) {
		Pageable pageable = new PageRequest(pageImpl.getPageNumber() - 1, pageImpl.getPageSize());
		pageImpl= bmglService.getFirstLevel(pageImpl, pageable, tableName, liJuanDanWei, luRuRen, mainTitle, dangHao);
		return pageImpl;
	}
	...
	@ResponseBody
	public JSONObject queryCountAdd(String danghao,String columnName, String tName, String chushiid,String lastColumnName) {
		JSONObject json = new JSONObject();
		int i = bmglService.queryCountAdd(danghao, columnName,tName,chushiid,lastColumnName);
		json.put("data", i);
		return json;
	}
}
