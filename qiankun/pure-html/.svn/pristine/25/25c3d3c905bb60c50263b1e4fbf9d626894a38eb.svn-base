...
//		directory.setSurveyDetailId(detail.getId());
	...
	public PageImpl getPageListDraft(Pageable pageable, PageImpl pageImpl, String wenjuanType,Integer surveyState,String startDate,String endDate,String surveyName) {
		...
		List<SurveyDirectory> listData =jdbcTemple.query(pageSql.toString(), new RowMapper<SurveyDirectory>(){
			...
			public SurveyDirectory mapRow(ResultSet rs, int arg1) throws SQLException {
				...
				surveyDirectory.setDeptState(rs.getString("DEPT_STATE"));
				surveyDirectory.setSubflag(rs.getString("SUBFLAG"));
				return surveyDirectory;
			}
				
		},params.toArray());
		for (SurveyDirectory surveyDirectory : listData) {
			if(!surveyDirectory.getSubflag().equals("0")){
				if(surveyDirectory.getSubflag().equals("1")){
					surveyDirectory.setCz("tongji");
				}else{
					surveyDirectory.setCz(CommonConstants.OPTION_DELETE+","+"tongji");
				}
			}else{
	 ...
	 */
	@Override
	public PageImpl getGenalsListDraft(Pageable pageable, PageImpl pageImpl, String wenjuanType,Integer surveyState,String startDate,String endDate,String surveyName) {
		//总个数
		StringBuilder sb = new StringBuilder("select count(1) from (");
		List<Object> params = queryGenalsSql(sb,wenjuanType,surveyState,startDate,endDate,surveyName);
		sb.append(")");
		Integer total=jdbcTemple.queryForObject(sb.toString(), Integer.class,params.toArray());
		//查询分页数据
		String pageSql = pageGenalsSql(pageable,wenjuanType,surveyState,startDate,endDate,surveyName,params);
		List<SurveyDirectory> listData =jdbcTemple.query(pageSql.toString(), new RowMapper<SurveyDirectory>(){
			@Override
			public SurveyDirectory mapRow(ResultSet rs, int arg1) throws SQLException {
				SurveyDirectory surveyDirectory = new SurveyDirectory();
	...
	private String pageGenalsSql(Pageable pageable, String wenjuanType, Integer surveyState, String startDate, String endDate,
						   String surveyNam,List<Object> params) {

		StringBuilder sb=new StringBuilder("select * from ( select a1.*,ROWNUM RN from (");
		queryGenalsSql(sb,wenjuanType,surveyState,startDate,endDate,surveyNam);
		sb.append("  ) a1 where ROWNUM <= ? ) where RN >=  ?");
		params.add(pageable.getOffset()+pageable.getPageSize());
		params.add(pageable.getOffset()+1);
		return sb.toString();
	...
	private List<Object>   querySql(StringBuilder sb, String wenjuanType, Integer surveyState, String startDate, String endDate,
		...
		sb.append("select t.* from t_survey_directory t where t.visibility='1'  and t.DEPT_STATE ='1'");
	...
	private List<Object>   queryGenalsSql(StringBuilder sb, String wenjuanType, Integer surveyState, String startDate, String endDate,
									String surveyName) {
		List<Object> params = new ArrayList<Object>();
		sb.append("select t.* from t_survey_directory t where t.visibility='1'  and t.DEPT_STATE ='0'");
		if(StringUtils.isNotBlank(wenjuanType)){
			sb.append("  and WENJUAN_TYPE= ? ");
			params.add(wenjuanType);
		}
	...
	public PageImpl getSurveyList(PageImpl pageImpl,String surveyName, String type, String startDate, String endDate) throws Exception {
		...
		if (authList != null && !authList.isEmpty()) {
			...
			criterions.add(Restrictions.eq("subflag","5"));	//发布状态
			if(StringUtils.isNotBlank(startDate) && StringUtils.isNotBlank(endDate)){
