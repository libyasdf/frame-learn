...
public class DataDictionarysDaoImpl implements DataDictionarysRepository {
    ...
    public List<DataDictionarys> getAllChildTypes(String id){
        String sql = "select * from xxkh_tree t where t.visible = '"+ CommonConstants.VISIBLE[1]+"' and t.type = '0'"
               + " start with t.id = '" + id + "' connect by prior id = pid order by t.cre_time";
        return jdbcTemplate.query(sql,new BeanPropertyRowMapper<>(DataDictionarys.class));
    }
     ...
     */
     ...
     public List<DataDictionarys> getLearningTimeTreeAllChildTypes(String id){
         ...
         String sql = "select * from xxkh_type_info_view t "
                + " start with t.id = '" + id + "' connect by prior id = pid order siblings by type desc,cre_time asc";
         return jdbcTemplate.query(sql,new BeanPropertyRowMapper<>(DataDictionarys.class));
     }
	...
	public int findPid(String id) {
		String sql = "select count(1) from xxkh_tree d where pid='"+id+"'";
		return  jdbcTemplate.queryForObject(sql, Integer.class);
	}

	@Override
	public List<DataDictionarys> getDicByType(String treeType, String type) {
		String sql="select t.* from xxkh_tree t where t.tree_type like '%"+treeType+"%' and t.type = '"+type+"' and  t.pId is null  and t.visible = '1'   order by t.sort";
		return jdbcTemplate.query(sql,new BeanPropertyRowMapper<>(DataDictionarys.class));
	}
	...
	public List<DataDictionarys> getAllpTypes(String id) {
        String sql = "select * from xxkh_tree t where t.visible = '"+ CommonConstants.VISIBLE[1]+"' and t.type = '0'"
                + " start with t.id = '" + id + "' connect by prior pid = id order by t.cre_time asc";
         return jdbcTemplate.query(sql,new BeanPropertyRowMapper<>(DataDictionarys.class));
	}

	@Override
	public List<DataDictionarys> getBuMenTree(String cruJuId, String treeType) {
		String sql="select t.* from xxkh_tree t where t.tree_type like '%"+treeType+"%' and t.type = '0' and  t.pId is null  and t.visible = '1' and  t.tree_type like '%"+cruJuId+"%' order by t.sort";
		return jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(DataDictionarys.class));
	}
	...
	public String getTreeTypeById(String id) {
		String treeType = "";
		String sql = "select t.tree_type from xxkh_tree t where t.id='"+id+"'";
	...
	public int getCountByIdAndTreeType(String id, String tableName) {
		int dataNum = 0;
		String sql = "select count(t.id) as dataNum from "+tableName+" t where t.node_id='"+id+"' and t.visible='1'";
		List<Map<String,Object>> list = new ArrayList<Map<String,Object>>();
		list = jdbcTemplate.queryForList(sql);
		if(list.size() > 0){
			Map<String,Object> map = list.get(0);
