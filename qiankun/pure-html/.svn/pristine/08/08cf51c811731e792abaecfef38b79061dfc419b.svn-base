...
public class BmglServiceImpl implements BmglService {
	 ...
	 **/
	@Override
	public List<Map<String, Object>> findUserLastOne(String tName, String userId){
		...
		String sql="select s.* from (select t.* from "+tableName+" t order by t.cre_time desc,t.recid desc) s where s.cre_user_id = ? and rownum = 1";
		/*if(tName.indexOf("_dak")==-1){
			//单位库排除已归档的数据
			sql+=" and s.archive_entity_strtus != '"+DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[2]+"'";
