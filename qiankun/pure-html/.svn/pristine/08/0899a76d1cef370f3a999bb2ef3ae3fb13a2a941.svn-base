...
@Repository
public class TQuestionDaoImpl extends BaseDaoImpl<TQuestion, String> implements TQuestionDao {
	
	public List<TQuestion> findByBelongTag(String qubankId,String tag){
	 ...
	 */
	@Override
	public void save(TQuestion entity) {
		Session session=getSession();
		saveQuestion(entity, session);
	}
	private void saveQuestion(TQuestion entity, Session session) {
		boolean isnew=false;
	...
	private void saveQuChenScore(TQuestion entity, Session session, boolean isnew) {
		//保存相关选项信息
	 ...
	 * @param session
	 */
	private void saveQuCompChenRadio(TQuestion entity, Session session,boolean isnew) {
		//保存相关选项信息
		saveRows(entity,session);
		saveColumns(entity,session);
	 ...
	 * @param session
	 */
	private void saveQuChenFbk(TQuestion entity, Session session,boolean isnew) {
		//保存相关选项信息
		saveRows(entity,session);
		saveColumns(entity,session);
	}
	 ...
	 * @param session
	 */
	private void saveQuChenCheckbox(TQuestion entity, Session session,boolean isnew) {
		//保存相关选项信息
		saveRows(entity,session);
		saveColumns(entity,session);		
	}
	 ...
	 * @param session
	 */
	private void saveQuChenRadio(TQuestion entity, Session session,boolean isnew) {
		//保存相关选项信息
		saveRows(entity,session);
		saveColumns(entity,session);		
	}
	 ...
	 * @param session
	 */
	private void deleteRows(TQuestion entity ,Session session){
		String delHql="delete from QuChenRow where quId=?";
		session.createQuery(delHql).setString(0, entity.getId()).executeUpdate();
	}
	private void deleteColumns(TQuestion entity ,Session session){
		String delHql="delete from QuChenColumn where quId=?";
		session.createQuery(delHql).setString(0, entity.getId()).executeUpdate();
	}
	private void deleteOptions(TQuestion entity ,Session session){
		String delHql="delete from QuChenOption where quId=?";
		session.createQuery(delHql).setString(0, entity.getId()).executeUpdate();
	}
	
	private void saveOptions(TQuestion entity, Session session) {
	...
	private void saveColumns(TQuestion entity, Session session) {
	...
	private void saveRows(TQuestion entity, Session session) {
	 ...
	 * @param session
	 */
	private void saveQuScore(TQuestion entity, Session session) {
	 ...
	 * @param session
	 */
	private void saveQuOrderby(TQuestion entity, Session session) {
	 ...
	 * @param session
	 */
	private void saveQuBig(TQuestion entity, Session session) {
		List<TQuestion> questions=entity.getQuestions();
		session.save(entity);
	 ...
	 * @param session
	 */
	private void saveRadio(TQuestion entity,Session session){
	 ...
	 * @param session
	 */
	private void saveCheckbox(TQuestion entity,Session session){
	 ...
	 * @param session
	 */
	private void saveMultiFillblank(TQuestion entity,Session session){
