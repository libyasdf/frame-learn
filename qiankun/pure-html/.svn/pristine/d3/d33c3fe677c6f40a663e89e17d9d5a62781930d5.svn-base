...
public class QuestionServiceImpl implements QuestionService {
	...
	public PageImpl getPageList(Pageable pageable, PageImpl pageImpl, Question questionInfo) {
		...
		if (StringUtils.isNotBlank(questionInfo.getDescribe())) {
			querySql.append(" and t.describe like '%"+questionInfo.getDescribe()+"%'");
	...
	public int delete(Question questionInfo) {
		...
		if(StringUtils.isNotBlank(questionInfo.getId())){
			try {
				//逻辑删除试题
				String delQuestion = "update Question q set q.visible='"+CommonConstants.VISIBLE[0]+"' where q.id='"+questionInfo.getId()+"'";
				n = questionDao.update(delQuestion);
				//逻辑删除试题对应的选项
				String delOption = "update Option o set o.visible='"+CommonConstants.VISIBLE[0]+"' where o.questionId='"+questionInfo.getId()+"'";
				optionDao.update(delOption);
			} catch (Exception e) {
	...
	public int commitQuestionById(String qId) {
		int n = 0;
		try {
			String commitSql = "update Question q set q.state='"+XXKHCommonConstants.RECORD_STATE[1]+"' where q.id='"+qId+"'";
			n = questionDao.update(commitSql);
		} catch (Exception e) {
			log.info(e.getMessage());
			log.info("提交试题发生异常");
		}
	...
	public int commitSelected(String ids) {
		int n = 0;
		try {
			String commitSql = "update Question q set q.state='"+XXKHCommonConstants.RECORD_STATE[1]+"' where q.id in("+CommonUtils.commomSplit(ids)+")";
