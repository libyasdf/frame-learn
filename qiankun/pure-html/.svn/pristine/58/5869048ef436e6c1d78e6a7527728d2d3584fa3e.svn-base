...
public class TestManageServiceImpl implements TestManageService {
	...
	private static  final String SQL = "select distinct t.userid,t.usernamefull as username,t.user_sex,t.phone,t.status,t.position_real_level ,p1.occupation_name as position_name_full,t.position_level ,p2.occupation_name as position_level_name,"
					...
					+ "	left join sys_user_position p1 on (t.position_real_level = p1.occupation_level and p1.occupation_type = "+XXKHCommonConstants.OCCUPATION_TYPE[0]+")	"
					+ "	left join sys_user_position p2 on (t.position_level = p2.occupation_level and p2.occupation_type = "+XXKHCommonConstants.OCCUPATION_TYPE[1]+")	"
	...
	public JSONObject getUserPositon() {
		...
		json.put("flag", "0");
		try {
			String sql = "select * from sys_user_position t where t.occupation_type = "+XXKHCommonConstants.OCCUPATION_TYPE[0]+" and t.order_no >= "+XXKHCommonConstants.OCCUPATION_LEVEL[2]+"order by t.order_no asc";
			String sql2 = "select * from sys_user_position t where t.occupation_type = "+XXKHCommonConstants.OCCUPATION_TYPE[1]+" and t.order_no >= "+XXKHCommonConstants.OCCUPATION_LEVEL[2]+"order by t.order_no asc";
	...
	public List<XxkhTestPaper> getPapersByTestId(String id) {
		...
		String sql = "select t.id,t.test_id,t.paper_id,t.sort,d.name from xxkh_test_paper t left join xxkh_paper_info d  on t.test_id = d.id  where t.visible = '"+CommonConstants.VISIBLE[1]+"' ";
	...
	public JSONObject getTestPersonCount(String deptId, String position, String positionLevel) {
		...
		try {
			...
			sql += "select distinct t.userid,t.usernamefull as username,t.user_sex,t.phone,t.status,t.position_real_level ,p1.occupation_name as position_name_full,t.position_level ,p2.occupation_name as position_level_name,	"
					+ "	d.deptid as userDeptId,fd.deptname as userDeptName,fd.chuid as userChushiId,fd.chuname as userChushiName,fd.juid as userJuId,fd.juname as userJuName "
					+ "	from sys_flow_user t "
					+ "	left join sys_user_position p1 on (t.position_real_level = p1.occupation_level and p1.occupation_type = "+XXKHCommonConstants.OCCUPATION_TYPE[0]+")	"
					+ "	left join sys_user_position p2 on (t.position_level = p2.occupation_level and p2.occupation_type = "+XXKHCommonConstants.OCCUPATION_TYPE[1]+")"
	...
	public List<SysTestUserVo> getTestPersonInfo(String deptId, String position, String positionLevel) {
		...
		try {
			...
			String jsql = "select t.deptid from sys_flow_dept t where substr(t.tree_id,0,12) in ("+treeSql+")";
			sql += "select distinct t.userid,t.usernamefull as username,t.user_sex as sex,t.phone,t.status,t.position_real_level as positionRealLevel ,p1.occupation_name as positionNameFull,t.position_level as positionLevel ,p2.occupation_name as positionLevelName,	"
					+ "	d.deptid as userDeptId,fd.deptname as userDeptName,fd.chuid as userChushiId,fd.chuname as userChushiName,fd.juid as userJuId,fd.juname as userJuName	"
					+ "	from sys_flow_user t "
					+ "	left join sys_user_position p1 on (t.position_real_level = p1.occupation_level and p1.occupation_type = "+XXKHCommonConstants.OCCUPATION_TYPE[0]+")	"
					+ "	left join sys_user_position p2 on (t.position_level = p2.occupation_level and p2.occupation_type = "+XXKHCommonConstants.OCCUPATION_TYPE[1]+")	"
	...
	public List<XxkhTestPaper> saveTestPaper(String testId,String paperId) {
		...
		if(paperId.indexOf(",")!= -1){
			...
			for (int i=0;i<papers.length;i++) {
				...
				//暂时按照一套试卷先保存
				if(i<1){
					List<XxkhTestPaper> li = getPapersByTestId(testId);
					if(li.size()==0){
						sort = 1;
					}else{
	...
	public int getMaxSortByTestId(String id) {
		String sql = "select max(t.sort) from xxkh_test_paper t where t.visible = '"+CommonConstants.VISIBLE[1]+"' ";
		if (StringUtils.isNotBlank(id)){
            sql += "	and t.test_id = '"+id+"'";
        }
	...
	public List<XxkhTestPaper> getPapersByTestId(XxkhTestPaper paper) {
		List<XxkhTestPaper> list = new ArrayList<>();
		String sql = "select t.id,t.test_id,t.paper_id,t.sort,d.name,d.full_score,d.state as paperState,d.is_share as paperShared,f.type	"
	 ...
	 */
	@Override
	public PageImpl getTestPersonCountInUserPaper(Pageable pageable, XxkhUserPaper paper, PageImpl pageImpl) {
		pageImpl.setFlag("0");
		StringBuilder querySql = new StringBuilder();
		List<Object> para = new ArrayList<>();
		//全部列表查询
		querySql.append(" select t.candidate_chushi_id,t.candidate_chushi_name,t.candidate_ju_id,t.candidate_ju_name,count(t.id) as num  from xxkh_user_paper t ");
		querySql.append("	where t.visible = '"+CommonConstants.VISIBLE[1]+"'");
		//拼接条件
		...
		String sql = "select m.*,(select count(distinct t.candidate_chushi_id) from xxkh_user_paper t where  t.test_id = '"+paper.getTestId()+"' ) as total from (select n.*,rownum rn from ("+querySql+") n where rownum<="+after+") m  where rn>="+pre;
	 ...
	 * @param testList 此次考试的试卷信息
	 * @param name 考试名称
	 * @throws Exception
	 * @return JSONObject
	 */
	@Override
	public JSONObject saveReportPerson(String id, String personIds, String deptId,List<XxkhTestPaper> testList,String name)throws Exception {
		JSONObject json = new JSONObject();
		json.put("falg", "0");
			...
			if("1".equals(result.get("flag")) ){
				...
				if(arr.size()==0){
					...
					try {
						...
						//遍历试卷信息
						for(XxkhTestPaper par : testList){
							//遍历人员信息
							for (SysTestUserVo uVo : userVoList) {
								XxkhUserPaper userPaper = saveUserInfo(uVo);
								//随机生成id
	...
	public XxkhUserPaper saveUserInfo(SysTestUserVo uVo) {
		...
		userPaper.setPaperSubjectiveScore("0");
		userPaper.setPaperObjectiveScore("0");
		return userPaper;
	}
	...
	public void updateObjectiveScore(XxkhUserPaper info){
		...
		//jdbcTemplate.update(sql,new BeanPropertyRowMapper<>(XxkhUserPaper.class));
