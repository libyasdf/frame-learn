var typeTree = function(){
    ...
    function onDrop(event, treeId, treeNodes, targetNode, moveType) {
    	...
    	for (var i=0,l=treeNodes.length; i<l; i++) {
    		...
    		$.ajax({
            	type:"post",
            	url:"/system/config/dictionary/updatetreeSort?id=" + idStr +"&dropId="+dropId+"&pId="+pId,
            	dataType:"json",
    ...
    function onClick(event, treeId, treeNode) {
        var opt = {
            url:"/system/config/dictionary/list?type=1&mark=" + treeNode.mark
        };
    ...
    var init = function (json) {
        ...
        $.getJSON("/system/config/dictionary/list?type=0&mark=" + _arg.mark, function (json) {
