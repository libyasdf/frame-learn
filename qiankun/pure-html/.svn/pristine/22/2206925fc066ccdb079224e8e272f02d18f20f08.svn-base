$(function() {
	...
	if($("#id").val() != ""){
		...
		httpRequest("get","/djgl/ddjs/shyk/zbwyh/edit",datas,function (data){
...
function saveForm(){
	...
	if(bootstrapValidator.isValid()){
		...
		$.ajax({
			type: "POST",
			url: "/djgl/ddjs/shyk/zbwyh/saveZbwyh",
			data:$("#form").serialize(),
			async: false,
...
function iniFileUpload(){
	...
	//初始化文件上传
	MyFileUpload.init({
		viewParams: {"tableId":$("#id").val(),"tableName":"DDJS_SHYK_COMMITTEE"},
