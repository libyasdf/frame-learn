var _id,_name,_mark,_index,_parentId,_parentName,_treeId,_groupid,_groupname,_sysDeptId,_typeCode;
/**
 * 打开群组人员树,数据加载方式：异步
 * json:
 ...
 */
function openGenaralsSelectZtree(json){
    debugger;
    ...
    _id = id,_name = name,_groupid =groupid,_groupname =groupname,_sysDeptId=sysDeptId,_parentId=parentId,_typeCode =typeCode;
    var contentUrl,title;
    title="选择群组";
    contentUrl = "/modules/system/component/groupTree/asynGenaralsUserTree.html";
    var param = "?url="+url+"&resId="+$("#"+resId).val()+"&subId="+subId+"&typeCode="+typeCode;
    layer.open({
        id:"selectZtree",
        type: 2,
        ...
        success:function(layero, index){
           ...
           $(obj).find('#sysDeptId').val(sysDeptId)
            $(obj).find('#parentId').val(parentId)
            $(obj).find('#typeCode').val(typeCode)
