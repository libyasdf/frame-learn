...
public class BmglServiceImpl implements BmglService {
	...
	public JSONObject dynamicAdd(String jsonStr, String tName, String fids, String all, String ranking) {
		...
		if(fid.length == 1 && !"".equals(fids)){
			...
			if("2".equals(all) && "2".equals(ranking) && fid.length == 1 && !"".equals(fids)){
				//关联信息
				String pTableSql =
						"SELECT t.*\n" +
				...
				List<Map<String,Object>> pTableList = jdbcTemplate.queryForList(pTableSql,tableName);
				if(pTableList.size()>0) {
					...
					for (Entry<String, String> entry : parseObject.entrySet()) {
						for(int i=0;i<colName.size();i++){
							//获取数量
							if(sColName.equals(entry.getKey())){
								sColData = entry.getValue();
							}
							if("cre_chushi_id".equals(entry.getKey())){
								creChushiId = entry.getValue();
							}
						}
					}
					//已组卷
					//与父表做了关联,查询出父表的那条数据//查询父表数据时添加同一个创建人、立卷单位的过滤条件 王磊 2019-03-02
					String pDataSql = "SELECT t.* FROM " + pTableName + " t WHERE t." + mColName + " = '" + sColData +"'";
					if(tableName.indexOf("dak")==-1){
						//单位预立库添加录入人和立卷单位标识
						pDataSql += " and t.CRE_USER_ID='" + UserUtil.getCruUserId() + "' and t.CRE_CHUSHI_ID='" + creChushiId + "'";
					}
					List<Map<String, Object>> pTableDataList = jdbcTemplate.queryForList(pDataSql);
					//新增两层表的第二层时，动态修改父表的文件开始时间和文件结束时间
					setStratAndEndTime(ranking,pTableName,pTableDataList);
				}
			}
		}
		//修改文件开始时间和文件结束时间
     ...
     */
	@Override
	public int dynamicUpdate(String jsonStr, String tName,String key,String value, String all, String ranking, String fileNumberRule) {

		//防止SQL注入，先去查询表名集合，判断是否含有传过来的表，含有，则取查询出来的表名，不用前台传过来的表名
		String tableName = "";
		String oldPageNumAddOrDel = "0";
		String newPageNumAddOrDel = "0";
		Map<String, String> fileNumberRuleData = new HashMap<String, String>();
		List<String> tableNames = findAllTables();
		for(int i=0;i<tableNames.size();i++){
			if(tName.equals(tableNames.get(i))){
				tableName = tableNames.get(i);
		...
		//查询档号规则，并统计出各项的值，修改子表，不进要修改档号字段，还要修改档号项字段，并且要考虑立卷单位这特殊字段。
		//档号规则
		List<Map<String, String>> fileNumberRuleList = com.alibaba.fastjson.JSONObject.parseObject(fileNumberRule,
		...
		//获取立卷单位的信息，更新子表的时候同样更新立卷单位的信息
		String creChushiId = "";
		String basefolderUnit = "";
		String archiveFlag = "";
		//修改的字段是否影响档号组成项，计算影响的数量，大于1则会归至另外一个父级当中。
		int dhColCount = 0;
		//便利修改的数据统一进行修改
		for (Entry<String, String> entry : parseObject.entrySet()) {
			for(int i=0;i<colName.size();i++){
				if(colName.get(i).equals(entry.getKey())){
					...
					if("page_num".equals(colName.get(i))&& all.equals(ranking) && Integer.parseInt(all)>1){
						newPageNumAddOrDel = "".equals(entry.getValue()) || null == entry.getValue()?"0":entry.getValue();
					}
					//得到立卷单位的信息
					if("cre_chushi_id".equals(colName.get(i))){
						creChushiId = entry.getValue();
						if(!creChushiId.equals(map.get("CRE_CHUSHI_ID")) && tableName.indexOf("_dak")==-1){
							//修改了单位预立库的立卷单位
							dhColCount++;
						}
					}
					if("basefolder_unit".equals(colName.get(i))){
						basefolderUnit = entry.getValue();
					}
					if("archive_flag".equals(colName.get(i))){
						archiveFlag = entry.getValue();
					}
				}
			}
			for(int j=0;j<fileNumberRuleList.size();j++){
				if(fileNumberRuleList.get(j).get("RULE_FIELD").equals(entry.getKey())){
					fileNumberRuleData.put(entry.getKey(),entry.getValue());
					String oldData = entry.getValue();
					String nowData = map.get(entry.getKey().toUpperCase())==null?"":map.get(entry.getKey().toUpperCase()).toString();
					if(!oldData.equals(nowData)){
						//修改了单位预立库的立卷单位
						dhColCount++;
					}
				}
		...
		if(Integer.parseInt(ranking)>1 && "已组卷".equals(map.get("ARCHIVE_FLAG")==null?"":map.get("ARCHIVE_FLAG").toString())){
			//一层往上,并且是已组卷才会影响父级的数量
			if(dhColCount==0){
				//修改的数据没有影响到档号项
				if(!("0".equals(pageNumAddOrDel)) && all.equals(ranking)){
					//只有最后一层，并且修改了数量，才会影响父级的数量
				...
				//修改的数据影响到档号项
				//减少以前父级的数量（相当于调出）
				String pageNumData = oldPageNumAddOrDel;
				String spieceNumData = map.get("PIECE_NUM")==null?"0":map.get("PIECE_NUM").toString();
				String pieceNumData = "-1";
				//把对象转成String，模拟修改表单，修改对应的属性后封装成String，动态修改
				net.sf.json.JSONObject oldJsonStr=net.sf.json.JSONObject.fromObject(map);
				List<Map<String, Object>> oldPdata = queryPData(tableName,oldJsonStr.toString());
				String oldPid = "";
				if(oldPdata.size()>0){
					oldPid = oldPdata.get(0).get("RECID")==null?"":oldPdata.get(0).get("RECID").toString();
				}
				int updateOldPNum = updatePNum(oldPid,value,tableName,all, (Integer.parseInt(ranking))+"",pieceNumData,"-"+pageNumData,"diaochu","-"+spieceNumData);
				//增加修改档号项之后关联的父级的数量（相当于调入）
				String nowPageNumData = newPageNumAddOrDel;
				String nowSpieceNumData = map.get("PIECE_NUM")==null?"0":map.get("PIECE_NUM").toString();
				String nowPieceNumData = "1";
				List<Map<String, Object>> nowPdata = queryPData(tableName,jsonStr);
				String nowPid = "";
				if(nowPdata.size()>0){
					nowPid = nowPdata.get(0).get("RECID")==null?"":nowPdata.get(0).get("RECID").toString();
				}
				int updateNowPNum = updatePNum(nowPid,value,tableName,all, (Integer.parseInt(ranking))+"",nowPieceNumData,nowPageNumData,"diaoru",nowSpieceNumData);
			}
		}
		//判断是否有子级，如果没有退出
		if (col_name.equals("")) {
			List<Map<String,Object>> findById = findById(tableName, value);

			//两层档案第二层已组卷的数据修改时，不管有没有修改文件形成日期，都去更新第一层的文件开始时间，和文件结束时间--王富康2019/07/18
			if("2".equals(all) && "2".equals(ranking) ){
				archiveFlag = findById.get(0).get("ARCHIVE_FLAG")==null?"":findById.get(0).get("ARCHIVE_FLAG").toString();
		...
		//涉及到档号变化就进行子表的更新，更新关联字段，单位预立库的立卷单位同样影响到档号；
		if (!(map.get(col_name).equals(col_nameValue) && map.get("cre_chushi_id").equals(creChushiId))) {
			//查询是否有子级
			...
			if (!TableName.isEmpty()) {
				...
				if (findDAGLByTableNameF2==null||findDAGLByTableNameF2.isEmpty()) {//判断是否有子级
					...
					List<Map<String,Object>> findChildren = findChildren((String)TableName.get("S_TABLE_NAME"), (String)TableName.get("S_COL_NAME"), (String)map.get(TableName.get("S_COL_NAME")),"",String.valueOf(map.get("CRE_CHUSHI_ID")));//查没改档案号之前的子级
					//如果查到，进行修改
					if (findChildren!=null&&!findChildren.isEmpty()) {
						for(Map<String, Object> maps:findChildren) {
							//把之前关联字段的值截取掉，保留连接符和“序号”aa
							String oldFolderNo = "";
							if(map.get(TableName.get("S_COL_NAME")).toString().indexOf("$$")>-1){
								oldFolderNo = map.get(TableName.get("S_COL_NAME")).toString().substring(0,map.get(TableName.get("S_COL_NAME")).toString().lastIndexOf("$$"));
							}else{
								oldFolderNo = map.get(TableName.get("S_COL_NAME")).toString();
							}
							String addString = "";
							if(col_nameValue.indexOf("$$")>-1){
								addString = col_nameValue.substring(0,col_nameValue.lastIndexOf("$$"));
							}else{
								addString = col_nameValue;
							}
							String newValue=addString+maps.get("ARCHIVE_NO").toString().substring(oldFolderNo.length());//新的文件档案号

							String updateSql = "update "+(String)TableName.get("S_TABLE_NAME")+" set "+(String)TableName.get("S_COL_NAME")+" = '" +col_nameValue + "',archive_no ='"+newValue+"'";
							//把档号规则相关字段的信息获取到，存到子表中
							Set<String> keyset=fileNumberRuleData.keySet();
							for(String ks:keyset)
							{
								updateSql +=","+ks+"='"+fileNumberRuleData.get(ks)+"'";
							}
							if(TableName.get("S_TABLE_NAME").toString().indexOf("_dak")==-1){
								//在单位预立库修改父级的立卷单位，子表数据同样跟着改动
								updateSql +=",cre_chushi_id='"+creChushiId+"',basefolder_unit='"+basefolderUnit+"' ";
							}
							if("3".equals(all)){
								//计算出项目级档号的长度
								//这里因为只有三层的第二层才会进这里，
								//上层连接符的长度
								int ConnectorLength = fileNumberRuleList.get(fileNumberRuleList.size()-2).get("CONNECTOR").toString().length();
								//本层的字段长度
								int lastColLength = Integer.parseInt(fileNumberRuleList.get(fileNumberRuleList.size()-1).get("NUMB_LENGTH"));

								String itemFolderNo = addString.substring(0,addString.length()-ConnectorLength-lastColLength);

								updateSql +=",item_folder_no='"+itemFolderNo+"'";
							}
							updateSql += " where recid = '"+(String)maps.get("recid")+"'";
					...
					//此时标识还有子级
					//先修改自己的子级(第二层)，目前只有修改三层表的第一层才会进这里
					List<Map<String,Object>> findChildren = findChildren((String)TableName.get("S_TABLE_NAME"), (String)TableName.get("S_COL_NAME"), (String)map.get(TableName.get("S_COL_NAME")),"",String.valueOf(map.get("CRE_CHUSHI_ID")));
					if (findChildren!=null&&!findChildren.isEmpty()) {
						for(Map<String, Object> maps:findChildren) {
							//把之前关联字段的值截取掉，保留连接符和“序号”
							String oldFolderNo = "";
							if(map.get(TableName.get("S_COL_NAME")).toString().indexOf("$$")>-1){
								oldFolderNo = map.get(TableName.get("S_COL_NAME")).toString().substring(0,map.get(TableName.get("S_COL_NAME")).toString().lastIndexOf("$$"));
							}else{
							...
							String newValue=addString+maps.get(findDAGLByTableNameF2.get("S_COL_NAME")).toString().substring(oldFolderNo.length());//新的文件档案号
							String updateSql = "update "+(String)TableName.get("S_TABLE_NAME")+" set "+(String)TableName.get("S_COL_NAME")+" = '" +col_nameValue + "',"+findDAGLByTableNameF2.get("S_COL_NAME")+" ='"+newValue+"'";
							//把档号规则相关字段的信息获取到，存到子表中
							Set<String> keyset=fileNumberRuleData.keySet();
							for(String ks:keyset)
							{
							...
							if (findChildren2!=null&&!findChildren2.isEmpty()) {
								...
								for(Map<String, Object> maps2:findChildren2) {
									//把之前关联字段的值截取掉，保留连接符和“序号”
									String sOldFolderNo = "";
									if(maps.get(findDAGLByTableNameF2.get("S_COL_NAME")).toString().indexOf("$$")>-1){
										sOldFolderNo = maps.get(findDAGLByTableNameF2.get("S_COL_NAME")).toString().substring(0,maps.get(findDAGLByTableNameF2.get("S_COL_NAME")).toString().lastIndexOf("$$"));
									}else{
										sOldFolderNo = maps.get(findDAGLByTableNameF2.get("S_COL_NAME")).toString();
									}
									String newValue2=newValue+maps2.get("ARCHIVE_NO").toString().substring(sOldFolderNo.length());//新的文件档案号
									String updateSql2 = "update "+(String)findDAGLByTableNameF2.get("S_TABLE_NAME")+" set "+(String)findDAGLByTableNameF2.get("S_COL_NAME")+" = '" +newValue + "',archive_no ='"+newValue2+"',item_folder_no='"+col_nameValue+"'";
									//把档号规则相关字段的信息获取到，存到子表中
									Set<String> sKeyset=fileNumberRuleData.keySet();
									for(String sks:sKeyset){
										updateSql2 +=","+sks+"='"+fileNumberRuleData.get(sks)+"'";
									}

									if(findDAGLByTableNameF2.get("S_TABLE_NAME").toString().indexOf("_dak")==-1){
										//在单位预立库修改父级的立卷单位，子表数据同样跟着改动
										updateSql2 +=" ,cre_chushi_id='"+creChushiId+"',basefolder_unit='"+basefolderUnit+"'";
									}
									updateSql2 +=  " where recid = '"+(String)maps2.get("recid")+"'";
     ...
     */
	@Override
	public net.sf.json.JSONObject dynamicList(String tName,PageImpl pageImpl,String jsonStr,String conditions,String fids,String parameters, String complexParam,String danweihao,String danweiku,Integer all,String ljdanweiAndRenyuan) {
		...
		if (StringUtils.isNotEmpty(danweiku)) {
			...
			if("dak".equals(danweiku)) {
				...
				//这里设置了一个数据字典，去查询数据字典
				//含有字典值
				//根据字段名称和表名去表描述查询出字典值，如果存在字典值，并且不是全宗的，取code拼接档号2019/07/16-王富康
				List<DataDictionary> listByMark = dataDictionaryDao.getListByMark("dwgdk_stratTime");
				String stratTime = listByMark.get(0).getCode();
				//归档日期大于设置的时间
				user += " and pigeonhole_date >"+stratTime;

			}else if("woshiyuliku".equals(danweiku)){
				//预立库不显示已归档的数据 王富康20181203
		...
		String sql = "select * from " +tableName+ " where 1=1 "+easyQuery+whereAnd+wherefid+danwei+user+ complexQuery +renyuanLjDw+archiveEntityStatus+sortName+ sortOrder;
		String sqlStr="select * from " +tableName+ " where 1=1 "+easyQuery_bak+whereAnd_bak+wherefid_bak+danwei_bak+user+ complexQuery +renyuanLjDw_bak+archiveEntityStatus+sortName+ sortOrder;
		...
		log.error("查询列表sql:"+sql+",para:"+para.toString());

		Integer queryForObject = 0;
		net.sf.json.JSONObject json = new net.sf.json.JSONObject();
		json.put("flag", "1");
		if(queryForList!=null&&!queryForList.isEmpty()) {
			String totalsql = "select count(1) from " +tableName+ " where 1=1 "+easyQuery+whereAnd+wherefid+danwei+user+complexQuery+renyuanLjDw+archiveEntityStatus;
			queryForObject = jdbcTemplate.queryForObject(totalsql, Integer.class,para.toArray());
			json.put("sqlStr", sqlStr);
		}else {
			json.put("sqlStr", sqlStr);
		}
		queryForList = nullToEmpty(queryForList);
		Page page = new Page(pageImpl.getPageNumber(), pageImpl.getPageSize(), queryForObject, queryForList);
		net.sf.json.JSONObject data = new net.sf.json.JSONObject();
		data.put("total", page.getTotalResult());
		data.put("rows", page.getRecordList());
		json.put("data", data);
		return json;
	}
     ...
     *
     * @author 颜振兴
     * @param isQ2 是否展示q2档案，true:展示，false:不展示
	 * @param isAdmin 是否管理员（档案馆管理员，立卷单位管理员），1：管理员，2：录入人
	 * @param dwylk 单位预立库中dwylk=danweiyuliku，单位归档库中,档案借阅-档案检索：dwylk=danweiguidang
	 * 按门类、按立卷单位、借阅管理-传统借阅都是null
	...
	public List<VirtualClass> findTree(boolean isQ2,String isAdmin,String dwylk) {
		...
		for(int i=0;i<doorList.size();i++) {
			//解决bug 36035 q2不显示，但是q2下虚拟分类显示且跟全宗同级 王磊 20190729
			//这里统一根据前端传值来决定q2是不是显示的依据,如果isQ2==false，这里就把q2去掉
			if(isQ2 == false && ("q2").equals(doorList.get(i).getCategoryCode().toLowerCase())){
				continue;
			}
			codes[i]="'"+doorList.get(i).getCategoryCode()+"'";
		}
		List<VirtualClass> findTree = dao.findTree("1",dwylk,codes);
		List<VirtualClass> list = new ArrayList<>();
		
		for (DaglCategoryTable categoryTable : doorList) {
			//解决bug 36035 q2不显示，但是q2下虚拟分类显示且跟全宗同级 王磊 20190729
			//这里统一根据前端传值来决定q2是不是显示的依据
			if (("q2").equals(categoryTable.getCategoryCode().toLowerCase())) {
				if (!isQ2) {
					continue;
				}
			}
     ...
     * @param fids
     * @param column
     * @param oldReplaceValue
	 * @param oldReplaceCode
	 * @param nowReplaceValue
     * @param nowReplaceCode
     * @return
     * @author 颜振兴
     * int
     * TODO 替换
     */
	@Override
	public net.sf.json.JSONObject replace(String tName, String fids, String column,String categoryCode,String fileNumberRule, String replaceInputType, String oldReplaceValue,String oldReplaceCode, String nowReplaceValue, String nowReplaceCode, String all, String ranking, String type) {
		net.sf.json.JSONObject json = new net.sf.json.JSONObject();
		//档号规则
		List<Map<String, String>> fileNumberRuleList = com.alibaba.fastjson.JSONObject.parseObject(fileNumberRule,
				new TypeReference<List<Map<String, String>>>() {
		...
		//修改的字段是否档号项
		boolean isFileNumberRule  = false;
		//修改的档案题名
		String updateMainTitle = "";
		//未修改成功的档案题名
		String notUpdateMainTitle = "";
		//防止SQL注入，先去查询表名集合，判断是否含有传过来的表，含有，则取查询出来的表名，不用前台传过来的表名
		String tableName = "";
		List<String> tableNames = findAllTables();
		for(int i=0;i<tableNames.size();i++){
		...
		//获取档号规则，判断字段是否属于档号规则
		for(int i=0;i<fileNumberRuleList.size();i++) {
				if(col.equals(fileNumberRuleList.get(i).get("RULE_FIELD"))){
					isFileNumberRule = true;
				}

		}
		//修改的不是档号字段
		String value11="";
		if(StringUtils.isNotEmpty(oldReplaceValue) && "replace".equals(type)) {
			value11 = " like ? ";
			if("select".equals(replaceInputType)){
				//下拉框取name值，所有的下拉，除了全宗号，目前都是存的name，全宗号必定是档号规则，所以这里不考虑全宗号，所有下拉去那么值

				para.add("%"+oldReplaceCode+"%");
			}else{
				//input取input值
				para.add("%"+oldReplaceValue+"%");
			}
		...
		//查询勾选数据中符合修改条件的数据
		String sql="select * from "+tableName+" where 1=1" + wherefid + "";
		if("replace".equals(type)){
			...
			for(Map<String,Object> map:queryForList) {
				//判断自己本身数据是否和修改的数据一样，如果一样的话，不用处理，直接跳过。
				String nowValue = "";
				if("select".equals(replaceInputType)){
					//如果是下拉框，存name值
					nowValue = nowReplaceCode;
				}else{
					//input取input值
					nowValue = nowReplaceValue;
				}
				String oldValue = map.get(column)==null?"":map.get(column).toString();
				if(oldValue.equals(nowValue)){
					//跳过本次循环；
					continue;
				}
			if(isFileNumberRule || ("basefolder_unit".equals(col) && tableName.indexOf("_dak")==-1)){
				//修改的是档号字段,或者是单位预立库的立卷单位
				//这里将会影响档号规则，向目标项目或者案卷或者立卷单位追加当前数据，及其子表的数据修改；
				// 如果是第二层，或者是第三层，先判断是否含有父级数据，没有，则提示不能修改
				//查到父表数据，并且只有一条父表数据，或者第一层，可以修改；
				//修改自己，及其子表
				//封装数据
				//修改立卷单位，往即将修改的立卷单位中添加，
				//查询最后一位“序号”
				//关联信息
				String pTableSql ="SELECT t.*\n" +
						"  FROM tables_relation t, table_description s\n" +
						" WHERE t.m_table_name = s.table_name\n" +
						"   and t.s_table_name = ? ";//排除父节点为门类的数据

				List<Map<String,Object>> tableRelations = jdbcTemplate.queryForList(pTableSql,tableName);

				String lastColLength = fileNumberRuleList.get(fileNumberRuleList.size()-1).get("NUMB_LENGTH");
				String lastColName = fileNumberRuleList.get(fileNumberRuleList.size()-1).get("RULE_FIELD");
				String danghaoCol = "";
				if("1".equals(ranking)){
					//判断第二层的档号
					//获取档号字段
				...
				String danghaoValue = "";
				//父级的父级的档号+连接符
				String pDanghaoValue = "";
				//获取最大的档号
				String creChushiId = "";
				int pSize = 0;
				if("basefolder_unit".equals(col)){
					creChushiId = nowReplaceValue;
					danghaoValue = map.get(danghaoCol).toString().substring(0,map.get(danghaoCol).toString().length()-Integer.parseInt(lastColLength));
				}else{
					creChushiId = map.get("cre_chushi_id").toString();
					//修改其他档号项需要重新拼接档号；自己，父级，父父级；
					for(int j=0;j<fileNumberRuleList.size()-1;j++){
						//去除最后一个“序号”
						//档号项的字段名
						String RuleField = fileNumberRuleList.get(j).get("RULE_FIELD");
						String Connector = "";
						String NumbLength = fileNumberRuleList.get(j).get("NUMB_LENGTH");
						if(!StringUtils.isBlank(fileNumberRuleList.get(j).get("CONNECTOR"))){//防止连接符为空
							Connector = fileNumberRuleList.get(j).get("CONNECTOR");
						}
						if("fonds_no".equals(RuleField)){
							//全宗号存的code，也不会改变，直接取
							danghaoValue += map.get("fonds_no").toString()+Connector;
							if(j<fileNumberRuleList.size()-1){
								pDanghaoValue += map.get("fonds_no").toString()+Connector;
							}
						}else if(RuleField.toUpperCase().equals(column)){
							danghaoValue += nowReplaceValue+Connector;
							if(j<fileNumberRuleList.size()-1){
								pDanghaoValue += nowReplaceValue+Connector;
							}
							if("".equals(nowReplaceCode) || null == nowReplaceCode){
								//文本框没有name值，存value
								map.put(column,nowReplaceValue);
							}else{
								map.put(column,nowReplaceCode);
							}
								...
								if(StringUtils.isNotEmpty(queryForObject)){
									...
									if(dicts.size()>0){//存在取code
										for(int m=0;m<dicts.size();m++){
											if( map.get(RuleField).equals(dicts.get(m).getName())){
												danghaoValue += dicts.get(m).getCode()+Connector;
												if(j<fileNumberRuleList.size()-1){
													pDanghaoValue += dicts.get(m).getCode()+Connector;
												}
											}
										}
									}else{//不存在正常存放
										danghaoValue += map.get(RuleField)+Connector;
										if(j<fileNumberRuleList.size()-1){
											pDanghaoValue += map.get(RuleField)+Connector;
										}
									}
								}else{
									//没有数据字典，那么就直接存值
				...
				int maxXuhao = queryCountAdd(danghaoValue,danghaoCol,tableName,creChushiId,map.get("cre_user_id").toString(),lastColName,ranking);
				//得到一个NumberFormat的实例，补0
				NumberFormat nf = NumberFormat.getInstance();
				//设置是否使用分组
				nf.setGroupingUsed(false);
				//设置最大整数位数
				nf.setMaximumIntegerDigits(Integer.parseInt(lastColLength));
				//设置最小整数位数
				nf.setMinimumIntegerDigits(Integer.parseInt(lastColLength));
				String newLastData = nf.format(maxXuhao+1);
				map.put(lastColName.toUpperCase(),newLastData);
				if("basefolder_unit".equals(col)){
					map.put("BASEFOLDER_UNIT",nowReplaceCode);
					map.put("CRE_CHUSHI_ID",nowReplaceValue);
					map.put(danghaoCol.toUpperCase(),danghaoValue+newLastData);
				}else{
					//其他档号规则字段
					//自己的
					map.put(danghaoCol.toUpperCase(),danghaoValue+newLastData);
				...
				if(tableRelations.size()>0){
					Map<String, Object> ppRelationData = tableRelations.get(0);
					//查询父表数据
					//父表表名
					String pTableName = String.valueOf(ppRelationData.get("M_TABLE_NAME"));
					//父表字段
					String pmColName = String.valueOf(ppRelationData.get("M_COL_NAME"));
					//子表字段
					String psColName = String.valueOf(ppRelationData.get("S_COL_NAME"));

					//子表关联字段的值（父表的档号）
					String sColData = String.valueOf(danghaoValue.substring(0,danghaoValue.length()-1));
					resultMap.put(psColName,danghaoValue.substring(0,danghaoValue.length()-1));
					//处室id
					String chushiId = String.valueOf(resultMap.get("cre_chushi_id"));
					//创建人id
					String creUserId = String.valueOf(resultMap.get("cre_user_id"));
					//查看当前数据的组卷状态，未组卷的话，就不往上级找了
					String archiveFlag = String.valueOf(resultMap.get("archive_flag"));
					if(DAGLCommonConstants.ARCHIVE_FLAG[0].equals(archiveFlag)) {
						//判断表中是否含有组卷标识的字段
						String queryArchiveFlagColSql = "SELECT COUNT(*) FROM USER_TAB_COLUMNS WHERE TABLE_NAME = ? AND COLUMN_NAME = 'ARCHIVE_FLAG'";
						int count = jdbcTemplate.queryForObject(queryArchiveFlagColSql, Integer.class, pTableName.toUpperCase());
						//查询父表记录（单位库）
						String pSql = "select t.* from " + pTableName + " t where 1=1 and t." + pmColName + " = '" + sColData + "'";
						if (count > 0) {
							pSql += "and t.archive_flag='" + DAGLCommonConstants.ARCHIVE_FLAG[0] + "'";
						}
						if (pTableName.indexOf("_dak") == -1) {
							//查父表记录条件:关联字段、立卷单位（上边的处室id）、创建人id一致
							pSql += "	and t.cre_chushi_id = '" + chushiId + "' and t.cre_user_id='" + creUserId + "' ";
						}
						List<Map<String, Object>> pTableDataList = jdbcTemplate.queryForList(pSql);
						//把父表的数据长度存起来
						pSize = pTableDataList.size();
					}
				}

				if(pSize==1 || Integer.parseInt(ranking) == 1){
					//把对象转成String，模拟修改表单，修改对应的属性后封装成String，动态修改
					net.sf.json.JSONObject jsonStr=net.sf.json.JSONObject.fromObject(resultMap);
					int updateNum = dynamicUpdate(jsonStr.toString(), tableName,"recid",map.get("RECID").toString(), all, ranking,fileNumberRule);
					if(updateNum>0){
						updateMainTitle += "【"+map.get("MAINTITLE").toString()+"】";
					}
				}else{
					notUpdateMainTitle +="【"+map.get("MAINTITLE").toString()+"】修改后不存在父表数据，不可修改!";
				}

				}else{
					String replaceAll="";
					//归档库的立卷单位修改的时候，需要把cre_chushi_id中存的立卷单位code替换了。
					String creChushi = "";
					if(StringUtils.isBlank(oldReplaceValue)) {
						//把空值替换成某个值
						if("select".equals(replaceInputType)){
							//如果是下拉框，存name值
							replaceAll = nowReplaceCode;
							if("basefolder_unit".equals(col)){
								creChushi = nowReplaceValue;
							}
						}else{
							//input取input值
							replaceAll = nowReplaceValue;
						}
					...
					if("select".equals(replaceInputType)){
						//如果是下拉框，存name值
						replaceAll = string1.replaceAll(oldReplaceCode, nowReplaceCode);
						if("basefolder_unit".equals(col)){
							creChushi = nowReplaceValue;
						}
					}else{
						//input取input值
						replaceAll = string1.replaceAll(oldReplaceValue, nowReplaceValue);
					}

				}

				String esql = " update "+tableName+ " set "+col+"= ? ";
				if("basefolder_unit".equals(col)){
					esql+=",cre_chushi_id ='"+creChushi+"'";
				}

				esql +=" where recid='"+map.get("recid")+"'";
				int update = jdbcTemplate.update(esql,replaceAll);
				i+=update;
				if(update>0){
					updateMainTitle += "【"+map.get("MAINTITLE").toString()+"】";
				}else{
					notUpdateMainTitle +="【"+map.get("MAINTITLE").toString()+"】修改未成功!";
				}
			}
		}
		if("".equals(notUpdateMainTitle)){
			if(queryForList.size()==0){
				json.put("flag","0");
				json.put("msg","未匹配到相关数据！");
			}else{
				json.put("flag","1");
				json.put("msg",updateMainTitle+"已修改！");
			}
		}else{
			json.put("flag","0");
			json.put("msg",notUpdateMainTitle);
		}
		return json;
	}
	...
	public List<VirtualClass> findVirtualClass(VirtualClass virtualClass){
        ...
        para.add(virtualClass.getIsAdmin());
	...
	public int dynamicDelete(String tName, String id,int all,int ranking) {
		...
		String tableName = "";
		String pTableName = "";
		List<Map<String, Object>> pTableDataList = new ArrayList<>();
		//删除的话总件数，或者上层的件数-1
		String pieceNumData = "-1";
		List<String> tableNames = findAllTables();
		for(int i=0;i<tableNames.size();i++){
		...
		if(findById!=null&&!findById.isEmpty()) {
			//如果是两层表的第二层，并且时已组卷的数据，查询父表数据，为调整文件开始时间和文件结束时间做准备-20190718-王富康
			if("2".equals(all+"") && "2".equals(ranking+"")){
				String archiveFlag = findById.get(0).get("ARCHIVE_FLAG").toString();
				if(DAGLCommonConstants.ARCHIVE_FLAG[0].equals(archiveFlag)){
					//关联信息
					String pTableSql =
							"SELECT t.*\n" +
					...
					if(pTableList.size()>0) {
						//有父表，查询出父表的状态
						Map<String, Object> relationMap = pTableList.get(0);
						pTableName = String.valueOf(relationMap.get("M_TABLE_NAME"));//父表表名
						...
						String sColData = findById.get(0).get(sColName).toString();
						//已组卷
						//与父表做了关联,查询出父表的那条数据//查询父表数据时添加同一个创建人、立卷单位的过滤条件 王磊 2019-03-02
						String pDataSql = "SELECT t.* FROM " + pTableName + " t WHERE t." + mColName + " = '" + sColData +"'";
						if(tableName.indexOf("dak")==-1){
							//单位预立库添加录入人和立卷单位标识
							pDataSql += " and t.CRE_USER_ID='" + findById.get(0).get("CRE_USER_ID").toString() + "' and t.CRE_CHUSHI_ID='" + findById.get(0).get("CRE_CHUSHI_ID").toString() + "'";
						}
						pTableDataList = jdbcTemplate.queryForList(pDataSql);
					}
				}
			}
				...
				if(sNameF!=null&&!sNameF.isEmpty()) {
					...
					for(Map<String,Object> map:findListByguanlian) {
						...
						for(Map<String,Object> map2:findListByguanlian3) {
							...
							//调整文电数据的文件状态 王磊 20190409-在执行删除之前执行更改文件状态-2019-08-09-王富康
		...
		if("2".equals(all+"") && "2".equals(ranking+"") && pTableDataList.size()>0){
			//删除两层表的第二层时，动态修改父表的文件开始时间和文件结束时间
			setStratAndEndTime(ranking+"",pTableName,pTableDataList);
		}
		return update;
	}
	...
	public int openVolume(String tName, String ids, String archiveFlag, String conCol,String conVal, String fid, String all, String ranking) {
		...
		List<Object> para = new ArrayList<>();
		List<Object> thisPara = new ArrayList<>();
		if(!(" ".equals(conVal))){
			para.add(conVal);
		}
		para.add(archiveFlag);
		thisPara.add(archiveFlag);
		String sql = "";
		int num =0;
		if(" ".equals(conVal)){
			...
			for(int i=0;i<id.length;i++){ 
				List<Object> paraNew = new ArrayList<>();
				List<Object> thisParaNew = new ArrayList<>();
				paraNew.addAll(para);
				thisParaNew.addAll(thisPara);
				//根据id先查询当前数据
				...
				String pieceNumData = "-1";
				//更新自己的组卷标识和关联字段的值
				//如果没有子表，那么更新自己的关联字段和组卷标识
				String tableSql =  "	select s.*\n" +
						"  		from tables_relation s\n" +
						" 		start with s.s_table_name = ? \n" +
						"		connect by prior s.s_table_name = s.m_table_name\n" +
						" 	order by rownum asc";
				//得到子表及关联的信息
				List<Map<String,Object>> tbleList = jdbcTemplate.queryForList(tableSql,tName);
				Map<String, Object> tableRelationMap = tbleList.get(0);
				//父表关联字段
				String SColnumName = String.valueOf(tableRelationMap.get("S_COL_NAME"));
				paraNew.add(thisData.get(0).get(SColnumName)+lastString);
				//这里是更新自己和父级的关联关系
				sql = "UPDATE "+tableName+" SET ARCHIVE_FLAG = ? , "+SColnumName+" = ? WHERE RECID IN ('"+id[i]+"')";
				log.info(sql);
				num += jdbcTemplate.update(sql,paraNew.toArray());
				//如果有子表数据，同时更新所有子表
				...
				if(sTableList.size()>0) {
					Map<String, Object> sTableRelationMap = sTableList.get(0);
					List<Object> sPara = new ArrayList<>();
					//更新所有子表的状态
					//子表表名
					...
					sPara.add(thisData.get(0).get(PColnumName)+lastString);
					sPara.add(thisData.get(0).get(PColnumName));
					String Ssql = "UPDATE "+sTableName+" SET "+sColnumName+" = ? WHERE "+sColnumName +" = ? ";
					if(sTableName.indexOf("_dak")==-1){
						//单位预立库加创建人和创建人处室
						Ssql+= " and cre_user_id= '"+UserUtil.getCruUserId()+"' and cre_chushi_id = '"+thisData.get(0).get("CRE_CHUSHI_ID")+"' ";
					}
					log.info(Ssql);
					int Snum = jdbcTemplate.update(Ssql,sPara.toArray());
					//这里是更新自己跟子表的关联关系，第二层和第三层的关联关系
					thisParaNew.add(thisData.get(0).get(PColnumName)+lastString);
					String thisSql = "UPDATE "+tableName+" SET ARCHIVE_FLAG = ? , "+PColnumName+" = ? WHERE RECID IN ('"+id[i]+"')";
					log.info(thisSql);
					num += jdbcTemplate.update(thisSql,thisParaNew.toArray());
				}
				//更新数量
	...
	public int updateFileRelation(String tName, String conCol, String conVal, String data,String categoryCode,String pName,String pId, String ranking, String all) {
		...
		try {
			...
			//更新子表的sql
			List<String> updateSTableStatusSql = new ArrayList<>();
				...
				if(Integer.parseInt(all)>2 && Integer.parseInt(ranking)<= Integer.parseInt(all)-2){
					...
					for(int j = 0;j<sTableList.size();j++){
						...
						for(int m = 0;m<sData.size();m++){
							...
							updateSTableStatusSql.add("update "+sTableName+
			...
			//更新第三层的子表数据
			for(int i=0;i<updateSTableStatusSql.size();i++){
				int[] updateSTableSum =  jdbcTemplate.batchUpdate(updateSTableStatusSql.get(i));
			}
			//执行完修改之后，如果是两层的表，并且是勾选第一层，调整第二层的时候，正常来说这里只需要判断层数为两层的就好了，
			// 但是，为了更严谨一些，判断勾选的是第一层，
			// 更新文件开始时间和文件结束时间
			if("2".equals(all) && "1".equals(ranking)){
				setStratAndEndTime(ranking,ptableName,pData);
			}
	...
	public List<Map<String, Object>> recordSubmit(String tName, String fids) {
		...
		//存储当前那些可以做移交数据的题名，用于给档案管理员发送消息
		List<String> mainTitleList = new ArrayList<String>();
		//存储当前那些可以做移交数据的档号，用于给档案管理员发送消息
		List<String> dangHaoList = new ArrayList<String>();
		...
		for(int i = 0;i < queryForList.size();i++){
			...
			if(DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[1].equals(archive_entity_status)||DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[2].equals(archive_entity_status)){
				...
				if(pTableList.size()>0){
					...
					String mColName = String.valueOf(relationMap.get("M_COL_NAME"));//父表字段
					String sColName = String.valueOf(relationMap.get("S_COL_NAME"));//子表字段
					sColData = String.valueOf(map.get(sColName));
					...
					if(DAGLCommonConstants.ARCHIVE_FLAG[0].equals(archiveFlag)){*/
						...
						if(pTableDataList.size()>0){//常理来说是不应该为空的，这里做判断是为了防止垃圾数据导致报错
							...
							if(DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[1].equals(P_archive_entity_status)||DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[2].equals(P_archive_entity_status)){
								//父表状态为已移交待汇总，或者已归档，则证明为补录，更新其状态为已移交待汇总
								//更新选中的档案的移交状态
								updateArchiveEntityStatus(tableName,map,DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[1]);
								
								successList.add(map);
								//添加档号和题名
								dangHaoList.add(sColData);
								mainTitleList.add(String.valueOf(map.get("MAINTITLE")));
					...
					successList.add(map);
					//添加档号和题名
					pTableSql ="select t.* from tables_relation t where t.s_table_name=?";
					pTableList = jdbcTemplate.queryForList(pTableSql,tName);
					Map<String,Object> relationMap = pTableList.get(0);
					//获得当前表档号字段
					String sColName = String.valueOf(relationMap.get("S_COL_NAME"));
					sColData = String.valueOf(map.get(sColName));
		...
		//给档案管理员发送消息，这里加上异常处理，防止发消息异常对本来的逻辑产生映射
		try {
			String selDangAnAdmin = "select sfd.deptname,sfd.deptid,duser.userid"
                    +"	from"
                    ...
                    +"	on d.id = f.u_dept_id"
                    +"	where f.roleid in (select t.roleid from sys_flow_role t where t.role_no in('"+DAGLCommonConstants.ADMIN+"')) and d.status = '"+ZBGLCommonConstants.STATE[1]+"') t"
                    +"	on d.deptid = t.deptid) duser"
                    +"	left join sys_flow_dept sfd on duser.treeid=sfd.tree_id where duser.userid is not null";
			net.sf.json.JSONObject result = userInfoService.getDateBySql(selDangAnAdmin);
			net.sf.json.JSONArray arr = result.getJSONArray("data");
			//arr.getJSONObject(0).get;
			for(int i = 0;i < arr.size();i++){
				String msgContent = "";
				//如果没有需要发消息的内容，则直接跳出，不发短信
				if(null == dangHaoList || ( null != dangHaoList && dangHaoList.size() == 0)){
					break;
				}
				//遍历档号list，查看哪些提交内容需要发消息
				for(int n = 0;n < dangHaoList.size();n++){
					msgContent = msgContent+dangHaoList.get(n)+","+mainTitleList.get(n)+"</br>";
				}
				//依次遍历查询到档案馆管理员id，给他们发消息
				String adminId = arr.getJSONObject(i).getString("userid");
				 //发送消息
                NotityMessage ms = new NotityMessage();
                ms.setSenderId(ConfigConsts.SYSTEM_ID);//系统id
                ms.setSubject(UserUtil.getCruChushiName()+"-"+UserUtil.getCruUserName()+"移交了档案，请及时做审核、汇总。");//消息主题
                ms.setContent(msgContent);//消息内容
                ms.setAccepterId(adminId);//接收人id
	 ...
	 * 退回不归档、退回待修改也要使用这个方法，这里添加一个状态的参数  王磊 20190722
	 * @Date 2018/11/24 9:40
	 * @Param []
	 * @return void
	 **/
	public void updateArchiveEntityStatus(String tName,Map<String,Object> map,String entityStatus){

		String updateStatusSql = "update "+tName+" t set t.archive_entity_status='"+entityStatus+"' where t.recid ='"+String.valueOf(map.get("RECID"))+"'";//更新状态的sql
			...
			for(int j= 0;j<sTableList.size();j++){//目前只更新前两层的数据
				...
				if(j==0){
					...
					for(int m = 0;m<sData.size();m++){
						...
						String updateSTableStatusSql = "update "+sTableName+" t set t.archive_entity_status='"+entityStatus+"' where t.recid='"+sTablerelationData+"'";//更新子表状态的sql
						...
						receiveService.updateState(sTablerelationData, DAGLCommonConstants.WEN_DIAN_STATUS[1]);
						sendService.updateState(sTablerelationData, DAGLCommonConstants.WEN_DIAN_STATUS[1]);
					}
				}else if(j == 1){
					//得到子表的数据
    ...
    private String complexSelect(String complexParam) {
        ...
        //连接符（01：包含；02：不包含；03：大于；04：小于；05：等于；06：不等于）
        ...
        for (int i = 0; i < selectNameArray.size(); i++) {
            if (selectNameArray.get(i) != null && !selectNameArray.get(i).toString().equals("")) {
                ...
                if(inputValueArray.get(i) != null && !inputValueArray.get(i).toString().equals("")){
                	//填写了对应查询的值
					String ljtj = ConditionMap.get(selectConditionArray.get(i).toString());

                    complexSelect += ConnectMap.get(selectConnectArray.get(i).toString())+" ";
					if(ljtj.equals(" not like ") ||ljtj.equals(" != ") ){
						complexSelect += " ( ";
					}
                    complexSelect += selectNameArray.get(i)+" ";
                    complexSelect += ConditionMap.get(selectConditionArray.get(i).toString())+" ";
                    if (ConditionMap.get(selectConditionArray.get(i).toString()).equals(" like ") || ConditionMap.get(selectConditionArray.get(i).toString()).equals(" not like ")) {
                    	//like 和not like 的参数需要用百分号括起来
					...
					if(ljtj.equals(" not like ") ||ljtj.equals(" != ") ){
						complexSelect += " or  "+selectNameArray.get(i)+" is null )";
					}
                }else{
	 ...
	 */
	@Override
	public int isRepetition(String column,String relevancyId, String tName, String chushiId, String userid, String recId, String ranking) {
		...
		if(StringUtils.isNotEmpty(chushiId) && tableName.indexOf("_dak")==-1) {
			...
			chushi=" and cre_chushi_id = ? and cre_user_id = ? and archive_entity_status != ?";
			para.add(chushiId);
			para.add(userid);
			para.add(DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[2]);
		}
		String id = "";
	...
	public net.sf.json.JSONObject recodeGather(String tName, String fids,String categoryCode,String total,String ranking) {
		...
		//根据门类code获得档号规则 查询门类放在公共区域，避免循环中多次查询
		String categoryId = findMenById(categoryCode);
		...
		try {
			...
			for(int i = 0;i < queryForList.size();i++){
				Map<String,Object> map = queryForList.get(i);
				String archive_entity_status = String.valueOf(map.get("ARCHIVE_ENTITY_STATUS"));//防止空指针，如果为空，返回null
				//添加 【已退回待处置】状态数据不能做汇总限制  王磊 20190725
				if(DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[0].equals(archive_entity_status) || DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[2].equals(archive_entity_status) ||DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[3].equals(archive_entity_status)){
					...
					if(pTableList.size()>0){//有父表
						...
						if(DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[1].equals(P_archive_entity_status)){
							...
							/**
							 * 注：这种情况为补录的情况，譬如归档了一卷文书传统档案，后来录入人又想给这一卷添加卷内的情况。
							 * 不过目前需求是，归档之后，录入人看不到这一卷，也就没法给这一卷添加卷内，只能档案管理员在归档库补录直接补录了。
							 * 所以这段逻辑目前不会走！！！后续如果有这方面需求再说。
							 * 王磊 20190724
							 */
							//父表状态为已归档
							//查询最大值
							...
							//当前表序号字段
							String serialNo = (String) dangHaoList.get(index).get("RULE_FIELD");
							String maxSql = "SELECT max(to_number("+serialNo+"))as maxNum,count(1) as countNum FROM "+tName+"_dak t WHERE "+sColName+"='"+newRecodeDAK+"'";
							log.info(maxSql);
							//防止没数据，这里对countNum做一个判断
							Map<String, Object> maxAndCountMap = jdbcTemplate.queryForMap(maxSql);
							//做大值默认0
							int max = 0;
							if(!"0".equals(String.valueOf(maxAndCountMap.get("countNum")))){
								max = Integer.parseInt(String.valueOf(maxAndCountMap.get("maxNum")));
							}
							max = max+1;
							...
							//父表为已移交------------------判断是否有子表
							insertChildDataToDAK(tName,map,Integer.valueOf(ranking),serialNo,no);//更新子表数据和新增正式库数据
							//提交status中绑定的事务
						...
						String maxSql2 = "SELECT max(to_number("+serialNo+")) as maxNum,count(1) as countNum FROM "+tName+"_dak WHERE substr("+recodeNo+",0,"+oldRecodeVal.length()+") = '"+oldRecodeVal+"'";
						//防止没数据，这里对countNum做一个判断 王磊 20190724
						Map<String, Object> maxAndCountMap = jdbcTemplate.queryForMap(maxSql2);
						//做大值默认0
						int max = 0;
						if(!"0".equals(String.valueOf(maxAndCountMap.get("countNum")))){
							max = Integer.parseInt(String.valueOf(maxAndCountMap.get("maxNum")));
	...
	/**
	 * 将子表数据归档到归档库
	 * @author 原作者李颖洁，王磊添加了一个当前表序号字段，一个当前序号值
	 * @Date 2019年7月24日 下午2:59:12
	 * @param tName 当前表名
	 * @param map 当前表数据
	 * @param ranking 当前属于第几层
	 * @param recodeNo 该门类的顶层表序号字段
	 * @param no 顶层表序号字段值
	 * @throws Exception
	 */
	private void insertChildDataToDAK(String tName,Map<String,Object> map,int ranking,String recodeNo,String no)throws Exception{
		try {
			String sTableSql =  "	select s.*\n" +
					"  		from tables_relation s\n" +
			...
			if(sTableList.size()>0){
				...
				//获取当前日期
				String nowDay = new SimpleDateFormat("yyyyMMdd").format(new Date());
				//记录需要更新的父表序号，譬如项目号或者是案卷号
				String updatePaSeriaNoPart = " ,"+recodeNo+"='"+no+"' ";
				do{//目前只更新前两层的数据
					//获取子表信息
					ranking += 1;
					Map<String,Object> sTableRelationMap = sTableList.get(j);
						...
						for (Map<String, Object> map2 : ids) {
							...
							if(ids1.size()>0){
								...
								for (int m=0;m<ids1.size();m++) {
									...
									//添加第一层的序号，譬如项目号或者案卷号开始 王磊 20190724
									sqls[m] += updatePaSeriaNoPart;
									//添加第一层的序号，譬如项目号或者案卷号结束 王磊 20190724
	...
	public net.sf.json.JSONObject borrowDangAnToChangeStatus(String recid, String tableName) {
		...
		try {
    		...
    		for(Map<String,Object> map : tableRelationPList){
    			...
    			try{
					pData = jdbcTemplate.queryForMap(selP,tempRecid);
				}catch (Exception e){
					//未组卷或者其他找不到父级的档案数据，会报错，这里忽略错误信息执行下一个循环
    				continue;
				}
    			//记录当前记录
	 ...
	 */
	@Override
	public Map<String, Object> adddd(String id, String tName,String ranking,String all) {
	...
	if("1".equals(ranking)){
		if("1".equals(all) || "2".equals(all)){
			...
			pageNum = queryForMap.get(0).get("PAGE_NUM")==null?"":queryForMap.get(0).get("PAGE_NUM").toString();
		}else{
			...
			if(pTableList.size()>0) {
				...
				//子表字段
				String sColName = String.valueOf(relationMap.get("S_COL_NAME"));
				String sColData = queryForMap.get(0).get(mColName.toUpperCase()).toString();
				//查询子表数据
				...
				if(tName.indexOf("_dak")==-1){
					...
					sTableSql += "   and (ARCHIVE_ENTITY_STATUS != '"+DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[2] +"' or ARCHIVE_ENTITY_STATUS is null)\n"+
							"   and cre_user_id = '"+queryForMap.get(0).get("CRE_USER_ID")+"'\n" +
							"   and cre_chushi_id = '"+queryForMap.get(0).get("CRE_CHUSHI_ID")+"'\n" ;
				}
				List<Map<String,Object>> sTableList = jdbcTemplate.queryForList(sTableSql);
				int page_num = 0;
				for(int i=0;i<sTableList.size();i++){
					page_num += Integer.parseInt(sTableList.get(i).get("PAGE_NUM")==null?"0":sTableList.get(i).get("PAGE_NUM").toString());
				}
				pageNum = ""+page_num;
			}
		}
	...
	map.put("security_class",queryForMap.get(0).get("SECURITY_CLASS")==null?"":queryForMap.get(0).get("SECURITY_CLASS"));
	map.put("leibiehao",queryForMap.get(0).get("LEIBIEHAO")==null?"":queryForMap.get(0).get("LEIBIEHAO"));
			...
			if(Integer.parseInt(nowData.get(0).get(dhgz.get(dhgz.size()-levelNum).get("RULE_FIELD").toString().toUpperCase()).toString()) < Integer.parseInt(newXuHao)){
				...
				if(tName.contains("_dak")){
					...
					if(newDataCount == 0){
						//不存在，提示用户应该输入一个存在的序号
						//查询该档案，做更新操作
						List<Object> thisDataPara = new ArrayList<>();
						thisDataPara.add(recid);
						String queryThisSql = "select * from "+tName+" t where t.recid = ? ";
						List<Map<String,Object>> queryForList = jdbcTemplate.queryForList(queryThisSql,thisDataPara.toArray());
						//把档号和序号做修改，封装进去
						//档号字段
						queryForList.get(0).put(danghaoColumn.toUpperCase(),changeDanghao);
						//最后一位，根据档号规则和层数决定
						String lastCol = dhgz.get(dhgz.size()-levelNum).get("rule_field")==null?"":dhgz.get(dhgz.size()-levelNum).get("rule_field").toString();
						queryForList.get(0).put(lastCol.toUpperCase(),newXuHao);
						for(Map<String,Object> map:queryForList) {
							//把map的key转成小写
							Map<String, Object> resultMap = new HashMap<>();
							Set<String> keySet = map.keySet();
							for (String key : keySet){
								String newKey = key.toLowerCase();
								resultMap.put(newKey, map.get(key)==null?"":map.get(key));
							}
							//顺序调整只能调整第一层
							//把对象转成String，模拟修改表单，修改对应的属性后封装成String，动态修改
							net.sf.json.JSONObject jsonStr=net.sf.json.JSONObject.fromObject(resultMap);
							int updateNum = dynamicUpdate(jsonStr.toString(), tableName,"recid",map.get("RECID").toString(), levelNum+"", "1",JSON.toJSONString(dhgz));
							if(updateNum>0){
								jsonb.put("flag", "1");
								return jsonb;
							}else{
	...
	public net.sf.json.JSONObject doAdjust(List<Map<String, Object>> nowData,List<Map<String, Object>> dhgz,
		...
		//根据表名获取含有数据字典的字段
		String queryselectDataSql ="select t.* from table_struct_description t where t.table_name = ? and t.column_input_type ='S' and t.column_select_no is not null";
		List<TableStructDescription> selectData = jdbcTemplate.query(queryselectDataSql, new BeanPropertyRowMapper<>(TableStructDescription.class),tableName);
		try {
			if(levelNum == 1){
				//当前门类只有一层，从配置里取出档号字段。
				...
				for(int i=0;i<dhgz.size()-levelNum;i++){
					...
					for(int j=0;j<selectData.size();j++){
						//数据字典的字段
						if(dhgz.get(i).get("RULE_FIELD").toString().toLowerCase().equals(selectData.get(j).getColumnName()) && !"fonds_no".equals(dhgz.get(i).get("RULE_FIELD").toString().toLowerCase())){
							//含有字典值
							//根据字段名称和表名去表描述查询出字典值，如果存在字典值，并且不是全宗的，取code拼接档号2019/07/16-王富康
							List<DataDictionary> listByMark = dataDictionaryDao.getListByMark(selectData.get(j).getColumnSelectNo());
				...
				if("asc".equals(order)){
					...
					for(int xh=start;xh<=end;xh++){
						...
						}while(pColValOld.length() < Integer.parseInt(dhgz.get(dhgz.size()-2).get("NUMB_LENGTH").toString()));
						//默认管理员归档库操作
	 ...
	 **/
	@Override
	public int queryCountAdd(String danghao,String columnName, String tName, String chushiid, String userid, String lastColumnName, String ranking){
		...
		List<Object> para = new ArrayList<>();
		String sql = "select max(to_number("+col+")) from "+tableName+" where "+columnName+" like ?  ";
		para.add(danghao+"%");
		if(tName.indexOf("_dak") == -1){
			//除去已汇总的文件2019/07/12
			sql += " and cre_chushi_id = ? and cre_user_id = ? ";
			//and archive_entity_status != ? ";
			para.add(chushiid);
			para.add(userid);
			//para.add(DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[2]);
		}
		...
		if(pTableList.size()>0){
			...
			String sTableSql =	"select *\n" +
				...
				if(tName.indexOf("_dak")==-1){
					//单位预立库根据立卷单位和创建人查询,并且是未归档的数据
					sTableSql += "   and (ARCHIVE_ENTITY_STATUS != '"+DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[2] +"' or ARCHIVE_ENTITY_STATUS is null)\n"+
							"   and cre_user_id = '"+relevancyUserData+"'\n" +
			...
			for(int i=0;i<Integer.parseInt(count);i++){
				...
				//判重
				int repetition = isRepetition(danghaoCol,newDanghao, sTableName, relevancyChushiData,relevancyUserData, recid, ranking);
				if(repetition==0) {
					//不重复，什么都不做
				}else {
					//重复
	 ...
	 * @return void
	 **/
	public void setStratAndEndTime(String ranking,String ptableName,List<Map<String,Object>> pData){
		//需要更新的表名
		String pTableName = "";
		//文件开始时间
		String docStartTime = "";
		...
		List<Object> para = new ArrayList<>();
			...
			//第一层，查询子表，更新文件开始时间，和文件结束时间，适用操作：两层档案第一层档案调整的调入调出
			...
			//得到子表及关联的信息
			List<Map<String,Object>> sTableList = jdbcTemplate.queryForList(sTableSql,ptableName);
			//循环更新
			for(int j = 0;j<sTableList.size();j++) {
				Map<String, Object> sTableRelationMap = sTableList.get(j);
				List<Object> sPara = new ArrayList<>();
				...
				List<Map<String, Object>> sData = new ArrayList<>();
				//去变化的id，没有就证明是原本的就在卷内的信息，直接根据去当前数据的id
				String value = pData.get(0).get(PColnumName) == null ? "" : pData.get(0).get(PColnumName).toString();
				String userid = pData.get(0).get("CRE_USER_ID") == null ? "" : pData.get(0).get("CRE_USER_ID").toString();
				String chushiid = pData.get(0).get("CRE_CHUSHI_ID") == null ? "" : pData.get(0).get("CRE_CHUSHI_ID").toString();
				pRecid = pData.get(0).get("RECID") == null ? "" : pData.get(0).get("RECID").toString();

				String findStratTimeSql = "select min(t.created_date) from "+sTableName+" t where t."+sColnumName+" = ? and t.archive_flag='"+DAGLCommonConstants.ARCHIVE_FLAG[0]+"' ";
				String findEndTimeSql = "select max(t.created_date) from "+sTableName+" t where t."+sColnumName+" = ? and t.archive_flag='"+DAGLCommonConstants.ARCHIVE_FLAG[0]+"' ";
				sPara.add(value);
				if (ptableName.indexOf("_dak") > 1) {
					//归档库，不需要考虑录入人和立卷单位
				} else {
					findStratTimeSql+=" and t.cre_user_id = ? and t.cre_chushi_id=?";
					findEndTimeSql+=" and t.cre_user_id = ? and t.cre_chushi_id=?";
					sPara.add(userid);
					sPara.add(chushiid);
				}
				docStartTime = jdbcTemplate.queryForObject(findStratTimeSql, String.class,sPara.toArray());
				docEndTime = jdbcTemplate.queryForObject(findEndTimeSql, String.class,sPara.toArray());
			}
		String updateSql = "update "+pTableName+" t set t.doc_start_time = ? ,t.doc_end_time = ? where t.recid = ?";
		para.add(docStartTime);
		para.add(docEndTime);
		para.add(pRecid);
		jdbcTemplate.update(updateSql,para.toArray());

	}

	/**
	 * @Author 王富康
	 * @Description //TODO 根据用户查询上次录入的数据，用来回填值继承字段的信息
	 * @Date 2019/7/18 19:58
	 * @Param []
	 * @return java.util.List<java.util.Map<java.lang.String,java.lang.Object>>
	 **/
	@Override
	public List<Map<String, Object>> findUserLastOne(String tName,String userId, String chushiId, String danghao, String columnName){
		...
		List<Object> para = new ArrayList<>();
		String sql="select s.* from (select t.* from "+tableName+" t where t."+columnName+" like ? order by t.cre_time desc,t.recid desc) s ";
		para.add(danghao+"%");
		if(tableName.indexOf("_dak")==-1){
			//单位库排除已归档的数据
			sql+="  where s.cre_user_id = ? and s.cre_chushi_id  = ? and rownum = 1";
			para.add(userId);
			para.add(chushiId);
		}else{
			//单位库排除已归档的数据
	...
	/**
	 * 退回档案，调整数据及子表数据状态为：已退回待处置
	 * @author 王磊
	 * @Date 2019年7月22日 上午10:56:23
	 * @param tName 当前勾选数据所在表
	 * @param ids 当前勾选的数据id串，逗号隔开
	 * @param entityStatus 更新操作：退回不归档、退回修改
	 * @param backReason 退回原因，目前用不到，这里留作扩展
	 */
	@Override
	public net.sf.json.JSONObject backRecords(String tName, String ids, String entityStatus, String backReason) {

		net.sf.json.JSONObject json = new net.sf.json.JSONObject();
		//查询表关联获得档号字段
		String selTableRelations =
				"SELECT t.*\n" +
						"  FROM tables_relation t, table_description s\n" +
						" WHERE t.m_table_name = s.table_name\n" +
						"   and t.m_table_name = ? ";//排除父节点为门类的数据
		List<Map<String,Object>> tableRelations = jdbcTemplate.queryForList(selTableRelations,tName);
		String dangHaoColName = "";
		if (tableRelations.size()>0){
			Map<String,Object> relationMap = tableRelations.get(0);
			//档号字段
			dangHaoColName = String.valueOf(relationMap.get("M_COL_NAME"));
		}else{
			dangHaoColName = "archive_no";
		}
		//查询勾选的多行数据列表
		List<Map<String,Object>> queryForList = getSelectedData(tName,ids);
		//定义创建人和消息内容存储结构:key为创建人id，value为消息内容
		Map<String,Object> userIdMsgs = new HashMap<String, Object>();
		//遍历数据调整当前及子表数据状态
		for(Map<String,Object> map : queryForList){
			//如果当前文件状态不是DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[1](已提交待汇总)，则不处理
			if(!DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[1].equals(String.valueOf(map.get("ARCHIVE_ENTITY_STATUS")))){
				//json
				json.put("flag","0");
				json.put("msg","请选择档案实体状态为【"+DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[1]+"】的档案进行"+entityStatus+"操作！");
				return json;
			}
		}
		for(Map<String,Object> map : queryForList){
			...
			//状态为DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[1](已移交待汇总)，需要调整数据及子表数据状态为：已退回待处置
			updateArchiveEntityStatus(tName,map,DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[3]);
			//判断是否包含创建人及对应的消息，如果不包含就追加新的消息
			if(!userIdMsgs.containsKey(String.valueOf(map.get("CRE_USER_ID")))){
				userIdMsgs.put(String.valueOf(map.get("CRE_USER_ID")), String.valueOf(map.get(dangHaoColName))+","+String.valueOf(map.get("MAINTITLE")));
			}else{
				//如果包含了，就追加
				userIdMsgs.put(String.valueOf(map.get("CRE_USER_ID")), userIdMsgs.get(String.valueOf(map.get("CRE_USER_ID")))+"</br>"+String.valueOf(map.get(dangHaoColName))+","+String.valueOf(map.get("MAINTITLE")));
			}
		}
		//给对应的创建人发送消息
		for(String userId : userIdMsgs.keySet()){
			NotityMessage ms = new NotityMessage();
			//系统id
	        ms.setSenderId(ConfigConsts.SYSTEM_ID);
	        //消息主题
	        ms.setSubject("档案管理员-"+UserUtil.getCruUserName()+"退回了档案("+entityStatus+")，请及时做确认。");
	        //消息内容
	        ms.setContent(String.valueOf(userIdMsgs.get(userId)));
	        //接收人id
	        ms.setAccepterId(userId);
	        //0：未读,1：已读
	        ms.setStatus("0");
	        //3：站内消息
	        ms.setType("3");
	 ...
	 **/
	@Override
	public List<Map<String, Object>> getYearFolderNo(String tName, String baseFolderUnit, String lrrId, String folderNo){
		...
		//找到父表,可以得到表名
		Map<String, Object> relationData = findDAGLByTableName(tableName);
		//查询父表数据
		//父表表名
		String ppTableName = String.valueOf(relationData.get("M_TABLE_NAME"));
		List<Object> para = new ArrayList<>();
		para.add(baseFolderUnit);
		para.add(lrrId);
		para.add(folderNo+"%");
		String yearSql="select t.* from "+ppTableName+" t where t.cre_chushi_id = ? and t.cre_user_id = ? and t.folder_no like ? order by year_folder_no asc";
		List<Map<String, Object>> list = jdbcTemplate.queryForList(yearSql,para.toArray());
		return list;
	}
	 ...
	 **/
	@Override
	public List<Map<String, Object>> queryPData(String tName,String jsonStr) {

		//防止SQL注入，先去查询表字段集合，判断是否含有传过来的字段，含有，则取查询出来的字段名，不用前台传过来的字段名
		List<String> colName = findColByTableName(tName);
		Map<String, String> parseObject = com.alibaba.fastjson.JSONObject.parseObject(jsonStr,
				new TypeReference<Map<String, String>>() {
				});
		//关联信息
		...
		if (pTableList.size() > 0) {
			...
			for (Entry<String, String> entry : parseObject.entrySet()) {
				for (int i = 0; i < colName.size(); i++) {
					//获取数量
					if (sColName.toLowerCase().equals(entry.getKey().toLowerCase())) {
						sColData = entry.getValue();
						break;
					}
					if ("cre_chushi_id".equals(entry.getKey().toLowerCase())) {
						creChushiId = entry.getValue();
						break;
					}
				}
			...
			if (tName.indexOf("dak") == -1) {
				//单位预立库添加录入人和立卷单位标识
				pDataSql += " and t.CRE_USER_ID='" + UserUtil.getCruUserId() + "' and t.CRE_CHUSHI_ID='" + creChushiId + "' and t.archive_entity_status !='"+DAGLCommonConstants.ARCHIVE_ENTITY_STATUS[2]+"'";
			}
			List<Map<String, Object>> pTableDataList = jdbcTemplate.queryForList(pDataSql);
			return pTableDataList;
		}
		return null;
	}
	...
	/**
	 * @Author 王富康
	 * @Description //TODO map的value如果是null处理成空字符串
	...
	public void synchronousUpdate(){
		//查询所有的业务表
		String queryTableSql = "select t.* from table_description t ";
		List<Map<String, Object>> pTableDataList = jdbcTemplate.queryForList(queryTableSql);
		for(int i =0;i<pTableDataList.size();i++){
			try{

				//拼接更新sql
				String updateSql = "update "+pTableDataList.get(i).get("TABLE_NAME")+" s\n" +
						"   set s.cre_chushi_id =\n" +
						"       (select t.code\n" +
				...
				log.info("同步表"+pTableDataList.get(i).get("TABLE_NAME")+"出现错误！");
			}
		}
	}
}
