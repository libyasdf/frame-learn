        ...
        if(StringUtils.isNotBlank(userId)){
        	if (StringUtils.isNotBlank(messageURL)) {
				if (messageURL.contains("?")) {
					messageURL = messageURL += "&workItemId1=" + entity.getId();
				}else{
					messageURL = messageURL += "?workItemId1=" + entity.getId();
				}
			}
    ...
    public SysWaitNoflow saveWaitNoflowSystem(SysWaitNoflow entity,Boolean sendMsg,String subject,String content,String messageURL) {
        ...
        if("1".equals(type)){
            ...
            if("0".equals(type)){//只有部门
                userList = userInfoService.getAllUserByDeptId(entity.getReceiveDeptId());
            }
            ...
            for (UserInfo user:userList){
                if(!userids.contains(String.valueOf(user.getUid()))){//如果存在当前用户则不再添加（处理一人多岗收到两条消息问题）
                    userids += String.valueOf(user.getUid())+",";
                }
            }
        }
        String creTime = JDateToolkit.getNowDate4();
            ...
            if ("1".equals(isOpen.get("isOpen").toString())) {
                if (sendMsg) {
					...
					for (String userid : userIdArray) {
						sendMsgsByUid(userid, subject, content, messageURL,creTime,entity);
					}
				}
            }
        }
