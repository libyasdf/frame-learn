...
public class KqglAttendenceHandleServiceImpl implements KqglAttendenceHandleService{
	...
	public KqglAttendenceHandle getKqglAttendenceHandle(String importDate,String cardNumber) {
		...
		//卡号
		if (StringUtils.isNotBlank(cardNumber)) {
			querySql.append("   and t.card_number = ?");
			para.add(cardNumber);
		}
		//日期
	...
	public List<KqglAttendenceHandle> findByCondition(String startDate, String endDate, String cardNum) {
		StringBuilder querySql = new StringBuilder();
		List<Object> para = new ArrayList<>();
		String inString = CommonUtils.solveSqlInjectionOfIn(cardNum, para);
		querySql.append("select * ");
		querySql.append("	from kqgl_attendence_handle t");
		querySql.append("	where t.card_number in (" + inString + ") ");
		//卡号
		if (StringUtils.isNotBlank(startDate) && StringUtils.isNotBlank(endDate)) {
			querySql.append("   and t.import_date >= ? and t.import_date <= ? ");
			para.add(startDate);
			para.add(endDate);
		}
		List<KqglAttendenceHandle> list = jdbcTemplate.query(querySql.toString(), new BeanPropertyRowMapper<>(KqglAttendenceHandle.class),para.toArray());
	...
	public List<KqglAttendenceHandle> getKqInfo(String startDate, String endDate, String cardNum) {
		List<Object> para = new ArrayList<>();
		String inString = CommonUtils.solveSqlInjectionOfIn(cardNum, para);
		StringBuilder kqSql = new StringBuilder("select t.* from kqgl_attendence_handle t where t.card_number in (" + inString + ")");
