...
var i = 0;
var canmove = true;
$("#xxtx-container li").mouseenter(function(){
	
    canmove=false;
    clearInterval(li_timer);
    i=$(this).index();
    $('#xxtx-container .gr_block_info_tab_ul li').eq(i).addClass('active').siblings().removeClass('active');
	  $('#xxtx-container .gr_block_info_tab_ul li').eq(i).addClass('true').siblings().removeClass('true');
//    $('#xxtx-container .gr_index_block_main').hide();
//    $('#xxtx-container .gr_index_block_main').eq(i).show();
	  if(i==0){
		  $("#tabOne").show();
		  $("#tabTwo").hide();
		  $("#tabThree").hide();
  ...
  $("#xxtx-container").mouseenter(function(){//只要用户鼠标在这个tab1区域内，就不自动跳转
        canmove=false;
    }).mouseleave(function(){
        clearInterval(li_timer);
