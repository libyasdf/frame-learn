$(function () {
    ...
    // 获取数据
    $.get("/zhbg/xxkh/xxkhPaperInfo/getById",{id:id,zz:Math.random()}, function (data) {
        console.log(data);
...
var $examNote = function() {
    var $dom = $(''
        +' <div class="exam_page_top">'
            +'温馨提醒：<br> 1. 答完试卷后请务必点击<b style="color:red">（交卷）</b>按钮，否则考试结果无法录入到系统。<br> 2. 请在规定时间内完成考试，考试时间结束系统会自动提交试卷。<br> 3. 考试结束后可查看客观题分数，主观题分数需要人工评卷完成之后，方可查看。'
        +'</div>');
...
var $shortAnswer  = function() {
    ...
    function init(data) {
        ...
        for(var i = 0; i < data.question.length; ++i) {
            ...
            // 添加选项
            var $optionClone = $option.clone();
            $optionClone.find('textarea').attr("name", data.question[i].id);
            $dom.find('.exam_answer:eq(' + i + ')').append($optionClone);
        }
        $('.exam_page').append($dom);
    }
