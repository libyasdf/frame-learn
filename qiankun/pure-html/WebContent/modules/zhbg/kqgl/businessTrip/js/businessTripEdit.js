...
$(function() {
	...
	if ($("#id").val() != "") {
		...
		$("#applicationUnitId").val(getcookie("deptid"));
...
function commitFlow() {
	if(!$("#businessTripTitle").val()){
		...
		var title=curDate+creUserName+creChuShiName+"出差申请";
		//var title=creUserName+"于"+curDate+"发出出差222申请";
		...
		if (plan != "") {
			...
			if($("#roleNo").val()==''){
				...
				$.post("/zhbg/kqgl/businessTrip/getRoleNoByUserId", {
					userId:$("#creUserId").val(),deptId:$("#applicationUnitId").val()
				}, function(data) {
					$("#roleNo").val(data);
				});
