...
$(function () {
    ...
    // 初始化左侧树
   typeTreeLj.init({});
    nodes1 = $.fn.zTree.getZTreeObj("treeDemo1").getSelectedNodes()
    if(nodes1.length>0){
        typeTreeLjToMenlei.init({isQ2:false,columnName:nodes1[0].columnName,id:nodes1[0].id});
        // 初始化页面
        initPage();
    }else{
        layer.alert("暂无录入人！",{icon:0});
    }
        ...
        success: function (data) {
            ...
            columnsArray.push({
                ...
                , formatter: function (value, row, index) {  //直接定义function,return就是html代码
                    if(!(row.ARCHIVE_ENTITY_STATUS=="已归档")){
                        var html = "";
            ...
            TableInit.init({
                ...
                queryParams: function (params) {
                    ...
                    params.danweiku='woshiyuliku',
                    params.all=all,
                    params.ljdanweiAndRenyuan=getRenKeyValue(),
...
function newlyAdded() {
    ...
    var object = JSON.parse(getRenKeyValue());
    var basefolderUnit = object['LJDW_MARK'];
    layer.open({
        type: 2,
        title:[ '新增','font-size:16px;font-weight:bold;'],
        content: '/modules/dagl/bmgl/form.html?categoryCode=' + $.fn.zTree.getZTreeObj("treeDemo").getSelectedNodes()[0].categoryCode+"&fids="+getFids()+"&where=yulikuLjdw&basefolderUnit="+basefolderUnit,
        area: ['1100px', '660px'],
        offset: '30px',
        resize: false,
        maxmin: true,
