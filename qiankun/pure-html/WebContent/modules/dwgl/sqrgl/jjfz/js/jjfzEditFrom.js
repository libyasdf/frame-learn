$(function() {
 ...
 /*   $("#determineActivistTime").val(getToday());*/
    if(regVlaue(theRequest.addOrUpdate)=="add"){
        $("#applicationTime").val(getToday());
        $("#determineActivistTime").val(getToday());
        if( $("#determineActivistTime").val()!=""){
            if($("#developmentObjectTime").val()!=""){
                $("#typeOfPersonnel").val("01,02,03");
            }else{
                $("#typeOfPersonnel").val("01,02");
            }
        }else{
            $("#typeOfPersonnel").val("01");
        }
    ...
    if(id != ""){
        ...
        httpRequest("get","/djgl/ddjs/sqrgl/sqrxx/edit",datas,function (data){
            if($("#opertation").val() == "VIEW"){
                ...
                strs=data.data.typeOfPersonnel.split(",")
                //如果人员类型为发展对象，不能更改发展为积极分子时间
               /* if( strs.includes("03")){
                    $('#determineActivistTime').attr('disabled',true);
                }*/
            }
...
function LoadFormpage(pageId){
    ...
    if(pageId == "formpage2"){
        ...
        if($('#determineActivistTime').val()!=""){
            var strs= new Array(); //定义一数组
            strs= $('#typeOfPersonnel').val().split(",")
            //如果人员类型为发展对象，不能更改发展为积极分子时间
            /* if( strs.includes("03")){
             $('#determineActivistTime').attr('readonly',true);
             }*/
        }
       /* if($('#applicationTime').val()!=""){
        ...
        if($("#applicationTime").val()!=""&&$("#developmentType").val()!=""&&$("#determineActivistTime").val()!="") {
            commitForm("0", false);
        }       
 ...
 */
function saveForm(){
    if($("#determineActivistTime").val()==""&&$("#developmentObjectTime").val()!=""){
        layer.msg("请去掉确定为发展对象时间或者添加确定为积极分子时间！", {
            icon : 1
        });
      return null;
    }
