$(function() {
	...
	if(regVlaue(theRequest.isAdd=="add")){
		...
		$("#partyOrganizationName").val(decodeURI(theRequest.orgName));
		/*$("#partyOrganizationNameTitle").val(decodeURI(theRequest.orgName));*/
	//	document.getElementById("partyOrganizationNameTitle").innerText(decodeURI(theRequest.orgName));
	}
	...
	if($("#id").val() != ""){
		...
		httpRequest("get","/djgl/ddjs/shyk/ndzj/edit",datas,function (data){
...
function saveForm(){
	...
	if(bootstrapValidator.isValid()){
		$.ajax({
			type: "POST",
			url: "/djgl/ddjs/shyk/ndzj/saveNdzj",
			data:$("#form").serialize(),
			async: false,
